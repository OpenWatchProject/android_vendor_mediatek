...
public class RcsSettings {
     ...
     * </ul>
     */
    public final static String CONFIGURATION_STATUS = "com.mediatek.presence.CONFIGURATION_STATUS_TO_APP";
     ...
     * Tag
     */
    private final static String TAG = "PresenceSettings";

    private final static String OPERATOR_CODE_CMCC = "OP01";

    private final static String OPERATOR_CODE_ATT  = "OP07";

    private final static String OPERATOR_CODE_TMO  = "OP08";

    private final static String OPERATOR_CODE_VZW  = "OP12";
     ...
     * Current instance
     */
    //TODO: Need to remove while DSDS IT done
    ...
    private boolean mVolteSupport = false;
    private boolean mVilteSupport = false;
    private boolean mImsCapControlByUser = false;
    private int mSlotId = 0;
    private final int SLOT0_INDEX = 0;
    private final int SLOT1_INDEX = 1;
     ...
     */
    public RcsSettings(Context ctx, int slotId) {
        mContext = ctx;
        this.cr = ctx.getContentResolver();
        mSlotId = slotId;
        LOGGER = Logger.getLogger(mSlotId, TAG);
        switch (slotId) {
            case SLOT0_INDEX:
                databaseUri = RcsSettingsData.CONTENT_URI_SLOT0;
                break;
            case SLOT1_INDEX:
                databaseUri = RcsSettingsData.CONTENT_URI_SLOT1;
                break;
        }
    }
    ...
    public void writeParameter(String key, String value) {
        ...
        if (RcsSettingsData.SERVICE_ACTIVATED.equals(key)) {
            LOGGER.debug("writeParameter: Key ="+ key +": " + values);
        }
    }
     ...
     * @param value Value
     */
    public void writeRCSParameter(String key, String value) {
        if ((key == null) || (value == null)) {
            return;
        }
        ContentValues values = new ContentValues();
        values.put(RcsSettingsData.KEY_VALUE, value);
        String where = RcsSettingsData.KEY_KEY + "='" + key + "'";
        cr.update(RcsSettingsData.RCS_CLIENT_SETTING_URI, values, where, null);
    }
    ...
    public boolean isServiceActivated() {
        ...
        LOGGER.error("isServiceActivated = " + result);
        return result;
    }

    public boolean isCmccSimCard() {
        String optr = SystemProperties.get("persist.vendor.operator.optr");
        if (OPERATOR_CODE_CMCC.equalsIgnoreCase(optr)) {
            return true;
        }
        return false;
    }

    public boolean isTmoSimCard() {
        String optr = SystemProperties.get("persist.vendor.operator.optr");
        if (OPERATOR_CODE_TMO.equalsIgnoreCase(optr)) {
            return true;
        }
        return false;
    }

    public boolean isAttSimCard() {
        String optr = SystemProperties.get("persist.vendor.operator.optr");
        if (OPERATOR_CODE_ATT.equalsIgnoreCase(optr)) {
            return true;
        }
        return false;
    }

    public boolean isVzwSimCard() {
        String optr = SystemProperties.get("persist.vendor.operator.optr");
        if (OPERATOR_CODE_VZW.equalsIgnoreCase(optr)) {
            return true;
        }
        return false;
    }
    ...
    public boolean isOP08TestSim() {
        ...
        String currentOperator = tm.getSimOperatorName(subIds[0]);

        //just check slot0 slot1.
        boolean testSim = SystemProperties.get("vendor.gsm.sim.ril.testsim").equals("1") ||
                          SystemProperties.get("vendor.gsm.sim.ril.testsim.2").equals("1");

        //compare the operator name
        if(currentOperator.equals("R&S 3G TestSIM") && testSim) {
            result = true;
        }
        if (LOGGER.isActivated()) {
            LOGGER.debug("isOP08TestSim: currentOperator:" + currentOperator + "testSim:" + testSim);
        }
    ...
    public boolean isTestSim() {
        //just check slot0 slot1.
        boolean result = SystemProperties.get("vendor.gsm.sim.ril.testsim").equals("1") ||
                          SystemProperties.get("vendor.gsm.sim.ril.testsim.2").equals("1");
        return result;
    }
    /**
    ...
    */
    ...
    public Capabilities getMyCapabilities() {
        ...
        capabilities.setStandaloneMsgSupport(isStandaloneMsgSupport());
        ...
        capabilities.setVideoSharingDuringCallSupport(istVideoSharingDuringCallSupported());
        capabilities.setVideoSharingOutsideOfVoiceCallSupport(
                isVideoSharingOutsideOfVoiceCallSupported());
        capabilities.setGeoLocationPullUsingFileTransferSupport(
                isGeoLocationPullUsingFileTransferSupported());
        capabilities.setGeoLocationPullSupport(isGeoLocationPullSupported());
        capabilities.setRcsIpVoiceCallSupport(isRcsIpVoiceCallSupported());
        capabilities.setRcsIpVideoCallSupport(isRcsIpVideoCallSupported());
        capabilities.setRcsIpVideoOnlyCallSupport(isRcsIpVideoOnlyCallSupported());

        //set burn after read capability
        capabilities.setBurnAfterRead(isCPMBurnAfterReadingSupported());

        capabilities.setChatbotSupport(isChatbotSupported());
        capabilities.setChatbotSaSupport(isChatbotSaSupported());
        capabilities.setBotVersion(getBotVersion());
        capabilities.setCallcomposerSupport(isCallcomposerSupported());

        if(RcsUaAdapterManager.getRcsUaAdapter(mSlotId)!=null) {
            //if registred via single registartion, ir94 voice call is enabled
            capabilities.setIR94_VoiceCall(RcsUaAdapterManager.getRcsUaAdapter(mSlotId).isImsRegistered());
            capabilities.setIR94_VideoCall(RcsUaAdapterManager.getRcsUaAdapter(mSlotId).isImsRegistered()
                    && isIR94VideoCallSupported());
            capabilities.setIR94_DuplexMode(RcsUaAdapterManager.getRcsUaAdapter(mSlotId).isImsRegistered()
                    && isIR94VideoCallSupported());
        }
    ...
    */
    ...
    public boolean isFallbackToPagerModeSupported() {
        return SimUtils.isTmoSimCard(mSlotId) && isCPMPagerModeSupported();
    }
     ...
     * @return Timeout in seconds
     */
    public int getNonRcsCapabilityExpiryTimeout() {
        int result = 72 * 3600;
        if (instance != null) {
            try {
                result = Integer
                         .parseInt(readParameter(RcsSettingsData.NON_RCS_CAPABILITY_EXPIRY_TIMEOUT));
            } catch (Exception e) {}
        }
        return result;
    }
    ...
    */
    ...
    public boolean isIPVideoCallSupported() {
        ...
        LOGGER.debug("isIPVideoCallSupported:" + result);
        return result;
    }
    ...
    public void backupAccountSettings(String account) {
        ...
        */        try {
            String packageName = "com.mediatek.presence";
...
// The observer list
    ...
    public void setRcseOnlyApnState(final boolean state) {
        ...
        new AsyncTask() {
            ...
            protected Object doInBackground(Object... arg0) {
                ...
                writeParameter(RcsSettingsData.RCS_APN_SWITCH, value);
                if(LOGGER.isActivated()) {
    ...
    */
    ...
    public boolean isCPMStandAloneModeSupported() {
        boolean result = false;
        return result;
    }
    ...
    * @return Boolean
    ...
    public int getMaxPagerContentSize() {
        ...
        if (instance != null) {
            try {
                result = Integer
                         .parseInt(readParameter(RcsSettingsData.MAX_PAGER_CONTENT_SIZE));
            } catch (Exception e) {
            }
        }
        return result;
    ...
    */
    public boolean getProfileAuth() {
        boolean result = false;
        if (instance != null) {
            result = Boolean.parseBoolean(readParameter(RcsSettingsData.PROFILE_AUTH));
        }
        return result;
    }
     ...
     * @return Auth Type
     */
    public boolean getNABAuth() {
        boolean result = false;
        if (instance != null) {
            result = Boolean.parseBoolean(readParameter(RcsSettingsData.NAB_AUTH));
        }
        return result;
    }
     ...
     * @return Auth Type
     */
    public boolean getPublicAccountAUTH() {
        boolean result = false;
        if (instance != null) {
            result = Boolean.parseBoolean(readParameter(RcsSettingsData.PUBLIC_ACCOUNT_AUTH));
        }
        return result;
    }
    ...
    public boolean getSSOAuth() {
        boolean result =  false;
        if (instance != null) {
            result = Boolean.parseBoolean(readParameter(RcsSettingsData.SSO_AUTH));
        }
        return result;
    }
    ...
    public String getProfileAddress() {
        String result = null;
        if (instance != null) {
            result = readParameter(RcsSettingsData.PROFILE_ADDRESS);
        }
        return result;
    }
     ...
     * @return port
     */
    public String getProfileAddressPort() {
        String result = null;
        if (instance != null) {
            result = readParameter(RcsSettingsData.PROFILE_ADDRESS_PORT);
        }
        return result;
    }
     ...
     * @return Address
     */
    public String getProfileAddressType() {
        String result = null;
        if (instance != null) {
            result = readParameter(RcsSettingsData.PROFILE_ADDRESS_TYPE);
        }
        return result;
    }
     ...
     * @return address
     */
    public String getNABAddress() {
        String result = null;
        if (instance != null) {
            result = readParameter(RcsSettingsData.NAB_ADDRESS);
        }
        return result;
    }
     ...
     * @return port
     */
    public String getNABAddressPort() {
        String result = null;
        if (instance != null) {
            result = readParameter(RcsSettingsData.NAB_ADDRESS_PORT);
        }
        return result;
    }
     ...
     * @return Address
     */
    public String getNABAddressType() {
        String result = null;
        if (instance != null) {
            result = readParameter(RcsSettingsData.NAB_ADDRESS_TYPE);
        }
        return result;
    }
     ...
     * @return address
     */
    public String getPublicAccountAddress() {
        String result = null;
        if (instance != null) {
            result = readParameter(RcsSettingsData.PUBLIC_ACCOUNT_ADDRESS);
        }
        return result;
    }
     ...
     * @return port
     */
    public String getPublicAccountAddressPort() {
        String result = null;
        if (instance != null) {
            result = readParameter(RcsSettingsData.PUBLIC_ACCOUNT_ADDRESS_PORT);
        }
        return result;
    }
     ...
     * @return Address
     */
    public String getPublicAccountAddressType() {
        String result = null;
        if (instance != null) {
            result = readParameter(RcsSettingsData.PUBLIC_ACCOUNT_ADDRESS_TYPE);
        }
        return result;
    }
    ...
    public String getSSOAddress() {
        String result = null;
        if (instance != null) {
            result = readParameter(RcsSettingsData.SSO_ADDRESS);
        }
        return result;
    }
     ...
     * @return port
     */
    public String getSSOAddressPort() {
        String result = null;
        if (instance != null) {
            result = readParameter(RcsSettingsData.SSO_ADDRESS_PORT);
        }
        return result;
    }
     ...
     * @return Address
     */
    public String getSSOAddressType() {
        String result = null;
        if (instance != null) {
            result = readParameter(RcsSettingsData.SSO_ADDRESS_TYPE);
        }
        return result;
    }
    ...
    */
    public String getSpamAddress() {
        String result = null;
        //Till configuration parameter is not present.
        /*if (instance != null) {
            result = readParameter(RcsSettingsData.SPAM_ADDRESS);

        }*/
        String optr = SystemProperties.get("persist.vendor.operator.optr");
        if(optr.equalsIgnoreCase("op01")) {
            result="+86100869999";
        } else if(optr.equalsIgnoreCase("op08")) {
            result="";
        } else {
            result="";
        }
    ...
    public boolean isIR94VideoCallSupported() {
        boolean vilteSupport;
        vilteSupport = SystemProperties.get("persist.vendor.mtk.vilte.enable").equals("1");
        LOGGER.debug("vilteSupport: "+ vilteSupport);

        return mImsCapControlByUser == true ? mVilteSupport : vilteSupport;
    }

    public boolean isIR92VoiceCallSupported() {
        boolean volteSupport;
        volteSupport = RcsUaAdapterManager.getRcsUaAdapter(mSlotId).isImsRegistered();
        LOGGER.debug("voiceSupport: "+ volteSupport);

        return mImsCapControlByUser == true ? mVolteSupport : volteSupport;
    }

    public void setImsCapControlFlag(boolean flag) {
        mImsCapControlByUser = flag;
    }

    public void setIR94VideoCallSupport(boolean supported) {
        mVilteSupport = supported;
    }

    public void setIR92VoiceCallSupport(boolean supported) {
        mVolteSupport = supported;
    }
    ...
    /*
     * RCS can do single registration only whne 1) Platform has rcs_ua support 2) Operators allow
     * single registration
     */
    public boolean isSingleRegistrationFeasible() {
        boolean status = false;

        int rcsVolteSingleRegistrationType = getRcsVolteSingleRegistrationType();
        ...
        } else if (rcsVolteSingleRegistrationType ==
                   RcsSettingsData.RCS_VOLTE_DUAL_REGISTRATION_MODE) {
            status = false;
        }*/
        LOGGER.debug("RcsVolteSingleRegistrationType "+ rcsVolteSingleRegistrationType);
        LOGGER.debug("isSingleRegistrationFeasible "+ status);
        return status;
    }
    ...
    public void setDefaultValuesConfig() {
        if (isOP08TestSim() || SimUtils.isAttSimCard(mSlotId)) {
            writeParameter(RcsSettingsData.AUTO_CONFIG_MODE, new String("" + RcsSettingsData.NO_AUTO_CONFIG));
        }
    }

    public void set489BadEventState(boolean state) {
        if (instance != null) {
            writeParameter(RcsSettingsData.SIP_489_BAD_EVENT_STATE, Boolean.toString(state));
        }
    }

    public boolean is489BadEventState() {
        boolean result = false;
        if (instance != null) {
            result = Boolean.parseBoolean(readParameter(RcsSettingsData.SIP_489_BAD_EVENT_STATE));
        }
        return result;
    }


    public void set489BadEventTime(long time) {
        if (instance != null) {
            writeParameter(RcsSettingsData.SIP_489_BAD_EVENT_TIME, String.valueOf(time));
        }
    }

    public long get489BadEventTime() {
        long result = 0;
        if (instance != null) {
            result = Long.parseLong(readParameter(RcsSettingsData.SIP_489_BAD_EVENT_TIME));
        }
        return result;
    }

    public void set489BadEventExpiredTime(long time) {
        if (instance != null) {
            writeParameter(RcsSettingsData.SIP_489_BAD_EVENT_EXPIRED_TIME, String.valueOf(time));
        }
    }

    public long get489BadEventExpiredTime() {
        long result = 72 * 60 * 60 * 1000;
        if (instance != null) {
            result = Long.parseLong(readParameter(RcsSettingsData.SIP_489_BAD_EVENT_EXPIRED_TIME));
        }
        return result;
    }

    public boolean istVideoSharingDuringCallSupported() {
        boolean result = false;
        if (instance != null) {
            result = Boolean.parseBoolean(readParameter(
                    RcsSettingsData.CAPABILITY_VIDEO_SHARING_CS));
        }
        return result;
    }

    public boolean isVideoSharingOutsideOfVoiceCallSupported() {
        boolean result = false;
        if (instance != null) {
            result = Boolean.parseBoolean(readParameter(
                    RcsSettingsData.CAPABILITY_VIDEO_SHARING_OUTSIDE_VOICE_CALL));
        }
        return result;
    }

    public boolean isGeoLocationPullUsingFileTransferSupported() {
        boolean result = false;
        if (instance != null) {
            result = Boolean.parseBoolean(readParameter(
                    RcsSettingsData.CAPABILITY_GEOLOCATION_PULL_FT));
        }
        return result;
    }
    ...
    public boolean isGeoLocationPullSupported() {
        ...
        if (instance != null) {
            result = Boolean.parseBoolean(readParameter(
                    RcsSettingsData.CAPABILITY_GEOLOCATION_PULL));
        }
        return result;
    }
    ...
    public boolean isRcsIpVoiceCallSupported() {
        ...
        if (instance != null) {
            result = Boolean.parseBoolean(readParameter(
                    RcsSettingsData.CAPABILITY_RCS_VOICE_CALL));
        }
        return result;
    }
    ...
    public boolean isRcsIpVideoCallSupported() {
        ...
        if (instance != null) {
            result = Boolean.parseBoolean(readParameter(
                    RcsSettingsData.CAPABILITY_RCS_VIDEO_CALL));
        }
        return result;
    }
    ...
    public boolean isRcsIpVideoOnlyCallSupported() {
        ...
        if (instance != null) {
            result = Boolean.parseBoolean(readParameter(
                    RcsSettingsData.CAPABILITY_RCS_VIDEO_ONLY_CALL));
        }
        return result;
    }
    ...
    public boolean isChatbotSupported() {
        ...
        if (instance != null) {
            result = Boolean.parseBoolean(readParameter(
                    RcsSettingsData.CHATBOT_SUPPORT));
        }
        return result;
    }
    ...
    public boolean isChatbotSaSupported() {
        ...
        if (instance != null) {
            result = Boolean.parseBoolean(readParameter(
                    RcsSettingsData.CHATBOT_SA_SUPPORT));
        }
        return result;
    }
    ...
    public String getBotVersion() {
        String result = "1.0";
        if (instance != null) {
            result = readParameter(RcsSettingsData.BOT_VERSION);
        }
        return result;
    }
    ...
    public boolean isCallcomposerSupported() {
        ...
        if (instance != null) {
            result = Boolean.parseBoolean(readParameter(
                    RcsSettingsData.CALLCOMPOSER_SUPPORT));
        }
        return result;
    }
}
