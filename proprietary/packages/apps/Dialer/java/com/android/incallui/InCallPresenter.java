...
import com.mediatek.incallui.wfc.RoveOutReceiver;
...
public class InCallPresenter implements CallList.Listener, AudioModeProvider.AudioModeListener {
  ...
  /// M: increase performance of call. @{
  /// Google code:
  ///  private static final String CALL_CONFIGURATION_EXTRA = "call_configuration";
  public static final String CALL_CONFIGURATION_EXTRA = "call_configuration";
  /// @}
  ...
  private final android.telecom.Call.Callback callCallback =
      new android.telecom.Call.Callback() {
        ...
        public void onDetailsChanged(
          ...
          /// M: for ALPS03462750 called after incallpresenter clean up @{
          if (callList == null) {
            LogUtil.w("InCallPresenter.onDetailsChanged", "callList is null");
            return;
          }
          /// @}
          final DialerCall call = callList.getDialerCallFromTelecomCall(telecomCall);
      ...
      new PhoneStateListener() {
        ...
        public void onCallStateChanged(int state, String incomingNumber) {
          if (state == TelephonyManager.CALL_STATE_RINGING) {
            /// M: for ALPS03489349 callback but IncallPresenter teardown @{
            if (context == null) {
              LogUtil.d("InCallPresenter.onCallStateChanged",
                "fail to deal because context is null");
              return;
            }
            /// @}
  ...
  public static synchronized InCallPresenter getInstance() {
    if (inCallPresenter == null) {
      ...
      LogUtil.d("InCallPresenter.getInstance", "One new InCallPresenter:" + inCallPresenter);
      Trace.endSection();
    }
  ...
  public void setUp(
    ...
    /// M: [VoLTE Conference] ALPS01328763, ALPS03440599 @{
    // remove the original one before add new listener
    if (this.statusBarNotifier != null) {
      removeListener(this.statusBarNotifier);
      EnrichedCallComponent.get(context)
          .getEnrichedCallManager()
          .unregisterStateChangedListener(this.statusBarNotifier);
      removeIncomingCallListener(this.statusBarNotifier);
      removeAutoDeclineListener(this.statusBarNotifier);
      this.statusBarNotifier.tearDown();
    }
    /// @}
    /// M: ALPS04502703 fix get camera id is null.InCallCameraManger is created by
    ///CallButtonPresenter.onInCallButtonUiReady.because of mContext is null,so when get camera by
    ///InCallCameraManger is null.when incallpresenter.setup excute, InCallCameraManger is
    ///cleared.so mContext isn't null when create a new InCallCameraManger object.@{
    inCallCameraManager = null;
    ///@}
    ...
    /// M: for ALPS01328763, ALPS03440599 @{
    // remove the original one before add new listener
    if (this.proximitySensor != null) {
      removeListener(this.proximitySensor);
    ...
    if (ImsManager.isWfcEnabledByPlatform(context)) {
      LogUtil.d("isWfcEnabledByPlatform.true", "Register for roveout");
      roveOutReceiver = new RoveOutReceiver(context);
      roveOutReceiver.register();
    }
    /// @}
    ...
    /// M: ALPS03666254 fix listener leak. @{
    if (spamCallListListener != null) {
      this.callList.removeListener(spamCallListListener);
    }
    /// @}
    ...
    /// M: Auto decline upgrade request.@{
    VideoSessionController.getInstance().setUp(this);
    addAutoDeclineListener(this.statusBarNotifier);
    /// @}

    /// M: add for OP08/OP18 plugin. @{
    ExtensionManager.getInCallExt().onInCallPresenterSetUp(statusBarNotifier,
        inCallState, callList);
    /// @}
  ...
  public void tearDown() {
    ...
    ///M: WFC @{
    if (roveOutReceiver != null) {
      roveOutReceiver.unregister();
      roveOutReceiver = null;
    } /// @}

    /// M: add for OP08/OP18 plugin. @{
    ExtensionManager.getInCallExt().onInCallPresenterTearDown();
    /// @}
    ...
    /// M: ALPS03440599 attemptCleanup() must be after removexxx to avoid leak.
    attemptCleanup();
  ...
  public void onIncomingCall(DialerCall call) {
    ...
    /// M: ALPS04393353, refresh the incoming account color in themColorManager, in order to
    /// refresh the backgroud color of answerframgment later. @{
    if (context != null) {
      themeColorManager.onForegroundCallChanged(context, call);
    }
    /// @}
  ...
  public void onUpgradeToVideo(DialerCall call) {
    if (VideoUtils.hasReceivedVideoUpgradeRequest(call.getVideoTech().getSessionModificationState())
        ...
        /// M: For OP12 plugin. @{
        && ExtensionManager.getInCallExt().declineUpgradeDuringIncoming(call)) {
        /// @}
      LogUtil.i(
  ...
  public void onSessionModificationStateChange(DialerCall call) {
    ...
    for (SessionStateListener listener : sessionListeners) {
      listener.onSessionModificationStateChanged(newState);
    }
    /// @}
  }
  ...
  public void onDisconnect(DialerCall call) {
    ...
    if (!callList.hasLiveCall()
        ...
        /// M: ALPS03555059, if the call is conference call or a child of conference, should not
        /// send postcall. If not having any subid, we should not send post call.
        /// And need to give dialer the sub info as well.
        /// M: ALPS04233847, And check disconnect cause to filter  the case about IMS merge. @{
        && call.getParentId() == null
        && !call.isConferenceCall()
        && call.getChildCallIds().size() == 0
        && call.getCallSubId() != SubscriptionManager.INVALID_SUBSCRIPTION_ID
        && call.getDisconnectCause().getCode() == DisconnectCause.LOCAL) {
        /// @}
      PostCall.onCallDisconnected(context,
          call.getNumber(), call.getConnectTimeMillis(), call.getCallSubId());
      LogUtil.d("InCallPresenter.onDisconnect", "Called postCall.onCallDisconnected");
    }
  }
  ...
  public void setBoundAndWaitingForOutgoingCall(boolean isBound, PhoneAccountHandle handle) {
    ...
    /// M: CTA update call's color.
    pendingPhoneAccountHandle = handle;
  }
  ...
  public boolean isShowingInCallUi() {
    ...
    if (manageConferenceActivity != null && manageConferenceActivity.isVisible()
      /// M: ALPS03582505. If incall app going to multi window mode from manage conference activty,
      /// user makes new outgoing call from dialer, we need launch incall UI again.
        && !manageConferenceActivity.isInMultiWindowMode()) {
      LogUtil.d("InCallPresenter.isShowingInCallUi", "conference manage screen");
      /// @}
      return true;
    }
  ...
  public void bringToForeground(boolean showDialpad) {
    ...
    /// M: [VoTLE Conference] ALPS02853151. Dismiss add member screen while return to InCall.@{
    else if (AddMemberScreenController.getInstance().IsAddMemberScreenShown()) {
      LogUtil.i("InCallPresenter.bringToForeground",
          "[bringToForeground] clear add member screen ");
  ...
  private InCallState startOrFinishUi(InCallState newState) {
    ...
    /// M: fix ALPS02522072/ALPS03579237 BT,Headset or auto pre-sanity case answering incoming video
    /// call in notification bar. @{
    showCallUi |= mainUiNotVisible && shouldLaunchMainUiForVideoCall(inCallState, newState);
    /// @}
    ...
    /// M: ALPS03543604, when make new mo calls, pop select account dialog.
    /// Need to refresh UI when state change from WAITING_FOR_ACCOUNT to INCALL. @{
    showCallUi |=
        InCallState.WAITING_FOR_ACCOUNT == inCallState
            && InCallState.INCALL == newState
    ...
    if (activityIsFinishing) {
      /// M: ALPS03535000, InCallActivity may finish itself without any call state change here,
      ...
      if (InCallState.NO_CALLS == newState) {
        LogUtil.i("InCallPresenter.startOrFinishUi",
            "newState = NO_CALLS when finishing activity");
        Trace.endSection();
        return newState;
      }
      if (InCallState.NO_CALLS != inCallState) {
      /// @}
        LogUtil.i(
  ...
  private void attemptCleanup() {
    if (isReadyForTearDown()) {
      ...
      if (statusBarNotifier != null) {
        ...
        /// M: ALPS03440599 statusBarNotifier need tearDown to avoid leak.
  ...
  public InCallCameraManager getInCallCameraManager() {
    synchronized (this) {
      if (inCallCameraManager == null) {
        ...
        /// M: ALPS04502703 Add some log. @{
        LogUtil.d("InCallPresenter.getInCallCameraManager",
            "new : " + inCallCameraManager + ", cotext : " + context);
      }

      LogUtil.d("InCallPresenter.getInCallCameraManager", "return :" + inCallCameraManager);
      /// @}
  ...
  public void showConferenceCallManager(boolean show) {
    LogUtil.v("InCallPresenter.showConferenceCallManager", "show: " + show);
    if (inCallActivity != null) {
  ...
  public boolean isInCallUiLocked() {
    /// M: ALPS04596126 fix can't exit screen when press back key. Because of  the change of google's AMS will
    /// bind call incallactivity.finish() when press back key. so it can't run in main thread and will happen assert
    /// exception in here. In order to fix this problem, we adjust the order of the two statements. @{
  ...
  private final Set<InCallVideoCallListener> inCallVideoCallListener = Collections
          .newSetFromMap(
          new ConcurrentHashMap<InCallVideoCallListener, Boolean>(8, 0.9f, 1));
  /// M: ALPS03449185 Update session state to refresh UI.
  private final Set<SessionStateListener> sessionListeners = Collections.newSetFromMap(
      new ConcurrentHashMap<SessionStateListener, Boolean>(8, 0.9f, 1));
  private Toast toast = null;

  /// M: WFC RoveOutReceiver
  private RoveOutReceiver roveOutReceiver = null;

  /**
   * M: ALPS03449185 interface for video session state changed .
   */
  public interface SessionStateListener {
     ...
     */
    void onSessionModificationStateChanged(int state);
  }

  /**
   * M: Add session listener to refresh UI.
   * @param listener SessionStateListener.
   */
  public void addSessionListener(SessionStateListener listener) {
    Objects.requireNonNull(listener);
    sessionListeners.add(listener);
  }

  /**
   * M:  Remove session listener to refresh UI.
   * @param listener SessionStateListener.
   */
  public void removeSessionListener(SessionStateListener listener) {
    if (listener != null) {
  ...
  public void showMessage(int resId) {
    /// M: ALPS03691369 Call end and context maybe null. @{
    if (context == null) {
        LogUtil.d("InCallPresenter.showMessage", "mContext is null");
        return;
    }
    /// @}
    ...
    if (toast == null) {
      ...
      // revert to google default location
  ...
  /**
   * M: add a method here to show toast with long duration.
   */
  public void showMessageWithLongDuration(int resId) {
    if (context == null) {
        LogUtil.d("InCallPresenter.showMessage", "context is null");
        return;
    }
  ...
  private boolean shouldLaunchMainUiForVideoCall(InCallState oldState, InCallState newState) {
    DialerCall call = getCallList().getActiveCall();
  ...
  public interface InCallVideoCallListener {
      ...
      void onPausePreviewRequest(boolean pause);
  }

  /// this listener is do the operation for video call @{
  public void addInCallVideoCallListener(InCallVideoCallListener listener) {
    if (listener != null) {
      inCallVideoCallListener.add(listener);
    }
  }

  public void removeInCallVideoCallListener(InCallVideoCallListener listener) {
      if (listener != null) {
          inCallVideoCallListener.remove(listener);
      }
  }
  ...
  public void notifyHideLocalVideoChanged(boolean hide) {
    for (InCallVideoCallListener listener : inCallVideoCallListener) {
      listener.onHidePreviewRequest(hide);
    }
  }

  /**
   * M: [Voice Record] Stop voice record function.
   */
  public void stopVoiceRecording() {
    TelecomAdapter.getInstance().toggleVoiceRecording(false);
  }

  /**
   * M: Called by the {@link CallButtonPresenter} to inform video in pause state or not.
   *
   * @param pause {@code True} if pause the preview video.
   */
  public void notifyPauseLocalVideoChanged(boolean pause) {
    for (InCallVideoCallListener listener : inCallVideoCallListener) {
      listener.onPausePreviewRequest(pause);
    }
  }

  /** Interface implemented by classes that need to know auto decline time and update UI. */
  public interface AutoDeclineTimerListener {
    void onTimeUpdate(CallList callList);
  }

  private final Set<AutoDeclineTimerListener> autoDeclineListeners = Collections
      .newSetFromMap(new ConcurrentHashMap<AutoDeclineTimerListener, Boolean>(8, 0.9f, 1));

  public void addAutoDeclineListener(AutoDeclineTimerListener listener) {
    Objects.requireNonNull(listener);
    autoDeclineListeners.add(listener);
  }

  public void removeAutoDeclineListener(AutoDeclineTimerListener listener) {
    if (listener != null) {
      autoDeclineListeners.remove(listener);
    }
  }
  ...
  public void onAutoDeclineCountdownChanged() {
    LogUtil.d("InCallPresenter.onAutoDeclineCountdownChanged", null);
    for (AutoDeclineTimerListener listener : autoDeclineListeners) {
      listener.onTimeUpdate(this.callList);
    }
  }

  public void declineUpgradeRequest(Context context) {
    LogUtil.d("InCallPresenter.declineUpgradeRequest", null);
    // Bail if we have been shut down and the call list is null.
    if (this.callList == null) {
      StatusBarNotifier.clearAllCallNotifications();
      LogUtil.e("InCallPresenter.declineUpgradeRequest", "callList is empty so returning");
      return;
    }

    DialerCall call = this.callList.getVideoUpgradeRequestCall();
  ...
  /// M:send cancel upgrade request out when the timer is timeout @{
  public void cancelUpgradeRequest(Context context) {
    LogUtil.d("InCallPresenter.cancelUpgradeRequest", "enter");
    // Bail if we have been shut down and the call list is null.
    if (callList == null) {
      StatusBarNotifier.clearAllCallNotifications();
      LogUtil.e("InCallPresenter.cancelUpgradeRequest", "callList is empty so returning");
      return;
    }

    DialerCall call = callList.getSendingVideoUpgradeRequestCall();
    VideoSessionController.getInstance().stopTiming();
  ...
  public long getAutoDeclineCountdown() {
    LogUtil.d("InCallPresenter.getAutoDeclineCountdown", null);
    return VideoSessionController.getInstance().getAutoDeclineCountdownSeconds();
  }

  public CharSequence appendCountdown(CharSequence originalText) {
    long countdown = getAutoDeclineCountdown();
    if (countdown < 0) {
        return originalText;
    }
   ...
   */
  public int getPrimaryColorFromCall(DialerCall call) {
  ...
  private MaterialPalette getColorsFromCall(DialerCall call) {
    if (call == null) {
      return getColorsFromPhoneAccountHandle(pendingPhoneAccountHandle);
    } else {
      /// M: [Performance] Increase call performace. @{
      int highlightColor = call.getAccountColor();
      if (highlightColor != PhoneAccount.NO_HIGHLIGHT_COLOR) {
        return new InCallUIMaterialColorMapUtils(this.context)
  ...
  private MaterialPalette getColorsFromPhoneAccountHandle(
      PhoneAccountHandle phoneAccountHandle) {
    /// M: for ALPS01978652. @{
    if (context == null) {
      Log.e(this,
          "getColorsFromPhoneAccountHandle()...context is null, do nothing !");
      return null;
    }
    /// @}
    ...
    if (phoneAccountHandle != null) {
      ...
      if (tm != null) {
        ...
        // For single-sim devices, there will be no selected highlight color, so
        // the phone
  ...
  public TelecomManager getTelecomManager() {
    if (telecomManager == null) {
      telecomManager = (TelecomManager) this.context
          .getSystemService(Context.TELECOM_SERVICE);
    }
    return telecomManager;
  }

  private DialerCall getPrimaryCall(CallList callList) {
  ...
  /**
   * Update theme colors according to primary call.
   */
  public void setThemeColors() {
    if (inCallActivity == null || context == null) {
      LogUtil.d("InCallPresenter.setThemeColors", "null");
      return;
    }

    DialerCall call = getPrimaryCall(CallList.getInstance());
    if (call != null) {
  ...
  /// M: ALPS04641699, set recorded conference activity as null.
  /// Need to check if the activity which will be set as null is current activity. @{
  public void setManageConferenceActivityAsNull(
      @Nullable ManageConferenceActivity oldManageConferenceActivity) {
    if (this.manageConferenceActivity != null
        && oldManageConferenceActivity == this.manageConferenceActivity) {
      LogUtil.d("InCallPresenter.setManageConferenceActivityAsNull", null);
