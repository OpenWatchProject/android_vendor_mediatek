...
public final class Utils {
    ...
    /// M: CTA requirement - permission control @{
    public static final boolean CTA_SUPPORT = CtaManagerFactory.getInstance().makeCtaManager()
            .isCtaSupported();
    ...
    static {
        ...
        if (CTA_SUPPORT) {
            ArrayMap<String, String> ctaPlatformPermissions = CtaManagerFactory.getInstance()
                    .makeCtaManager().getCtaPlatformPerms();
            int numCtaPlatformPermissions = ctaPlatformPermissions.size();
            for (int i = 0; i < numCtaPlatformPermissions; i++) {
                PLATFORM_PERMISSIONS.put(ctaPlatformPermissions.keyAt(i),
                        ctaPlatformPermissions.valueAt(i));
            }
        }
    ...
    public static boolean shouldShowPermission(Context context, AppPermissionGroup group) {
        ...
        /// M: CTA requirement - permission control @{
        final boolean isPlatformPermission = CtaManagerFactory.getInstance().makeCtaManager()
    ...
    public static boolean isPermissionIndividuallyControlled(Context context, String permission) {
        ...
        if (!CTA_SUPPORT) {
            ...
            if (permInfo.getProtection() != PermissionInfo.PROTECTION_DANGEROUS) {
                return false;
            }
            return isDangerousPermissionGroup(getGroupOfPermission(permInfo));
        }
        ///@}
    }
    ...
    private static boolean isDangerousPermissionGroup(String group) {
        ...
        return Manifest.permission_group.SMS.equals(group)
                ...
                || CtaManagerFactory.getInstance().makeCtaManager().isCtaAddedPermGroup(group);
    }
    ///@}
}
