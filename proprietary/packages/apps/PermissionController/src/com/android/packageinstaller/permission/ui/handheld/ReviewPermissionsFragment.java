...
public final class ReviewPermissionsFragment extends PreferenceFragmentCompat
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (manager.isCtaSupported()) {
            UtilsExt.initPermStateCache(getContext(), mAppPermissions, null);
        }
        ///@}
    }
    ...
    public void onClick(View view) {
        ...
        if (view == mContinueButton) {
            ...
            intent.putExtra("more_permission",true);//M
    ...
    private void bindUi() {
        ...
        ///M: ALPS03227840.
        PackageInfo info = mAppPermissions.getPackageInfo();
        CharSequence label = mAppPermissions.getAppLabel();
        ...
        /// M: CTA requirement - permission control @{
        // add usage description for the more buttion
        CtaManager manager = CtaManagerFactory.getInstance().makeCtaManager();
        if (manager.isCtaSupported()) {
            String moreButton = getString(R.string.all_permissions);
            SpannableString descriptionOfMoreButton = new SpannableString(
                    getString(R.string.description_of_more_button,moreButton));
            //color the button string
            final int moreButtonStart =
                    descriptionOfMoreButton.toString().indexOf(moreButton, 0);
            final int moreButtonLength = moreButton.length();
            TypedValue typedValue = new TypedValue();
            getActivity().getTheme().resolveAttribute(android.R.attr.colorAccent,
                    typedValue, true);
            final int color = getContext().getColor(typedValue.resourceId);
            descriptionOfMoreButton.setSpan(new ForegroundColorSpan(color),
                    moreButtonStart,
                    moreButtonStart + moreButtonLength, 0);

            TextView descriptionOfMoreView =
                    (TextView) getActivity().findViewById(
                            R.id.detailed_description_of_more_button);
            descriptionOfMoreView.setText(descriptionOfMoreButton);
            descriptionOfMoreView.setVisibility(View.VISIBLE);
        }
        ///@}
        ...
        /// M: CTA requirement - permission control @{
        if (manager.isCtaSupported()) {
            mMoreInfoButton.setText(R.string.all_permissions);
    ...
    private void loadPreferences() {
        ...
        CtaManager manager = CtaManagerFactory.getInstance().makeCtaManager();
        for (AppPermissionGroup group : mAppPermissions.getPermissionGroups()) {
            if (!Utils.shouldShowPermission(getContext(), group) || !manager
                    .isPlatformPermissionGroup(group.getDeclaringPackage(), group.getName())) {
                continue;
            }
            ///@}
    ...
    /// M: Pre-grant permissions in review UI @{
    private void confirmPermissionsReviewforCTA() {
        final List<PreferenceGroup> preferenceGroups = new ArrayList<>();
        if (mNewPermissionsCategory != null) {
            preferenceGroups.add(mNewPermissionsCategory);
            preferenceGroups.add(mCurrentPermissionsCategory);
            ...
            for (int prefNum = 0; prefNum < preferenceCount; prefNum++) {
                ...
                if (preference instanceof PermissionReviewPreference) {
                    ...
                    for (int j = 0; j < permissionCount; j++) {
                        ...
                        boolean grant = UtilsExt.isPermGrantedByCache(permission);
                        if (grant) {
                    ...
                    if (backgroundGroup != null) {
                        for (Permission permission : backgroundGroup.getPermissions()) {
                            ...
                            boolean grant = UtilsExt.isPermGrantedByCache(permission);
                            if (grant) {
                                backgroundGroup.grantRuntimePermissions(false, filterPermissions);
                            } else {
