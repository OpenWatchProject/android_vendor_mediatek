...
public class CallAudioRouteStateMachine extends StateMachine {
    ...
    /**
     * M: ALPS04922814.
     * speaker is turn on by 3rd part apk, telecom should update
     *    the audio route
     */
    private BroadcastReceiver mSpeakerPhoneChangeReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals(AudioManager.ACTION_SPEAKERPHONE_STATE_CHANGED)) {
                ...
                if (speakerOn) {
                    Log.i(this, "Turn on speaker externally");
                    sendMessageWithSessionInfo(USER_SWITCH_SPEAKER);
                } else {
                    Log.i(this, "Turn off speaker externally");
                    sendMessageWithSessionInfo(
                            USER_SWITCH_BASELINE_ROUTE,
                            NO_INCLUDE_BLUETOOTH_IN_BASELINE);
                }
            }
        }
    };
    ...
    public void initialize(CallAudioState initState) {
        ...
        /// M: ALPS04922814 listen to speaker mode changes
