...
public class TelecomServiceImpl {
    ...
    private final ITelecomService.Stub mBinderImpl = new ITelecomService.Stub() {
        ...
        public List<PhoneAccountHandle> getCallCapablePhoneAccounts(
            ...
            try {
                // M: Fix log too much issue.
                // TODO: Check why this API is called so many times.
        ...
        public boolean handlePinMmi(String dialString, String callingPackage) {
            try {
                ...
                /// M:  ALPS03688394 @{
                /// This is a google default design, when handle pinMMI, it would acquire the lock
                /// and then binder call to phone process. If the phone process registerPhoneAccount
                /// at the same time, ANR would happen due to dead lock.
                /// Solution:
                /// The calling to phone process doesn't need the lock, move the code out of the
                /// lock.
                synchronized (mLock) {
                    enforcePermissionOrPrivilegedDialer(MODIFY_PHONE_STATE, callingPackage);
                }

                // Switch identity so that TelephonyManager checks Telecom's
                // permissions instead.
                long token = Binder.clearCallingIdentity();
        ...
        public void placeCall(Uri handle, Bundle extras, String callingPackage) {
            try {
                ...
                if (MtkUtil.isConferenceInvitation(extras)) {
                    // This check should be made before Binder calling identity cleard.
                    if (!MtkUtil.canConference(callingPackage, "placeCall")) {
                        return;
                    }
                }
                /// @}
            ...
            try {
                ...
                synchronized (mLock) {
                    ...
                    try {
                        ...
                        /// M: plugin to control video call for low battery.
                        if (!ExtensionManager.getCallMgrExt()
                                .shouldPreventVideoCallIfLowBattery(mContext, intent)) {
