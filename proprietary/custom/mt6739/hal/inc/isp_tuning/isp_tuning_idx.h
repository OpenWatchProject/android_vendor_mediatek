...
private:
    ...
    INDEX_T const* m_pSuperNight_Cap            [eNUM_OF_ISO_IDX];
...
private:    ////    Normal
    ...
    inline INDEX_T const* get_Multi_Pass_ANR1(MUINT32 const sensor __attribute__((unused)), MUINT32 const scene __attribute__((unused)), MUINT32 const iso) const
    {
        return  isInvalid(0, 0, iso) ? NULL : m_pMulti_Pass_ANR1[iso];
    }
    inline INDEX_T const* get_Multi_Pass_ANR2(MUINT32 const sensor __attribute__((unused)), MUINT32 const scene __attribute__((unused)), MUINT32 const iso) const
    {
        return  isInvalid(0, 0, iso) ? NULL : m_pMulti_Pass_ANR2[iso];
    }
    inline INDEX_T const* get_Super_Night_Cap(MUINT32 const sensor __attribute__((unused)), MUINT32 const scene __attribute__((unused)), MUINT32 const iso) const
    {
        return  isInvalid(0, 0, iso) ? NULL : m_pSuperNight_Cap[iso];
    }
...
#define IDX_MODE_Super_Night_Cap(iso)\
    template <> INDEX_T const IdxSet<EIspProfile_3rd_Party_Processed_Raw_Reprocess, 0, 0, iso>::idx =
