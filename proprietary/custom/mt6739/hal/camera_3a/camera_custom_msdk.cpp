...
MINT64 compareSensorIdAndModuleId(MUINT32 r_u4SensorId, MUINT32 r_u8ModuleId)
{
    ...
    for (i=0;i<MAX_NUM_OF_SUPPORT_SENSOR;i++)
    {
        if (pstSensorInitFunc[i].SensorId == 0){
            CAM_MSDK_LOG("last item in sensorList found.  i(%u)", i);
            break;
        }
...
MINT32 GetCameraBpciDefaultTbl(MUINT32 SensorId, PCAMERA_BPCI_STRUCT pCameraBPCIDefault)
{
    ...
    if(pCameraBPCIDefault!=NULL && pCameraBPCIDefault->PDC_FULL_TBL.bpci_xsize==0)
    {
        CAM_MSDK_ERR("the BPCI table for sensor id %d is not porting under per-sensor camera_indexmgr_api.cpp", SensorId);
        aee_system_exception(LOG_TAG, NULL, DB_OPT_DEFAULT, "No BPCI table for this sensor\nCRDISPATCH_KEY:MSDK_BPCI");
        exit(0);
    }
