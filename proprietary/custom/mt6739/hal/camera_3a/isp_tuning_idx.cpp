...
#define LINK_SUPER_NIGHT_CAP(sensor, scene, iso)\
m_pSuperNight_Cap[iso] = &IdxSet<EIspProfile_3rd_Party_Processed_Raw_Reprocess, sensor, scene, iso>::idx
...
#define LINK_SUPER_NIGHT_CAP(sensor, scene, iso)\
m_pSuperNight_Cap[iso] = &IdxSet<EIspProfile_3rd_Party_Processed_Raw_Reprocess, sensor, MTK_CONTROL_SCENE_MODE_UNSUPPORTED, iso>::idx
    ...
    LINK_ONE_SENSOR_ONE_SCENE_ISOs(LINK_SUPER_NIGHT_CAP, 0, 0);
...
get(MUINT32 ispProfile, MUINT32 sensor/*=0*/, MUINT32 const /*scene_=0*/, MUINT32 const iso/*=0*/) const
{
    ...
    case EIspProfile_3rd_Party_Processed_Raw_Reprocess:
        return get_Super_Night_Cap(sensor, scene, iso);
    default:
        break;
    }
    return  NULL;
