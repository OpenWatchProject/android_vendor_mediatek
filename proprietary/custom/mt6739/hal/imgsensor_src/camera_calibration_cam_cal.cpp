...
UINT32 DoCamCal2AGain(INT32 CamcamFID, UINT32 start_addr, UINT32 BlockSize, UINT32* pGetSensorCalData)
{
    ...
    DoCamCal_Dump_All(CamcamFID, pGetSensorCalData);
...
////////////////////////////////////////////////////////////////////////////////
            if((0x2 & AWBAFConfig) || (0x4 & AWBAFConfig)){
                if ((AFInf == 0xffff && AFMacro == 0xffff) || (AFInf == 0 && AFMacro == 0)) {
                    /*
                     * EEPROM content is abnormal, the AF flag enabled, but no AF content.
                     * Clear S2aBitEn[1] and S2aAfBitflagEn
                     */
                    CAM_CAL_LOG("The EEPROM AF content is abnormal, AF flag enabled, but no AF content\n");
                    AWBAFConfig &= (~0xE); // Clear flag
                    pCamCalData->Single2A.S2aBitEn = (0x03 & AWBAFConfig);
                    pCamCalData->Single2A.S2aAfBitflagEn = (0x0C & AWBAFConfig);// //Bit: step 0(inf.), 1(marco), 2, 3, 4,5,6,7
                }
            }
