...
UINT32 DoCamCalLayoutCheck(void)
{
    ...
    if ((gIsInitedCamCal==CAM_CAL_LAYOUT_RTN_QUEUE) && (CAM_CALInit() != CAM_CAL_NONE)
            && (CAM_CALDeviceName(&cBuf[0], sizeof(cBuf)) == CAM_CAL_SUCCESS))
    {
...
//    CAM_CAL_LOG_IF(dumpEnable,"CAM_CALGetCalData(0x%8x)----\n",(unsigned int)pGetSensorCalData);
    ...
    if(DoCamCalLayoutCheck()==CAM_CAL_ERR_NO_ERR)
    {
        ...
        if ((CAM_CALInit() != CAM_CAL_NONE) && (CAM_CALDeviceName(&cBuf[0], sizeof(cBuf)) == CAM_CAL_SUCCESS))
        {
