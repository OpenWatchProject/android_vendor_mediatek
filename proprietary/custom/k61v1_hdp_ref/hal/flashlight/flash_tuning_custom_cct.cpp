...
int cust_fillDefaultStrobeNVRam_main (void* data)
{
    ...
    static short engTab[]=
    { 254, 812,1387,1902,2406,2893,3361,3816,4216,4629,5025,5500,5885,6248,6606,7055,7407,7736,8070,8471,8772,9076,9455,9717,9999};
    //version
    ...
    for(i=0; i<8; i++)
    {
        p->tuningPara[i].yTarget = 178;
        p->tuningPara[i].fgWIncreaseLevelbySize = 10;
        p->tuningPara[i].fgWIncreaseLevelbyRef = 0;//5
        p->tuningPara[i].ambientRefAccuracyRatio = 5;//255
        p->tuningPara[i].flashRefAccuracyRatio = 1;//255
        p->tuningPara[i].backlightAccuracyRatio = 18;
        p->tuningPara[i].backlightUnderY = 40;
        p->tuningPara[i].backlightWeakRefRatio = 32;
        p->tuningPara[i].safetyExp =33322;//66644
        ...
        p->tuningPara[i].fgEnhanceMaxOverExpRatio = 6;//2
        p->tuningPara[i].isFollowCapPline = 1;
        p->tuningPara[i].histStretchMaxFgYTarget = 266;//285;//300;
        p->tuningPara[i].histStretchBrightestYTarget = 328;//404;//480;
        p->tuningPara[i].fgSizeShiftRatio = 0;
        p->tuningPara[i].backlitPreflashTriggerLV = 90;
        p->tuningPara[i].backlitMinYTarget = 100;//90
    }
    ...
    //index mode
    //torch
    p->engLevel.torchDuty = 4;
    //af
    p->engLevel.afDuty = 4;
    //pf, mf, normal
    p->engLevel.pfDuty = 4;
    p->engLevel.mfDutyMax = 24;
    p->engLevel.mfDutyMin = 0;
    //low bat
    p->engLevel.IChangeByVBatEn=1;
    p->engLevel.vBatL = 3760;//mv
    p->engLevel.pfDutyL = 4;
    p->engLevel.mfDutyMaxL = 15;
    p->engLevel.mfDutyMinL = 0;
    //burst setting
    p->engLevel.IChangeByBurstEn=1;
    p->engLevel.pfDutyB = 4;
