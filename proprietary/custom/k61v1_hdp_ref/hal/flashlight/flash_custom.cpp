...
void cust_getGoldenAvgY(int sensorDev, short* avgY)
{
    if(sensorDev==DUAL_CAMERA_MAIN_SENSOR)
    {
        if(g_mainId==1)
        {
            ALOGD("cust_getGoldenAvgY devid main id1");
            cust_getGoldenAvgY_main(avgY);
        }
        else
        {
            ALOGD("cust_getGoldenAvgY devid main id2");
            cust_getGoldenAvgY_main2(avgY);
        }
    }
    else
    {
        if(g_subId==1)
        {
            ALOGD("cust_getGoldenAvgY devid sub id1");
            cust_getGoldenAvgY_sub(avgY);
        }
        else
        {
            ALOGD("cust_getGoldenAvgY devid sub id2");
            cust_getGoldenAvgY_sub2(avgY);
        }
    }
