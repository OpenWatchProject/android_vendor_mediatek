...
public:     ////    Attributes
    ...
    virtual INDEX_T const*  getDefaultIndex(EIspProfile_T const eIspProfile,
                                            ...
                                            EIndex_ZOOM_T eIdx_Zoom) const = 0;
...
public:     ////    Operations.
    ...
    virtual
    MBOOL
    is_to_invoke_remap_ISP_ISO_with_HDR_Ratio(
        RAWIspCamInfo const& rCamInfo
    );

    virtual
    MUINT32
    remap_ISP_ISO_with_HDR_Ratio(
    HDR_EV_INFO_T const& hdrInfo, MUINT32 const realISO, MBOOL enable, ISP_NVRAM_ISO_INTERVAL_STRUCT const& rISO_ENV
    );
