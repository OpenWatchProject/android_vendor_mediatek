...
MBOOL PDBufMgr::SetPDBlockInfo( SET_PD_BLOCK_INFO_T &iPDBlockInfo)
{
    ...
    AAA_LOGD( "[PD] set block information    %d %d %d %d %d %d %d",
...
MVOID PDBufMgr::getPdMapping(PDPIXEL_T* pPdPix, int pdNum) {
    ...
    subBlkY = 0;
    for (int i=0; i<pdNum; i++) {
        if (pPdOut[i].bx > subBlkX)
            subBlkX = pPdOut[i].bx;
        if (pPdOut[i].by > subBlkY)
            subBlkY = pPdOut[i].by;
        ...
        for (unsigned int y=0; y<pd_y_num; y+=2) {
            ...
            if (rowNum == 1) {
                ...
                for (unsigned int x=0; x<pd_x_num; x+=1) {
                    *(pR++) = *(rowRL++) >> iShift; *(pL++) = *(rowRL++) >> iShift;
                }
            }
            else { //rowNum == 2
                unsigned short *rowR = row[pPdMap[0].row], *rowL = row[pPdMap[1].row];
                for (unsigned int x=0; x<pd_x_num; x+=1) {
                    *(pR++) = *(rowR++) >> iShift; *(pL++) = *(rowL++) >> iShift;
                }
            }
        }
    }
        ...
        unsigned int mapIdx = ri*(subBlkX<<1);
            ...
            if (subBlkX == 4) { //optimize path
                ...
                for (unsigned int x=0; x<pd_x_num; x+=subBlkX) {
                    *(pR++) = *(row[rIdx0]++) >> iShift; *(pL++) = *(row[rIdx1]++) >> iShift;
                    *(pR++) = *(row[rIdx2]++) >> iShift; *(pL++) = *(row[rIdx3]++) >> iShift;
                    *(pR++) = *(row[rIdx4]++) >> iShift; *(pL++) = *(row[rIdx5]++) >> iShift;
                    *(pR++) = *(row[rIdx6]++) >> iShift; *(pL++) = *(row[rIdx7]++) >> iShift;
                }
            }
            else { //general path
                for (unsigned int x=0; x<pd_x_num; x+=subBlkX) {
