...
#include "SonyIMX386PdafTransCoord.h"
}
...
#define LOG_TAG "SonyIMX386PdafLibraryWrapper"
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
SonyIMX386PdafLibraryWrapper::SonyIMX386PdafLibraryWrapper()
{
    m_Libhandle       = NULL;
    ...
    m_Libhandle = dlopen( "libSonyIMX386PdafLibrary.so", RTLD_NOW);
    error = dlerror();
    if( (m_Libhandle!=NULL)&&(error==NULL))
    {
        ...
        if( error==NULL)
        {
            ...
            if( error==NULL)
            {
                ...
                AAA_LOGD("dlopen libSonyIMX386PdafLibrary.so done. ver=%lu.%lu", ver.MajorVersion, ver.MinorVersion);
    ...
    {
        AAA_LOGE("dlopen libSonyIMX386PdafLibrary.so fail");
    }
    ...
    if( m_bLdDn)
    {
         ...
         */
        m_pDefocusArgIn       = new SonyPdLibInputData_t;
        m_pDefocusOKNGThrLine = new DefocusOKNGThrLine_t[D_IMX386_DEFOCUS_OKNG_THR_LINE_BLK_NUM_TOTAL];
        m_pTransRectArgIn     = new SonyPdLibSensorCoordSetting_t;

        memset( m_pDefocusArgIn,               0, sizeof(SonyPdLibInputData_t));
        memset( m_pDefocusOKNGThrLine,         0, sizeof(DefocusOKNGThrLine_t)*D_IMX386_DEFOCUS_OKNG_THR_LINE_BLK_NUM_TOTAL);
        memset( m_pTransRectArgIn,             0, sizeof(SonyPdLibSensorCoordSetting_t));
        memset( m_aData_Slope,                 0, sizeof(signed long)*D_IMX386_SLOPE_OFFSET_BLK_NUM_TOTAL);
        memset( m_aData_Offset,                0, sizeof(signed long)*D_IMX386_SLOPE_OFFSET_BLK_NUM_TOTAL);
        memset( m_aXAddress_Knot_Slope_Offset, 0, sizeof(MUINT16)*D_IMX386_SLOPE_OFFSET_BLK_NUM_W);
        memset( m_aYAddress_Knot_Slope_Offset, 0, sizeof(MUINT16)*D_IMX386_SLOPE_OFFSET_BLK_NUM_H);

        DefocusOKNGThrLine_t *pDefocusOKNGThrLine = (DefocusOKNGThrLine_t *)m_pDefocusOKNGThrLine;
        for( unsigned int i=0; i<D_IMX386_DEFOCUS_OKNG_THR_LINE_BLK_NUM_TOTAL; i++)
        {
            pDefocusOKNGThrLine[i].PointNum = D_IMX386_DEFOCUS_OKNG_THR_LINE_ONE_BLK_POINT_NUM;
            pDefocusOKNGThrLine[i].p_AnalogGain = new unsigned long [D_IMX386_DEFOCUS_OKNG_THR_LINE_ONE_BLK_POINT_NUM];
            pDefocusOKNGThrLine[i].p_Confidence = new unsigned long [D_IMX386_DEFOCUS_OKNG_THR_LINE_ONE_BLK_POINT_NUM];
        }
...
SonyIMX386PdafLibraryWrapper::~SonyIMX386PdafLibraryWrapper()
{
    if( m_bLdDn)
    {
        ...
        if( error==NULL)
        {
            AAA_LOGD("dlclose libSonyIMX386PdafLibrary.so done");
        }

        if( m_pDefocusArgIn)
            delete ((SonyPdLibInputData_t *)m_pDefocusArgIn);
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MBOOL SonyIMX386PdafLibraryWrapper::DbgDumpSetting()
{
    if( m_bLdDn)
    {
        m_u8DbgEn = property_get_int32("debug.af_mgr.enable", 0);

        if( m_bDmpSetting)
        {
            SonyPdLibInputData_t          *in             = (SonyPdLibInputData_t *)m_pDefocusArgIn;

            SonyPdLibSensorCoordSetting_t *pSensorSetting = (SonyPdLibSensorCoordSetting_t *)m_pTransRectArgIn;

            AAA_LOGD(" ******** Dump Setting ********");
            AAA_LOGD("D_IMX386_DCC_IN_EEPROM_BLK_NUM_W                  : %d", D_IMX386_DCC_IN_EEPROM_BLK_NUM_W);
            AAA_LOGD("D_IMX386_DCC_IN_EEPROM_BLK_NUM_H                  : %d", D_IMX386_DCC_IN_EEPROM_BLK_NUM_H);
            AAA_LOGD("D_IMX386_DCC_IN_EEPROM_BLK_NUM_TOTAL              : %d", D_IMX386_DCC_IN_EEPROM_BLK_NUM_TOTAL);
            AAA_LOGD("D_IMX386_PD_AREA_OFFSET_X                         : %d", D_IMX386_PD_AREA_OFFSET_X);
            AAA_LOGD("D_IMX386_PD_AREA_OFFSET_Y                         : %d", D_IMX386_PD_AREA_OFFSET_Y);
            AAA_LOGD("D_IMX386_PD_AREA_SIZE_X                           : %d", D_IMX386_PD_AREA_SIZE_X);
            AAA_LOGD("D_IMX386_PD_AREA_SIZE_Y                           : %d", D_IMX386_PD_AREA_SIZE_Y);
            AAA_LOGD("D_IMX386_SLOPE_OFFSET_BLK_NUM_W                   : %d", D_IMX386_SLOPE_OFFSET_BLK_NUM_W);
            AAA_LOGD("D_IMX386_SLOPE_OFFSET_BLK_NUM_H                   : %d", D_IMX386_SLOPE_OFFSET_BLK_NUM_H);
            AAA_LOGD("D_IMX386_SLOPE_OFFSET_BLK_NUM_TOTAL               : %d", D_IMX386_SLOPE_OFFSET_BLK_NUM_TOTAL);
            AAA_LOGD("D_IMX386_DEFOCUS_OKNG_THR_LINE_BLK_NUM_W          : %d", D_IMX386_DEFOCUS_OKNG_THR_LINE_BLK_NUM_W);
            AAA_LOGD("D_IMX386_DEFOCUS_OKNG_THR_LINE_BLK_NUM_H          : %d", D_IMX386_DEFOCUS_OKNG_THR_LINE_BLK_NUM_H);
            AAA_LOGD("D_IMX386_DEFOCUS_OKNG_THR_LINE_BLK_NUM_TOTAL      : %d", D_IMX386_DEFOCUS_OKNG_THR_LINE_BLK_NUM_TOTAL);
            AAA_LOGD("D_IMX386_DEFOCUS_OKNG_THR_LINE_ONE_BLK_POINT_NUM  : %d", D_IMX386_DEFOCUS_OKNG_THR_LINE_ONE_BLK_POINT_NUM);
            AAA_LOGD("D_IMX386_AF_MOTOR_DRV_CONT_CODE_RESOLUTION        : %d", D_IMX386_AF_MOTOR_DRV_CONT_CODE_RESOLUTION);
            AAA_LOGD("D_IMX386_EXTRA_SHIFT_BIT                          : %d", D_IMX386_EXTRA_SHIFT_BIT);
            AAA_LOGD("\n");
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MRESULT SonyIMX386PdafLibraryWrapper::GetVersionOfPdafLibrary( SPDLibVersion_t &tOutSWVer)
{
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MBOOL SonyIMX386PdafLibraryWrapper::Init( SPDProfile_t &iPdProfile)
{
    if( m_bLdDn)
    {
        SonyPdLibInputData_t *in = (SonyPdLibInputData_t *)m_pDefocusArgIn;
        ...
        m_u8FiexibleWindowEn = 0;
        m_u8WindowModeSelect = 0;
        memset( in, 0, sizeof(SonyPdLibInputData_t));
         ...
         */
        for( MUINT32 i=0; i < D_IMX386_SLOPE_OFFSET_BLK_NUM_W; i++)
        {
            m_aXAddress_Knot_Slope_Offset[i] = (unsigned short)(D_IMX386_PD_AREA_OFFSET_X + (D_IMX386_PD_AREA_SIZE_X * i) + (D_IMX386_PD_AREA_SIZE_X / 2));
        }

        for( MUINT32 i=0; i < D_IMX386_SLOPE_OFFSET_BLK_NUM_H; i++)
        {
            m_aYAddress_Knot_Slope_Offset[i] = (unsigned short)(D_IMX386_PD_AREA_OFFSET_Y + (D_IMX386_PD_AREA_SIZE_Y * i) + (D_IMX386_PD_AREA_SIZE_Y / 2));
        }

        // Set the number of knots
        in->XKnotNumSlopeOffset = D_IMX386_SLOPE_OFFSET_BLK_NUM_W;
        in->YKnotNumSlopeOffset = D_IMX386_SLOPE_OFFSET_BLK_NUM_H;
        ...
        case( SENS_MODE1_HDR_ALL_PIXEL):
            in->AdjCoeffSlope = D_SONY_PD_LIB_SLOPE_ADJ_COEFF_SENS_MODE1;
            break;
        case( SENS_MODE2_NORMAL_V2_BINNING):
            in->AdjCoeffSlope = D_SONY_PD_LIB_SLOPE_ADJ_COEFF_SENS_MODE2;
            break;
        case( SENS_MODE3_HDR_V2_BINNING):
            in->AdjCoeffSlope = D_SONY_PD_LIB_SLOPE_ADJ_COEFF_SENS_MODE3;
            break;
        case( SENS_MODE4_NORMAL_V4_BINNING):
            in->AdjCoeffSlope = D_SONY_PD_LIB_SLOPE_ADJ_COEFF_SENS_MODE4;
            break;
        default: // In the case of default, it will set a 0 mode.
            in->AdjCoeffSlope = D_SONY_PD_LIB_SLOPE_ADJ_COEFF_SENS_MODE0;
            break;
        }
         ...
         */
        // Set the number of knots
        in->XKnotNumDefocusOKNG = D_IMX386_DEFOCUS_OKNG_THR_LINE_BLK_NUM_W;
        in->YKnotNumDefocusOKNG = D_IMX386_DEFOCUS_OKNG_THR_LINE_BLK_NUM_H;
        ...
        MUINT16 knot_size_okng_x = (D_IMX386_ALL_PIXEL_SIZE_W-1) / (in->XKnotNumDefocusOKNG-1);
        MUINT16 knot_size_okng_y = (D_IMX386_ALL_PIXEL_SIZE_H-1) / (in->YKnotNumDefocusOKNG-1);
         ...
         * Please input "D_SONY_PD_LIB_DENSITY_SENS_MODE0" as a fixed value regardless of image sensor mode.
         */
        switch( m_u8SensMode)
        {
        ...
        case( SENS_MODE4_NORMAL_V4_BINNING) :
        default :
            in->DensityOfPhasePix = D_SONY_PD_LIB_DENSITY_SENS_MODE0;
            break;
        }
    }
    return MTRUE;
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MBOOL SonyIMX386PdafLibraryWrapper::SetCaliData( MUINT32 u4CaliDaSz, MUINT8 *pCaliDaBuf, MUINT32 u4RegAddDaPairSz, MUINT16 *pRegAddDaPairBuf)
{
    MBOOL ret = MFALSE;

    if( m_bLdDn)
    {
        m_u4SlopeOffsetDaNum = D_IMX386_SLOPE_OFFSET_BLK_NUM_TOTAL;
        memset( m_aData_Slope,  0, sizeof(signed long)*D_IMX386_SLOPE_OFFSET_BLK_NUM_TOTAL);
        memset( m_aData_Offset, 0, sizeof(signed long)*D_IMX386_SLOPE_OFFSET_BLK_NUM_TOTAL);
        ...
        if( pCaliDaBuf!=NULL)
        {
            ...
            if( (               u4CaliDaSz == (m_u4SlopeOffsetDaNum*sizeof(signed short))) &&
                    (D_IMX386_DCC_IN_EEPROM_BLK_NUM_W ==  D_IMX386_SLOPE_OFFSET_BLK_NUM_W)                  &&
                    (D_IMX386_DCC_IN_EEPROM_BLK_NUM_H ==  D_IMX386_SLOPE_OFFSET_BLK_NUM_H) )
            {
                AAA_LOGD("%s num:%d, sz:%d Byte, directly use DCC", __FUNCTION__, m_u4SlopeOffsetDaNum, u4CaliDaSz);

                for( j=0, k=0; j<D_IMX386_SLOPE_OFFSET_BLK_NUM_H; j++)
                {
                    for( i=0; i<D_IMX386_SLOPE_OFFSET_BLK_NUM_W; i++, k++)
                    {
                        unsigned long idx = j*D_IMX386_DCC_IN_EEPROM_BLK_NUM_W*sizeof(signed short) + (2*i);

                        m_aData_Slope[k]  = 0xffff & ( pCaliDaBuf[idx+1]<<8 | pCaliDaBuf[idx]);
                        m_aData_Offset[k] = 0;
                    }
                }
            }
            else if( (               u4CaliDaSz*4 == (m_u4SlopeOffsetDaNum*sizeof(signed short))) &&
                     (D_IMX386_DCC_IN_EEPROM_BLK_NUM_W*2 ==  D_IMX386_SLOPE_OFFSET_BLK_NUM_W)                  &&
                     (D_IMX386_DCC_IN_EEPROM_BLK_NUM_H*2 ==  D_IMX386_SLOPE_OFFSET_BLK_NUM_H) )
            {
                ...
                for( j=0, k=0; j<D_IMX386_SLOPE_OFFSET_BLK_NUM_H; j++)
                {
                    for( i=0; i<D_IMX386_SLOPE_OFFSET_BLK_NUM_W; i++, k++)
                    {
                        unsigned long idx = (j/2)*D_IMX386_DCC_IN_EEPROM_BLK_NUM_W*sizeof(signed short) + (2*i/2);
                     ...
                     (D_IMX386_DCC_IN_EEPROM_BLK_NUM_H == 2*D_IMX386_SLOPE_OFFSET_BLK_NUM_H) )
            {
                ...
                for( j=0, k=0; j<D_IMX386_SLOPE_OFFSET_BLK_NUM_H; j++)
                {
                    for( i=0; i<D_IMX386_SLOPE_OFFSET_BLK_NUM_W; i++, k++)
                    {
                        unsigned long idx0 = (2*j  )*D_IMX386_DCC_IN_EEPROM_BLK_NUM_W*sizeof(signed short) + (2*2*i);
                        unsigned long idx1 = (2*j+1)*D_IMX386_DCC_IN_EEPROM_BLK_NUM_W*sizeof(signed short) + (2*2*i);

                        signed long val00 = 0xffff & ( pCaliDaBuf[idx0+1]<<8 | pCaliDaBuf[idx0+0]);
                        signed long val01 = 0xffff & ( pCaliDaBuf[idx0+3]<<8 | pCaliDaBuf[idx0+2]);
                        signed long val10 = 0xffff & ( pCaliDaBuf[idx1+1]<<8 | pCaliDaBuf[idx1+0]);
                        signed long val11 = 0xffff & ( pCaliDaBuf[idx1+3]<<8 | pCaliDaBuf[idx1+2]);
        ...
        if( ret==MFALSE)
        {
            AAA_LOGE("[%s] Cali data is not valid : %p sz=%u(%u) W:%d(%d) H:%d(%d)",
                     __FUNCTION__,
                     pCaliDaBuf,
                     m_u4SlopeOffsetDaNum*sizeof(signed short),
                     u4CaliDaSz,
                     D_IMX386_DCC_IN_EEPROM_BLK_NUM_W,
                     D_IMX386_SLOPE_OFFSET_BLK_NUM_W,
                     D_IMX386_DCC_IN_EEPROM_BLK_NUM_H,
                     D_IMX386_SLOPE_OFFSET_BLK_NUM_H);
        }
        ...
        if( ret)
        {
            MUINT8 reg_addr_0x3100=0, reg_addr_0x3101=0, reg_addr_0x3102=0, reg_addr_0x3103=0;
            MUINT8 reg_addr_0x3104=0, reg_addr_0x3105=0, reg_addr_0x3106=0, reg_addr_0x3107=0;
            ...
            for( MUINT32 i=0; i<pairsz; i++)
            {
                ...
                case (0x300d) :
                    ...
                    break;
                case (0x3100) :
                    reg_addr_0x3100 = pRegAddDaPairBuf[idx+1];
                    break;
                case (0x3101) :
                    reg_addr_0x3101 = pRegAddDaPairBuf[idx+1];
                    break;
                case (0x3102) :
                    reg_addr_0x3102 = pRegAddDaPairBuf[idx+1];
                    break;
                case (0x3103) :
                    reg_addr_0x3103 = pRegAddDaPairBuf[idx+1];
                    break;
                case (0x3104) :
                    reg_addr_0x3104 = pRegAddDaPairBuf[idx+1];
                    break;
                case (0x3105) :
                    reg_addr_0x3105 = pRegAddDaPairBuf[idx+1];
                    break;
                case (0x3106) :
                    reg_addr_0x3106 = pRegAddDaPairBuf[idx+1];
                    break;
                case (0x3107) :
                    reg_addr_0x3107 = pRegAddDaPairBuf[idx+1];
                    break;
                default :
                    break;
                }
            ...
            m_u8PDArea_XOffset = ((reg_addr_0x3100&0x1F)<<8) | reg_addr_0x3101;
            m_u8PDArea_YOffset = ((reg_addr_0x3102&0x1F)<<8) | reg_addr_0x3103;
            m_u8PDArea_Width   = ((reg_addr_0x3104&0x1F)<<8) | reg_addr_0x3105;
            m_u8PDArea_Height  = ((reg_addr_0x3106&0x1F)<<8) | reg_addr_0x3107;
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MBOOL SonyIMX386PdafLibraryWrapper::SetPDCLData( MUINT32 u4PDCLDaSz, MUINT8 *pPDCLDaBuf)
{
    MBOOL ret = MFALSE;
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MBOOL SonyIMX386PdafLibraryWrapper::GetDefocus( SPDROIInput_T &iPDInputData, SPDROIResult_T &oPdOutputData)
{
    ...
    if( m_pfunGetDefocus && m_bLdDn && m_bInitDn)
    {
        ...
        if( (m_u8WindowModeSelect==0x0) || (m_u8WindowModeSelect==0x1))
        {
            ...
            {
                MUINT32 roi_center_x = (iPDInputData.ROI.i4XStart + iPDInputData.ROI.i4XEnd)/2;
                MUINT32 roi_center_y = (iPDInputData.ROI.i4YStart + iPDInputData.ROI.i4YEnd)/2;
            ...
            {
                AAA_LOGD("Config win mode setting : %d(%d)", m_u8WindowModeSelect, iPDInputData.mode);
                return 0;
            }
        }
        else if( m_u8WindowModeSelect==0x2) // Sensor is configured as fiexible window mode
        {
            // Host configure PD window mode as fiexible window setting
            if( ( (0x0<iPDInputData.mode)&&(iPDInputData.mode<=0xFF)) &&     // Checking host controlling mode
                    ( iPDInputData.mode==m_u8FiexibleWindowEn)            &&     // Checking fiexible window flag is valid
                    ( 0<=iPDInputData.ROI.i4Info && iPDInputData.ROI.i4Info<=7)) // Checking PD and CL data index
            {
                idxx = iPDInputData.ROI.i4Info;
                idxy = 0;
            }
            else
            {
                AAA_LOGD("Config win mode setting : 0x%x(0x%x)(0x%x),0x%x",
                         m_u8WindowModeSelect,
                         iPDInputData.mode,
                         m_u8FiexibleWindowEn,
                         iPDInputData.ROI.i4Info);
                return 0;
            }
        }
        ...
        MUINT16 CL  = 0x7FF & ( ((ptrPDCLDa[idx+0]<<3)&0x7F8) | ((ptrPDCLDa[idx+1]>>5)&0x7));
        MUINT16 PD  = 0x7FF & ( ((ptrPDCLDa[idx+1]<<6)&0x7C0) | ((ptrPDCLDa[idx+2]>>2)&0x3F));

        // Phase difference data and confidence level
        in->PhaseDifference = (signed long) (PD>=1024 ? PD-2048 : PD); // s6.4

        if( ((SonyPdLibSensorCoordSetting_t *)m_pTransRectArgIn)->img_orientation_h==1)
        {
            /*
             * When the image orientation for horizontal direction is set to normal, the sign of Phase Difference data
             * has the negative at the inf lens position, and has the positive at the macro lens position.
             *
             * When the image orientation for horizontal direction is set to reverse, the sign of Phase
             * Difference data has the positive at the inf lens position, and has the negative at the macro lens position.
             *
             * ISP should be reversed the sign of Phase Difference data which received from this sensor in case of horizontal reverse.
             */
            in->PhaseDifference = (-1)*in->PhaseDifference;
        }

        // Confidence level from image sensor.
        in->ConfidenceLevel = (unsigned long) (CL);   //u11.0
        ...
        WinAddr_I.sta.x = iPDInputData.ROI.i4XStart;
        WinAddr_I.sta.y = iPDInputData.ROI.i4YStart;
        WinAddr_I.end.x = iPDInputData.ROI.i4XEnd;
        WinAddr_I.end.y = iPDInputData.ROI.i4YEnd;
         ...
         * Execute
         */
        SonyPdLibOutputData_t  out;
        memset( &out, 0, sizeof(SonyPdLibOutputData_t));
        signed long err = ((GetDefocus_t)(m_pfunGetDefocus))( &in[0], &out);

        /*
         * Output to PD manager
         */
        if( err==D_SONY_PD_LIB_E_OK)
        {
            long defocus_14bit_shift = S1714_TO_S3100(out.Defocus);
            long defocus_extra_shift;
            int  extra_shift_bit = (D_IMX386_EXTRA_SHIFT_BIT)<0 ? (-1)*(D_IMX386_EXTRA_SHIFT_BIT) : (D_IMX386_EXTRA_SHIFT_BIT);

            if( (D_IMX386_EXTRA_SHIFT_BIT)<0)
            {
            ...
            AAA_LOGD_IF( m_u8DbgEn, "   Defocus [Extra Shift]  : %d (%dbit %s shifted)", (int)defocus_extra_shift, extra_shift_bit, (D_IMX386_EXTRA_SHIFT_BIT)<0 ? "left" : "right");
            ...
            oPdOutputData.Defocus                = (int)(iPDInputData.curLensPos + defocus_extra_shift);
            //(2) normalize to fit API spec : confidence level is in ragne 0~100.
            oPdOutputData.DefocusConfidence      = (int)(in->ConfidenceLevel*100/0x7ff);
            //(3) normalize to fit API spec : confidence level is in ragne 0~100.
            oPdOutputData.DefocusConfidenceLevel = (int)(in->ConfidenceLevel*100/0x7ff);
            //(4) normalize to fit API spec : pixel base.
            oPdOutputData.PhaseDifference        = (-1)*in->PhaseDifference*1000/16;
        }
        else
        {
        ...
        AAA_LOGD( "ImgSz:(%4d,%4d) ROI:{(%4d,%4d),(%4d,%4d)} idx:(%2d,%2d) P:%4d C:%3d G:%4d, Res={Pos:%4d->%4d C:%3d CL:%3d PD:%5d}",
                  (int)in->XSizeOfImage,
                  (int)in->YSizeOfImage,
                  ...
                  (int)oPdOutputData.DefocusConfidence,
