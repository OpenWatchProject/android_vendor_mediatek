...
CAM_MSDK_LOG("msdkGetFlickerPara ln=%d",__LINE__);
    pstSensorInitFunc[i].getCameraFlickerPara(SensorMode, 1, buf);
CAM_MSDK_LOG("msdkGetFlickerPara ln=%d",__LINE__);
    return 0;
}
...
int msdkGetFlickerParaAll(MUINT32 SensorId, int SensorMode, FLICKER_CUST_PARA*** buf, int binRatio)
{
    ...
    for(int j=0;j<SensorMode;j++)
    {
        ...
        {
            pstSensorInitFunc[i].getCameraFlickerPara(j, k+1, &((*buf)[j][k]));
        }
    }
...
MINT32 GetCameraBpciDefaultTbl(MUINT32 SensorId, PCAMERA_BPCI_STRUCT pCameraBPCIDefault)
{
    MINT32 ret = -1;
    MUINT32 i;
    CAM_MSDK_LOG("GetCameraBpciDefaultTbl\n");
    if (NULL == pstSensorInitFunc[0].getCameraDefault)
    {
        CAM_MSDK_LOG("[GetCameraDefaultPara]: uninit yet\n\n");
        return ret; //not initial pstSensorInitFunc yet
    ...
    CAM_MSDK_LOG("GetCameraBpciDefaultTbl - ret(%d)\n", ret);
