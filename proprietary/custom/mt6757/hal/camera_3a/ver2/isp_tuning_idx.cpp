...
dump() const
{
    MY_LOG(
        "[IndexMgr][dump]"
        " DBS:%d, OBC:%d, BPC:%d, NR1:%d, PDC:%d, RMM:%d ,RNR:%d, UDM:%d, ANR:%d, ANR2:%d,  CCR:%d, BOK:%d, HFG:%d, EE:%d, MFB:%d, MIX3:%d"
        , DBS,    OBC,    BNR_BPC,BNR_NR1,BNR_PDC,RMM,    RNR,    UDM,    ANR,    ANR2,     CCR,    BOK,    HFG,    EE,    MFB,    MIXER3
    );
}
...
init()
{
    ...
    ::memset(m_pN3D_HighRes,                     0, sizeof(m_pN3D_HighRes                 ));
    ::memset(m_pN3D_HighRes_toYUV,               0, sizeof(m_pN3D_HighRes_toYUV           ));
    ...
    ::memset(m_pHDR_Flash_Capture,            0, sizeof(m_pHDR_Flash_Capture));
...
#define LINK_N3D_HIGHRES(sensor, scene, iso)\
m_pN3D_HighRes[iso] = &IdxSet<EIspProfile_N3D_HighRes, sensor, scene, iso>::idx

#define LINK_N3D_HIGHRES_TOYUV(sensor, scene, iso)\
m_pN3D_HighRes_toYUV[iso] = &IdxSet<EIspProfile_N3D_HighRes_toYUV, sensor, scene, iso>::idx
...
#define LINK_HDR_FLASH_CAPTURE(sensor, scene, iso)\
m_pHDR_Flash_Capture[iso] = &IdxSet<EIspProfile_zHDR_Flash_Capture, sensor, scene, iso>::idx
...
LINK_ONE_SENSOR_ONE_SCENE_ISOs(LINK_N3D_HIGHRES, 0, 0);
...
LINK_ONE_SENSOR_ONE_SCENE_ISOs(LINK_N3D_HIGHRES_TOYUV, 0, 0);
...
LINK_ONE_SENSOR_ONE_SCENE_ISOs(LINK_HDR_FLASH_CAPTURE, 0, 0);
//==================================================================================================
...
get(MUINT32 ispProfile, MUINT32 sensor/*=0*/, MUINT32 const scene_/*=0*/, MUINT32 const iso/*=0*/, MUINT32 const zoom) const
{
    ...
    case EIspProfile_N3D_HighRes_toYUV:
        return get_N3D_HighRes_toYUV(sensor, scene, iso);
    case EIspProfile_N3D_HighRes:
        return  get_N3D_HighRes(sensor, scene, iso);
    ...
    case EIspProfile_zHDR_Flash_Capture:
        return  get_HDR_Flash_Capture(sensor, scene, iso);
