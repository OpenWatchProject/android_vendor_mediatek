...
MBOOL PDBufMgr::SetPDBlockInfo( SET_PD_BLOCK_INFO_T &iPDBlockInfo, SPDProfile_t &iPdProfile)
{
    ...
    if (m_sPdProfile.BufType == EPDBUF_PDO)
    {
        ...
        MUINT32 pitch_x = m_PDBlockInfo.i4PitchX;
        MUINT32 pitch_y = m_PDBlockInfo.i4PitchY;
        MUINT32 pairNum = m_PDBlockInfo.i4PairNum;

        /*******************************************************************************
         * Reference crop region and mirrorflip inction to modify PD block information.
         *******************************************************************************/
        MUINT32 full_pd_blk_offx = m_PDBlockInfo.i4OffsetX;
        MUINT32 full_pd_blk_offy = m_PDBlockInfo.i4OffsetY;

        // Current image related full size coordinate
        MUINT32 crop_x         = m_PDBlockInfo.i4Crop[m_sPdProfile.i4SensorMode][0];
        MUINT32 crop_y         = m_PDBlockInfo.i4Crop[m_sPdProfile.i4SensorMode][1];

        // Current pd block offset related to full size coordinate
        MINT32 offx = crop_x-full_pd_blk_offx;
		if( offx<=0)
		{
			offx = -crop_x;
		}
		else if( offx%pitch_x)
		{
			offx = ( (offx+(pitch_x-1)) / pitch_x) * pitch_x - crop_x;
		}
		else
		{
			offx = ( offx / pitch_x) * pitch_x - crop_x;
		}


        MINT32 offy = crop_y-full_pd_blk_offy;
		if( offy<=0)
		{
			offy = -crop_y;
		}
        else if( offy%pitch_y)
        {
            offy = ( (offy+(pitch_y-1)) / pitch_y) * pitch_y - crop_y;
        }
        else
        {
            offy = ( offy / pitch_y) * pitch_y - crop_y;
        }

        AAA_LOGD( "coordinate offset in current sensor mode (%d, %d)", offx, offy);

        /*******************************************************************************
         * calculate pd pixels' position by orientation and crop information for general separate function
         *******************************************************************************/
        MUINT32 cur_pd_blk_offx    = m_PDBlockInfo.i4OffsetX;
        MUINT32 cur_pd_blk_offy    = m_PDBlockInfo.i4OffsetY;

        AAA_LOGD( "PD block offset : X(%d->%d Y(%d->%d)",
        ...
        {
            MUINT32 PosL_X = m_PDBlockInfo.i4PosL[Pidx][0] + offx;
            MUINT32 PosL_Y = m_PDBlockInfo.i4PosL[Pidx][1] + offy;
            MUINT32 PosR_X = m_PDBlockInfo.i4PosR[Pidx][0] + offx;
            MUINT32 PosR_Y = m_PDBlockInfo.i4PosR[Pidx][1] + offy;
            MUINT32 idx    = Pidx;

            /* mirror*/
            if( m_PDBlockInfo.iMirrorFlip & 0x1)
            {
                PosL_X = pitch_x - (PosL_X - cur_pd_blk_offx) - 1 + cur_pd_blk_offx;
                PosR_X = pitch_x - (PosR_X - cur_pd_blk_offx) - 1 + cur_pd_blk_offx;
            }

            /* flip*/
            if( m_PDBlockInfo.iMirrorFlip & 0x2)
            {
                PosL_Y = pitch_y - (PosL_Y - cur_pd_blk_offy) - 1 + cur_pd_blk_offy;
                PosR_Y = pitch_y - (PosR_Y - cur_pd_blk_offy) - 1 + cur_pd_blk_offy;
                idx    = (pairNum - 1) - Pidx;
            }

            pdPixels[idx][0] = PosR_X;
            pdPixels[idx][1] = PosR_Y;
            pdPixels[idx + pairNum][0] = PosL_X;
            pdPixels[idx + pairNum][1] = PosL_Y;

            AAA_LOGD( "PDPos [L][%3d %3d]->[%3d %3d], [R][%3d %3d]->[%3d %3d]",
                      m_PDBlockInfo.i4PosL[Pidx][0],
                      m_PDBlockInfo.i4PosL[Pidx][1],
                      pdPixels[idx + pairNum][0],
                      pdPixels[idx + pairNum][1],
                      m_PDBlockInfo.i4PosR[Pidx][0],
                      m_PDBlockInfo.i4PosR[Pidx][1],
                      pdPixels[idx][0],
                      pdPixels[idx][1]);
        }
        if( pdoPixelPos( pdPixels, pairNum*2, m_PDBlockInfo.i4SubBlkH) == MFALSE)
        {
            AAA_LOGE("pdoPixelsPos parsing PD coordinates Error!");
        }
    }
...
MBOOL PDBufMgr::pdoPixelPos(MUINT32 pPdoPixPos[][2], MUINT32 pdNum, MUINT32 subBlkH)
{
    ...
    parsePdPixels(pdPixels, pdNum, subBlkH);
