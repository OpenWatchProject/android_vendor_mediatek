...
{
    CAM_MSDK_LOG("[%s] this function is not support yet", __func__);
    return 0;
}
...
MUINT32 GetCameraCalData(MUINT32 SensorId, MUINT32* pGetSensorCalData)
{
    ...
    for (i=0; pstSensorInitFunc[i].SensorId != 0 ;i++)
    {
        if (SensorId == pstSensorInitFunc[i].SensorId)
        {
            CAM_MSDK_LOG("SensorId == pstSensorInitFunc[%d].SensorId=%x \n",i ,pstSensorInitFunc[i].SensorId);
            break;
        }
		else
		{
    ...
    if (pstSensorInitFunc[i].getCameraCalData != NULL)
    {
		...
		CAM_MSDK_LOG("Enter! GetCameraCalData(MainSensorIdx=%x) Enter\n",SensorId);
		CAM_MSDK_LOG("pstSensorInitFunc(MainSensorIdx=%x) Enter\n",pstSensorInitFunc[i].SensorId);
        result = pstSensorInitFunc[i].getCameraCalData(pGetSensorCalData);
        CAM_MSDK_LOG("Done! GetCameraCalData(MainSensorIdx=%x) Enter\n",SensorId);
...
void GetLensDefaultPara(PNVRAM_LENS_PARA_STRUCT pLensParaDefault)
{
    ...
    */
	...
	{
        MUINT32 i = (gMainLensIdx<MAX_NUM_OF_SUPPORT_LENS) ? gMainLensIdx : 0;
...
int msdkGetFlickerParaAll(MUINT32 SensorId, int SensorMode, FLICKER_CUST_PARA*** buf, int binRatio)
{
    ...
    if (!pstSensorInitFunc[0].getCameraFlickerPara) {
        CAM_MSDK_LOG("msdkGetFlickerPara(): function pointer is NULL.");
        return -1; //not initial pstSensorInitFunc yet
    }

    for (i = 0; i < MAX_NUM_OF_SUPPORT_SENSOR; i++) {
        if (SensorId == pstSensorInitFunc[i].SensorId) {
            CAM_MSDK_LOG("msdkGetFlickerPara(): find sensor(%d).", i);
            break;
        }
    }

    if (MAX_NUM_OF_SUPPORT_SENSOR == i) {
        CAM_MSDK_LOG("msdkGetFlickerPara(): no match id(%d).", SensorId);
        return -1;
    }

    for(int j=0;j<SensorMode;j++)
    {
            pstSensorInitFunc[i].getCameraFlickerPara(j, &((*buf)[j][0]));
    }
...
MINT32 GetCameraBpciDefaultTbl(MUINT32 SensorId, PCAMERA_BPCI_STRUCT pCameraBPCIDefault)
{
    ...
    if (pCameraBPCIDefault!=NULL)
        ret = pstSensorInitFunc[i].getCameraDefault(CAMERA_DATA_PDC_TABLE,(VOID*)pCameraBPCIDefault,sizeof(CAMERA_BPCI_STRUCT));
