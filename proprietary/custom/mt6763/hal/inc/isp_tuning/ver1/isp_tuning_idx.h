...
private:
    ...
    INDEX_T const* m_pN3D_HighRes                   [eNUM_OF_ISO_IDX];
    INDEX_T const* m_pN3D_HighRes_toYUV             [eNUM_OF_ISO_IDX];
    ...
    INDEX_T const* m_pHDR_Flash_Capture             [eNUM_OF_ISO_IDX];
...
private:    ////    Normal
    ...
    inline INDEX_T const* get_N3D_HighRes(MUINT32 const /*sensor*/, MUINT32 const /*scene*/, MUINT32 const iso) const
    {
        return  isInvalid(0, 0, iso) ? NULL : m_pN3D_HighRes[iso];
    }

    inline INDEX_T const* get_N3D_HighRes_toYUV(MUINT32 const /*sensor*/, MUINT32 const /*scene*/, MUINT32 const iso) const
    {
        return  isInvalid(0, 0, iso) ? NULL : m_pN3D_HighRes_toYUV[iso];
    }
    ...
    inline INDEX_T const* get_HDR_Flash_Capture(MUINT32 const /*sensor*/, MUINT32 const /*scene*/, MUINT32 const iso) const
    {
        return  isInvalid(0, 0, iso) ? NULL : m_pHDR_Flash_Capture[iso];
    }
...
// IDX_SET
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
#define IDX_SET(DBS, OBC, BNR_BPC, BNR_NR1, BNR_PDC, RNR, UDM, ANR, ANR2, CCR, HFG, EE, NR3D, MFB, MIXER3)\
    {\
        DBS, OBC, BNR_BPC, BNR_NR1, BNR_PDC, RNR, UDM, ANR, ANR2, CCR, HFG, EE, NR3D, MFB, MIXER3\
    }
...
#define IDX_MODE_N3D_HighRes(iso)\
    template <> INDEX_T const IdxSet<EIspProfile_N3D_HighRes, 0, 0, iso>::idx =

#define IDX_MODE_N3D_HighRes_toYUV(iso)\
    template <> INDEX_T const IdxSet<EIspProfile_N3D_HighRes_toYUV, 0, 0, iso>::idx =
...
#define IDX_MODE_HDR_Flash_Capture(iso)\
