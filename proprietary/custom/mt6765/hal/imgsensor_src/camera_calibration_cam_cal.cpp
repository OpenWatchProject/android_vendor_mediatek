...
static UINT32 DoCamCal2AGain_2P7(INT32 CamcamFID, UINT32 start_addr, UINT32 BlockSize, UINT32* pGetSensorCalData);
static UINT32 DoCamCalPDAF_2P7(INT32 CamcamFID, UINT32 start_addr, UINT32 BlockSize, UINT32* pGetSensorCalData);
...
#define GOLDEN_BGAIN  349 // Gr/Gb
    ...
    else if(pCamCalData->DataVer < CAM_CAL_N3D_DATA)
    {
        ...
        cam_calCfg.u4Offset = start_addr;//start_addr==0x0030
        cam_calCfg.u4Length = 1;
        cam_calCfg.pu1Params = (u8 *)&AWB_Flag;//cam_calCfg.pu1Params = (u8 *)&AWBAFConfig;//0x0f
        cam_calCfg.sensorID = pCamCalData->sensorID;
        cam_calCfg.deviceID = pCamCalData->deviceID;
        ioctlerr= ioctl(CamcamFID, CAM_CALIOC_G_READ, &cam_calCfg);
        if(ioctlerr>0)
        ...
        if(0x1 == AWB_Flag){
             ...
             cam_calCfg.u4Offset = (start_addr+1); //start_addr==0x0030,  0x0031 AWB  information size=12
             CAM_CAL_LOG_IF(dumpEnable,"AWB offset=%d\n", cam_calCfg.u4Offset);
             cam_calCfg.u4Length = 12;//4;
             cam_calCfg.pu1Params = (u8 *)&rgbGain[0];//(u8 *)&CalGain;
             ioctlerr= ioctl(CamcamFID, CAM_CALIOC_G_READ, &cam_calCfg);
            ...
            cam_calCfg.u4Offset = 0x0020;//(start_addr+7); //start_addr==0x0020, AF Flag
            cam_calCfg.u4Length = 1;
            cam_calCfg.pu1Params = (u8 *)&AF_Flag;//0x0f
            ioctlerr= ioctl(CamcamFID, CAM_CALIOC_G_READ, &cam_calCfg);
            ...
            if(0x1 == AF_Flag){
                ...
                cam_calCfg.u4Offset = 0x0022; //start_addr==0x0022, AF infinite
                cam_calCfg.u4Length = 2;
                //cam_calCfg.pu1Params = (u8 *)&AFInf;
                cam_calCfg.pu1Params = (u8 *)&AFInf[0];
                ioctlerr= ioctl(CamcamFID, CAM_CALIOC_G_READ, &cam_calCfg);
                if(ioctlerr>0)
                {
                     err = CAM_CAL_ERR_NO_ERR;
                ...
                cam_calCfg.u4Offset = 0x0024; //start_addr==0x0024, AF Macro
                cam_calCfg.u4Length = 2;
                //cam_calCfg.pu1Params = (u8 *)&AFMacro;
                cam_calCfg.pu1Params = (u8 *)&AFMacro[0];
                ioctlerr= ioctl(CamcamFID, CAM_CALIOC_G_READ, &cam_calCfg);
                if(ioctlerr>0)
                {
                    err = CAM_CAL_ERR_NO_ERR;
...
static UINT32 DoCamCalPDAF_2P7(INT32 CamcamFID, UINT32 start_addr, UINT32 BlockSize, UINT32* pGetSensorCalData)
{
    ...
    cam_calCfg.u4Offset = (start_addr + 4); //start_addr==0x0900,  0x0904 PDAF Proc1 Flag == 0x01

    cam_calCfg.u4Length = 1;
    cam_calCfg.pu1Params= (u8 *)&pdaf_valid;
    CAM_CAL_LOG_IF(dumpEnable,"u4Offset=%d u4Length=%d pu1Params= %p ", cam_calCfg.u4Offset, cam_calCfg.u4Length, cam_calCfg.pu1Params);
    ioctlerr= ioctl(CamcamFID, CAM_CALIOC_G_READ, &cam_calCfg);
    if(!ioctlerr)
    {
    ...
    if(pdaf_valid == 1)
    {
        cam_calCfg.u4Offset = (start_addr + 7); //start_addr==0x0900,  PDAF Proc1 start 0x0907 size 496
    ...
    cam_calCfg.u4Offset = (start_addr + 0x1F8); //start_addr==0x0900,  PDAF Proc2 Flag=0x0AF8 size 1
    cam_calCfg.u4Length = 1;
    cam_calCfg.pu1Params= (u8 *)&pdaf_valid;
    CAM_CAL_LOG_IF(dumpEnable,"u4Offset=%d u4Length=%d pu1Params= %p ", cam_calCfg.u4Offset, cam_calCfg.u4Length, cam_calCfg.pu1Params);
    ioctlerr= ioctl(CamcamFID, CAM_CALIOC_G_READ, &cam_calCfg);
    ...
    if(pdaf_valid == 1)
    {
        cam_calCfg.u4Offset = (start_addr + 0x1FB); //start_addr==0x0900,  PDAF Step2 start=0x0AFB size 876
        cam_calCfg.u4Length = 908;
