...
_getAFParam<ESensorDev_SubSecond>( AF_PARAM_T const **outAFParam)
{
    ...
    AAA_LOGD( "%s : dev( ESensorDev_SubSecond ) get specialization setting, tbl_size(%d)",
              __FUNCTION__,
              (*outAFParam)->i4TBLL);
...
_getAFConfig<ESensorDev_SubSecond>( AF_CONFIG_T const **outAFCfg)
{
    ...
    AAA_LOGD( "%s : dev( ESensorDev_SubSecond ) get specialization setting, AF_EXT_ENABLE(%d)",
              __FUNCTION__,
