...
typedef struct ScnParam{
    unsigned char CaptureIntent;
    unsigned char HdrMode;
    int Sync2AMode;
    int TargetMode;
    unsigned int SensorMode;
    ScnParam(): CaptureIntent(1), HdrMode(0), Sync2AMode(0), TargetMode(0), SensorMode(0){}
    ScnParam(unsigned char cap, unsigned char hdr, int sync2a, int target, unsigned int sensor)
    : CaptureIntent(cap)
    , HdrMode(hdr)
    , Sync2AMode(sync2a)
    , TargetMode(target)
