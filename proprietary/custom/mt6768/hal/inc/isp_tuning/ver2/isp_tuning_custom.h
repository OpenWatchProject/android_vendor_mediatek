...
public:     ////    Operations.
    ...
    MVOID
    refine_SL2J(
        RAWIspCamInfo const& rCamInfo, IspNvramRegMgr & rIspRegMgr, ISP_NVRAM_SL2_T& rSL2J
    );
    ...
    MVOID
    refine_HLR(
        RAWIspCamInfo const& rCamInfo, IspNvramRegMgr & rIspRegMgr, ISP_NVRAM_HLR_T& rHLR
    );
    ...
    virtual
    EIndex_PCA_LUT_CT_T
    evaluate_SmoothCOLOR_CT_idx    (
        RAWIspCamInfo const& rCamInfo, MUINT32 &CTlower, MUINT32 &CTupper
    );

    virtual
    EIndex_ANR_TBL_ISO_T
    evaluate_ANR_TBL_ISO_index(
        RAWIspCamInfo const& rCamInfo, MBOOL &inter, EIndex_ANR_TBL_ISO_T const ePrvIdx
    );

    virtual
    EIndex_ANR_TBL_CT_T
    evaluate_ANR_TBL_CT_index(
        RAWIspCamInfo const& rCamInfo, EIndex_ANR_TBL_CT_T const ePrvIdx
    );
