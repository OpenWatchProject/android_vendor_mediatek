...
static inline MBOOL isLimitEIS12()
{
    ...
    if (gPowerHal == NULL)
    {
        MY_LOGD("PowerHal is NULL");
        return false;
    }

    MUINT32 coreNum = 0;
    MUINT32 numOfCluster = gPowerHal->querySysInfo(MtkQueryCmd::CMD_GET_CLUSTER_NUM, 0);

    for( MUINT32 i = 0; i < numOfCluster; i++)
    {
        coreNum += gPowerHal->querySysInfo(MtkQueryCmd::CMD_GET_CLUSTER_CPU_NUM, i);
    }

    if( coreNum < 5 )
    {
        MY_LOGD("Current coreNum=%d. Only support EIS 1.2", coreNum);
        return true;
    }
