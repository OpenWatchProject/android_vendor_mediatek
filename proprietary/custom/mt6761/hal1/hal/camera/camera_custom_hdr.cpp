...
MVOID getHDRExpSetting(const HDRExpSettingInputParam_T& rInput, HDRExpSettingOutputParam_T& rOutput)
{
    ...
    else //if(rInput.u4AOEMode!=1) //for normal mode (non-VHDR mode) //rInput.u4AOEMode==0 : standard bracketing, rInput.u4AOEMode==2 : adaptive bracketing
    {
        ...
        */
        ...
        if(rOutput.u4OutputFrameNum == 3)
        {
            ...
            {
                CAM_LOGD("SWHDR Error calculation of GainDiff = %f %f\n",dfGainDiff[0], dfGainDiff[1]);
            }
            ...
            {
                CAM_LOGD("getHDRExpSetting dfGainDiff Error ! %d %d\n", dfGainDiff[0], dfGainDiff[1]);
            }
        }
        else if(rOutput.u4OutputFrameNum == 2)
        {
            ...
            {
                CAM_LOGD("SWHDR Error calculation of GainDiff = %f\n",dfGainDiff[0]);
            }
            ...
            {
                CAM_LOGD("getHDRExpSetting dfGainDiff Error ! %d\n", dfGainDiff[0]);
            }
        }
    ...
    if(hdr_debug_mode)
    {
        ...
        CAM_LOGD("SWHDR Input delta EV(Bottom) = %f, Normal EV = %f, TopEV = %f, Avg0_40 = %d Avg0_40_2 = %f", dfTargetBottomEV, dfTargetNormalEV, dfTargetTopEV, rInput.i4LE_LowAvg, DarkAvg);

        CAM_LOGD("SWHDR Face detected= %d", rInput.bDetectFace);

        CAM_LOGD("SWHDR AE HDRFlag Mode = %d", HDRFlag);
        CAM_LOGD("SWHDR Input i4aeTableCurrentIndex = %d, 0EV Time = %d, Gain = %d Flare = %d", rInput.i4aeTableCurrentIndex, rInput.u4ExpTimeInUS0EV, rInput.u4SensorGain0EV,rInput.u1FlareOffset0EV);
        CAM_LOGD("SWHDR Output u4TargetTopIdx %d, u4TargetNormalIdx %d, u4TargetBottomIdx %d", u4TargetTopIdx, u4TargetNormalIdx, u4TargetBottomIdx);
