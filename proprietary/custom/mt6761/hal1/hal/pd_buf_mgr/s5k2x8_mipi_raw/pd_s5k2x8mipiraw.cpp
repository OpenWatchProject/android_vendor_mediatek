...
void PD_S5K2X8MIPIRAW::seprate( unsigned int stride, unsigned char *ptr, int pd_x_num, int pd_y_num, unsigned short *ptrLROut, unsigned int bitDepth)
{
    ...
    for( int i=0; i<pd_y_num/8; i++)
    {
        for( int j=0; j<pd_x_num; j+=4)
        {
            ...
            for( int k=0; k<4; k++)
            {
                *pout1 = tempMap[(*ptable1)+((*pmultiple1)*count)+idx]>>shift_bits;
                *pout2 = tempMap[(*ptable2)+((*pmultiple2)*count)+idx]>>shift_bits;
                *pout3 = tempMap[(*ptable3)+((*pmultiple3)*count)+idx]>>shift_bits;
                *pout4 = tempMap[(*ptable4)+((*pmultiple4)*count)+idx]>>shift_bits;
                *pout5 = tempMap[(*ptable5)+((*pmultiple5)*count)+idx]>>shift_bits;
                *pout6 = tempMap[(*ptable6)+((*pmultiple6)*count)+idx]>>shift_bits;
                *pout7 = tempMap[(*ptable7)+((*pmultiple7)*count)+idx]>>shift_bits;
                *pout8 = tempMap[(*ptable8)+((*pmultiple8)*count)+idx]>>shift_bits;
                ptable1++;
                ptable2++;
                ptable3++;
                ptable4++;
...
MUINT16* PD_S5K2X8MIPIRAW::ConvertPDBufFormat( MUINT32 /* i4Size */, MUINT32 i4Stride, MUINT8 *ptrBufAddr, MUINT32 u4BitDepth)
{
    ...
    seprate( i4Stride, ptrBufAddr, m_PDXSz, m_PDYSz, m_PDBuf, u4BitDepth);
