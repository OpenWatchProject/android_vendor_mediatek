...
MUINT32 PD_IMX519MIPIRAW::m_u4Reg_AdDaPair_Sz = (sizeof(MUINT16)*84/sizeof(MUINT8));

MUINT16 PD_IMX519MIPIRAW::m_u2Reg_AdDaPair[84] =
{
    0x0101, 0x0000,
    0x0220, 0x0000,
    0x0221, 0x0000,
...
MUINT16 PD_IMX519MIPIRAW::m_u2Reg_AdDaPair_PDWin_Flexible_Cmd[146] =
{
    0x0000, 0x00,
};
...
GetPDWinSetting( MVOID* arg1, MVOID* arg2, MVOID* arg3)
{
    ...
    S_ANALYSIS_DATA_OPEN_T *cfgRoi = (S_ANALYSIS_DATA_OPEN_T *)arg1;
...
#define MAX_FLEXIBLE_WIN_SIZE 8
    ...
    m_LastWinCfgIdx = 0;
    m_LastWinCfgIdx = (m_LastWinCfgIdx<(cfgRoi->numROIs-1))       ? (cfgRoi->numROIs-1) : m_LastWinCfgIdx;
    ...
    for( MUINT32 i=0, roi_idx=0; i<MAX_FLEXIBLE_WIN_SIZE; i++)
    {
        ...
        AAA_LOGD( "%d cfgNum(0x%x) , roi_idx = %d", i, cfgRoi->cfgNum,roi_idx);
    ...
    *((MUINT16 **)arg3) = &(m_u2Reg_AdDaPair_PDWin_Flexible_Cmd[0]);
    ...
    {
        AAA_LOGD( "DUMP_REG_ALL : %d (0x%x, 0x%x)", i, m_u2Reg_AdDaPair_PDWin_Flexible_All[i], m_u2Reg_AdDaPair_PDWin_Flexible_All[i+1]);
    }
    for(int i=0; i<146; i+=2)
    {
        AAA_LOGD( "DUMP_REG_CMD : %d (0x%x, 0x%x)", i, m_u2Reg_AdDaPair_PDWin_Flexible_Cmd[i], m_u2Reg_AdDaPair_PDWin_Flexible_Cmd[i+1]);
    }
    ...
    *((MUINT16 **)arg3) = &(m_u2Reg_AdDaPair_PDWin_Fix[0]);
    ...
    {
        AAA_LOGD( "DUMP_REG_CMD : %d (0x%x, 0x%x)", i, m_u2Reg_AdDaPair_PDWin_Fix[i], m_u2Reg_AdDaPair_PDWin_Fix[i+1]);
    }
    ret = MTRUE;
#endif
...
IsSupport( SPDProfile_t &iPdProfile)
{
    ...
    else if(iPdProfile.uImgXsz==4656 && iPdProfile.uImgYsz==2618)
    {
        ret = MTRUE;
        m_CurrMode = MODE_FULL;
    }
