...
typedef enum
{
    ...
    ACDK_CCT_D1_OP_ISP_GET_DNG_PARA,
    ACDK_CCT_D1_OP_ISP_SET_DNG_PARA,
    ...
    ACDK_CCT_V2_OP_GET_ISP_CCM_RATIO,
    ACDK_CCT_V2_OP_SET_ISP_CCM_RATIO,
...
typedef struct
{
    ...
    MUINT16 GrabXOffset;
    MUINT16 GrabYOffset;
    MUINT32 rawFmtType; //Bayer or Mono,....
}ACDK_CCT_SENSOR_STATIC_INFO;
...
struct ACDK_CCT_SET_CURRENT_CCM_STRUCT
{
    MUINT8                      u1Type; //0: ISP (default); 1: MDP
...
struct ACDK_CCT_NVRAM_CCM_PARA
{
    ...
    ACDK_CCT_CCM_STRUCT         mdp_ccm[NVRAM_CCM_TBL_NUM];
};

struct ACDK_CCT_SET_CCM_WB_STRUCT
{
    MUINT8                        u1Type; //0: ISP (default); 1: MDP
    ISP_NVRAM_CCM_AWB_GAIN_STRUCT ccm_awb_gain;
};

struct ACDK_CCT_SET_CCM_POLY22_STRUCT
{
    MUINT8                        u1Type; //0: ISP (default); 1: MDP
    ISP_NVRAM_CCM_POLY22_STRUCT   ccm_poly22;
};

struct ACDK_CCT_COMMON_IO_STRUCT
{
    #define ACDK_CCT_COMMON_IO_STRUCT_DATA_LENGTH 100
    MUINT8                      data[ACDK_CCT_COMMON_IO_STRUCT_DATA_LENGTH]; //100 bytes
    ACDK_CCT_COMMON_IO_STRUCT()
        {
            ::memset(&data[0], 0x0, ACDK_CCT_COMMON_IO_STRUCT_DATA_LENGTH); //initialized to be 0
        }
};
