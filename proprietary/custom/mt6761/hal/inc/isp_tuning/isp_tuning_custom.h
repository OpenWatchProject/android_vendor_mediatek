...
public:  ////    Ctor/Dtor.
    IspTuningCustom() 
	{
	   s_bPerFramePropEnable = ::property_get_bool("debug.aaa.perframe_prop.enable", MFALSE);
	}
    virtual ~IspTuningCustom() {}

    MINT32 propertyGet(const char * pProperty, MINT32 i4Default);

    static IspTuningCustom* createInstance(ESensorDev_T const eSensorDev, MUINT32 const u4SensorID);
    virtual void destroyInstance() = 0;
private:
    static MBOOL s_bPerFramePropEnable;
...
public:     ////    Operations.
    ...
    MVOID
    refine_NSL2(
        RAWIspCamInfo const& rCamInfo, IspNvramRegMgr & rIspRegMgr, ISP_NVRAM_NSL2_T& rNSL2
    );
    ...
    evaluate_ANR_TBL_ISO_index(
        RAWIspCamInfo const& rCamInfo, MBOOL &inter, MUINT32 &UpperIso, MUINT32 &LowerIso
    );
