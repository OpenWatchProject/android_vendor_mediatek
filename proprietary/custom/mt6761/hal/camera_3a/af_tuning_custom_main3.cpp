...
template <>
MBOOL
_getAFParam<ESensorDev_MainThird>( AF_PARAM_T const **outAFParam)
{
    ...
    AAA_LOGD( "%s : dev( ESensorDev_MainThird ) get specialization setting, 0x%x, tbl_size(%d)",
              __FUNCTION__,
              (*outAFParam),
              (*outAFParam)->i4TBLL);
...
template <>
MBOOL
_getAFConfig<ESensorDev_MainThird>( AF_CONFIG_T const **outAFCfg)
{
    ...
    AAA_LOGD( "%s : dev( ESensorDev_MainThird ) get specialization setting, 0x%x, AF_EXT_ENABLE(%d)",
              __FUNCTION__,
              (*outAFCfg),
