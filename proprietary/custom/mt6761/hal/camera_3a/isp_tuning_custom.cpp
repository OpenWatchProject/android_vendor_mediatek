...
static MTK_GMA_ENV_INFO_STRUCT gsGMAEnvParam_main =
{
    ...
    {
        eFIXED_GMA_MODE,//eMTK_GMA_MODE,  // eGMAMode //modify for Gamma mode by lili 20180626
        8,                  // i4LowContrastThr
...
static MTK_GMA_ENV_INFO_STRUCT gsGMAEnvParam_sub2 =
{
    {
        {
            {
...
static ISP_CUSTOM_PARA_PCA_T gsSmoothPCA_para_main ={0, 0, 0, 0, 64, 0, 0};

static ISP_CUSTOM_PARA_PCA_T gsSmoothPCA_para_sub ={0, 0, 0, 0, 64, 0, 0};

static ISP_CUSTOM_PARA_PCA_T gsSmoothPCA_para_main2 ={0, 0, 0, 0, 64, 0, 0};

static ISP_CUSTOM_PARA_PCA_T gsSmoothPCA_para_sub2 ={0, 0, 0, 0, 64, 0, 0};
...
MBOOL IspTuningCustom::s_bPerFramePropEnable = MFALSE;

MINT32
IspTuningCustom::propertyGet(const char * pProperty, MINT32 i4Default)
{
...
//..............................................................................
    ...
    property_get("vendor.debug.nvram_idx.log", InputValue, "0");
    ...
    if(TUNING_FOR_AIS) {
        if(IS_AIS) {
            ...
            )
            {
                ...
                // base on AIS index
                MINT32 deltaUdm = map_ISO_value_to_index(cfaIso) - aisIndex;
...
#if 0//def TUNING_ISP_DBG
char value[PROPERTY_VALUE_MAX] = {0};
property_get("tuning_isp_debug", value, "0");
tuneDebug = atoi(value);
#endif
MINT32  MY_OBC = rIdxMgr.getIdx_OBC();
MINT32  MY_BPC = rIdxMgr.getIdx_BPC();
MINT32  MY_UDM = rIdxMgr.getIdx_UDM();
MINT32  MY_ANR = rIdxMgr.getIdx_ANR();
MINT32  MY_EE =  rIdxMgr.getIdx_EE();
MINT32  MY_ISO = rCamInfo.rAEInfo.u4RealISOValue;
MINT32  ISO_index = map_ISO_value_to_index(rCamInfo.rAEInfo.u4RealISOValue) ;

#if 1
    if((getSensorID() == 0x80c4)||(getSensorID() == 0x8044))
	{
	   	if((rCamInfo.eIspProfile==0) && (rCamInfo.eSensorMode==1)&&(rCamInfo.rAEInfo.i4LightValue_x10<=0))//for lowlight to preview Capture mode
		{
			 //rIdxMgr.setIdx_GGM(1);
			 rIdxMgr.setIdx_UDM(19);  // 019: profile = preview, sensor mode = capture iso = 3200
			 rIdxMgr.setIdx_ANR(19);  // 019: profile = preview, sensor mode = capture iso = 3200
			 rIdxMgr.setIdx_EE(19);   // 019: profile = preview, sensor mode = capture iso = 3200
		}

		if((rCamInfo.rFlashInfo.isFlash != 0)||(rCamInfo.eIspProfile==5))//for flash to preview video mode
		{
			 rIdxMgr.setIdx_UDM(20+ISO_index);  // 020: profile = preview, sensor mode = video, iso = 100
			 rIdxMgr.setIdx_ANR(20+ISO_index);  // 020: profile = preview, sensor mode = video, iso = 100
			 rIdxMgr.setIdx_EE(20+ISO_index);   // 020: profile = preview, sensor mode = video, iso = 100
		}

		if((rCamInfo.i4ZoomRatio_x100>200)&&(rCamInfo.i4ZoomRatio_x100<300))//for perview zoomratio2x to preview video1 mode
		{
			 rIdxMgr.setIdx_UDM(30+ISO_index);  // 030: profile = preview, sensor mode = SlimVideo1, iso = 100
			 rIdxMgr.setIdx_ANR(30+ISO_index);  // 030: profile = preview, sensor mode = SlimVideo1, iso = 100
			 rIdxMgr.setIdx_EE(30+ISO_index);   // 030: profile = preview, sensor mode = SlimVideo1, iso = 100
		}

		if(rCamInfo.i4ZoomRatio_x100>=300)//for perview zoomratio3X &4X to preview video2 mode
		{
			 rIdxMgr.setIdx_UDM(40+ISO_index);  // 040: profile = preview, sensor mode = SlimVideo2, iso = 100
			 rIdxMgr.setIdx_ANR(40+ISO_index);  // 040: profile = preview, sensor mode = SlimVideo2, iso = 100
			 rIdxMgr.setIdx_EE(40+ISO_index);   // 040: profile = preview, sensor mode = SlimVideo2, iso = 100
		}
		//MY_LOG("sandy test---->zoomration>%d\n",rCamInfo.i4ZoomRatio_x100);
		//TODO:to turn  hdr or night mode 4 1 to 1 0,  Please check this log when use auto hdr & hdr & night mode

	   	if((rCamInfo.eIspProfile==4) && (rCamInfo.eSensorMode==1))//for nightmode to preview video mode
		{
			 rIdxMgr.setIdx_UDM(110+ISO_index);   // 110: profile = capture, sensor mode = capture, iso = 100
			 rIdxMgr.setIdx_ANR(110+ISO_index);   // 110: profile = capture, sensor mode = capture, iso = 100
			 rIdxMgr.setIdx_EE(110+ISO_index);    // 110: profile = capture, sensor mode = capture, iso = 100
		}

		if((rCamInfo.eIdx_Scene == 18)&&(rCamInfo.eIspProfile != 0))//for nightmode to preview video mode
		{
			 rIdxMgr.setIdx_UDM(140+ISO_index);   // 140: profile = capture, sensor mode = SlimVideo2, iso = 100
			 rIdxMgr.setIdx_ANR(140+ISO_index);   // 140: profile = capture, sensor mode = SlimVideo2, iso = 100
			 rIdxMgr.setIdx_EE(140+ISO_index);    // 140: profile = capture, sensor mode = SlimVideo2, iso = 100
		}
...
#if 1
    if((getSensorID() == 0x30c6) || (getSensorID() == 0xb0c6))
   {
         MINT32 ISO_index=map_ISO_value_to_index(rCamInfo.rAEInfo.u4RealISOValue);
         if((rCamInfo.eIspProfile==0 && rCamInfo.eSensorMode ==1)&&(rCamInfo.i4ZoomRatio_x100>170)&&(rCamInfo.i4ZoomRatio_x100<800))
		{
			/* rIdxMgr.setIdx_UDM(30+ISO_index);  // profile = preview, sensor mode = SlimVideo1
			 rIdxMgr.setIdx_ANR(30+ISO_index);  // profile = preview, sensor mode = SlimVideo1
			 rIdxMgr.setIdx_EE(30+ISO_index);   // profile = preview, sensor mode = SlimVideo1 */
		}else if((rCamInfo.eIspProfile==0 && rCamInfo.eSensorMode ==1)&&(rCamInfo.i4ZoomRatio_x100>=800))
		{
		     rIdxMgr.setIdx_BPC(40+ISO_index);  // profile = preview, sensor mode = SlimVideo2
			 rIdxMgr.setIdx_UDM(40+ISO_index);  // profile = preview, sensor mode = SlimVideo2
			 rIdxMgr.setIdx_ANR(40+ISO_index);  // profile = preview, sensor mode = SlimVideo2
			 rIdxMgr.setIdx_EE(40+ISO_index);   // profile = preview, sensor mode = SlimVideo2
        }
   }
	MY_LOG("tuning_isp: sensor_id=[0x%x],bpc=[%d],udm=[%d],anr=[%d],ee=[%d],\
[eProfile,eSensor]=[%d ,%d],rCamInfo.eIdx_Scene=[%d]\n",getSensorID(),\
rIdxMgr.getIdx_BPC(),rIdxMgr.getIdx_UDM(),rIdxMgr.getIdx_ANR(),rIdxMgr.getIdx_EE(),\
rCamInfo.eIspProfile,rCamInfo.eSensorMode,rCamInfo.eIdx_Scene);

#endif

MY_OBC = rIdxMgr.getIdx_OBC();
MY_BPC = rIdxMgr.getIdx_BPC();
MY_UDM = rIdxMgr.getIdx_UDM();
MY_ANR = rIdxMgr.getIdx_ANR();
MY_EE =  rIdxMgr.getIdx_EE();
MY_ISO = rCamInfo.rAEInfo.u4RealISOValue;//rCamInfo.rAEInfo.u4RealISOValue;
MY_LOG_IF(tuneDebug,"tuning test---->index> sensor_id=[0x%x],myiso=[%d],ISO_index=[%d],bpc=[%d],udm=[%d],anr=[%d],ee=[%d],obc=[%d],Scene[%d],[eProfile,eSensor]=[%d ,%d],rCamInfo.eIdx_Scene=[%d],zoomration=[%d],ifflash=[%d],tuneDebug=%d\n",
														getSensorID(),MY_ISO,ISO_index,
														MY_BPC,MY_UDM,MY_ANR,MY_EE,MY_OBC,
														rCamInfo.eIdx_Scene,rCamInfo.eIspProfile,rCamInfo.eSensorMode,
														rCamInfo.eIdx_Scene,rCamInfo.i4ZoomRatio_x100,rCamInfo.rFlashInfo.isFlash,tuneDebug);
...
MVOID
IspTuningCustom::
refine_NSL2(RAWIspCamInfo const& rCamInfo, IspNvramRegMgr & rIspRegMgr, ISP_NVRAM_NSL2_T& rNSL2)
{
    #if 0
    MY_LOG("%s()\n", __FUNCTION__);
    // TODO: Add your code below...
...
refine_CCM(RAWIspCamInfo const& rCamInfo, IspNvramRegMgr & rIspRegMgr, ISP_NVRAM_CCM_T& rCCM)
{
	...
	if(rCamInfo.rAEInfo.u4RealISOValue<600 || rCamInfo.rAEInfo.u4RealISOValue > 3400){
		MINT32 chip_sumValue = 512;
		...
		MFLOAT temp_01 = (MFLOAT)digit_01;
		MFLOAT temp_02 = (MFLOAT)digit_02;
		MFLOAT temp_10 = (MFLOAT)digit_10;
		MFLOAT temp_12 = (MFLOAT)digit_12;
		MFLOAT temp_20 = (MFLOAT)digit_20;
		MFLOAT temp_21 = (MFLOAT)digit_21;

		MINT32 final_00 = ((MFLOAT)digit_00-chip_sumValue) + 0.5 + chip_sumValue;
		...
		MINT32 final_11 = ((MFLOAT)digit_11-chip_sumValue) + 0.5 + chip_sumValue;
		...
		MINT32 final_22 = ((MFLOAT)digit_22-chip_sumValue) + 0.5 + chip_sumValue;
...
/*
	    ...
	    /*MY_LOG("1tuningrCCM_final_00 final_01 final_02 = %d %d %d", final_00,final_01,final_02);
	    MY_LOG("1tuningrCCM_final_10 final_11 final_12 = %d %d %d", final_10,final_11,final_12);
	    MY_LOG("1tuningrCCM_final_20 final_21 final_22 = %d %d %d", final_20,final_21,final_22);*/
	}else{
		MINT32 iso_l = 3600;
		MINT32 iso_h = 3800;
		const MINT32 chip_sumValue = 512;
		MFLOAT gain_l = 1;
		MFLOAT gain_h = 1;
		MFLOAT current_gain = 1;
		MINT32 current_iso =rCamInfo.rAEInfo.u4RealISOValue;
		//main:0x30c6 main2:0x2905  sub:0x80c4
		if((getSensorID() == 0x80c4)||(getSensorID() == 0x8044))
		{
			 	if(rCamInfo.eIspProfile==0 && rCamInfo.eSensorMode==1){//preview mode
					iso_l = 1500;
					iso_h = 1800;
					gain_h = 0.82;
					if((0<rCamInfo.rAEInfo.i4LightValue_x10)&&(rCamInfo.rAEInfo.i4LightValue_x10<10))
					{
						gain_h= 0.82-(1-rCamInfo.rAEInfo.i4LightValue_x10/10)*0.1;//
					}
					if(0==rCamInfo.rAEInfo.i4LightValue_x10)
					{
						gain_h= 0.65;//
					}
				}else if(rCamInfo.eIspProfile==2 && rCamInfo.eSensorMode==1){//capture mode
					iso_l = 1500;
					iso_h = 1800;
					gain_h = 0.82;
					if((0<rCamInfo.rAEInfo.i4LightValue_x10)&&(rCamInfo.rAEInfo.i4LightValue_x10<10))
				...
				if((rCamInfo.rFlashInfo.isFlash != 0))
				{
					iso_l = 100;
					iso_h = 1800;
					gain_h = 0.55;
		...
		#if defined (HAVE_SKIN_COLOR_COMPATIBLE)
		else if((getSensorID() == 0x80cb)||(getSensorID() == 0x804b))
		{
			 	if(rCamInfo.eIspProfile==0 && rCamInfo.eSensorMode==1){//preview mode
					iso_l = 1500;
					iso_h = 1800;
					...
					if((0<rCamInfo.rAEInfo.i4LightValue_x10)&&(rCamInfo.rAEInfo.i4LightValue_x10<10))
					{
						gain_h= 0.72-(1-rCamInfo.rAEInfo.i4LightValue_x10/10)*0.1;//
					}
				}else if(rCamInfo.eIspProfile==2 && rCamInfo.eSensorMode==1){//capture mode
					iso_l = 1500;
					iso_h = 1800;
		...
		#endif
		else if(getSensorID() == 0x30c6)
		{
				iso_l = 1200;
				iso_h = 3200;
				gain_h = 0.8;
		     if(rCamInfo.rAEInfo.i4LightValue_x10<=10)
				gain_h= 0.8-(10-rCamInfo.rAEInfo.i4LightValue_x10)*0.01;
		}
		#if defined (HAVE_SKIN_COLOR_COMPATIBLE)
		else if(getSensorID() == 0xb0c6)
		{
				iso_l = 1200;
				iso_h = 3200;
				gain_h = 1;
		     if(rCamInfo.rAEInfo.i4LightValue_x10<=10)
				gain_h= 0.9-(10-rCamInfo.rAEInfo.i4LightValue_x10)*0.01;
		}
		#endif
	    /*
		--------------+---------------------------------------------------+-------------------------------
		...
		*/
		...
		if(current_iso < iso_l)
		{
			...
			if(current_iso > iso_h)
			{
					current_gain = gain_h;
		 			...
		 			current_gain = gain_l+(MFLOAT)((gain_h-gain_l)*(current_iso-iso_l)/(iso_h-iso_l));
		 	}
	    }
		MY_LOG("tuning_ccm>>>Android_P sensor_id=0x%x>>>ISO%d>>%d>>current_gain=%f>>iso_l=%d>iso_h=%d>>>[profile,sensor]=[%d,%d]\n",getSensorID(),
																rCamInfo.rAEInfo.u4RealISOValue,
																rCamInfo.rAEInfo.i4LightValue_x10,
																current_gain,
																iso_l,
																iso_h,
																rCamInfo.eIspProfile,
																rCamInfo.eSensorMode);
		MINT32 digit_00 = Complement2((rCCM.cnv_1.val & 0x1FFF),13);
	    MINT32 digit_01 = Complement2(((rCCM.cnv_1.val & 0x1FFF0000) >> 16),13);
	    MINT32 digit_02 = Complement2((rCCM.cnv_2.val & 0x1FFF),13);
	    MINT32 digit_10 = Complement2((rCCM.cnv_3.val & 0x1FFF),13);
		...
		MINT32 final_00 = ((MFLOAT)digit_00-chip_sumValue) * current_gain + 0.5 + chip_sumValue;
		MINT32 final_01 = (temp_01 > 0) ? temp_01 + 0.5 : temp_01 - 0.5;
		MINT32 final_02 = (temp_02 > 0) ? temp_02 + 0.5 : temp_02 - 0.5;
		MINT32 final_10 = (temp_10 > 0) ? temp_10 + 0.5 : temp_10 - 0.5;
		MINT32 final_11 = ((MFLOAT)digit_11-chip_sumValue) * current_gain + 0.5 + chip_sumValue;
		MINT32 final_12 = (temp_12 > 0) ? temp_12 + 0.5 : temp_12 - 0.5;
		MINT32 final_20 = (temp_20 > 0) ? temp_20 + 0.5 : temp_20 - 0.5;
		MINT32 final_21 = (temp_21 > 0) ? temp_21 + 0.5 : temp_21 - 0.5;
		MINT32 final_22 = ((MFLOAT)digit_22-chip_sumValue) * current_gain + 0.5 + chip_sumValue;
		//> check sum = 512 at each raw
...
/*
		...
		rCCM.cnv_5.val = 0 | (final_20 & 0x1FFF) | ((final_21 & 0x1FFF) << 16);
		rCCM.cnv_6.val = 0 | (final_22 & 0x1FFF);
		/*MY_LOG("2tuningrCCM_final_00 final_01 final_02 = %d %d %d", final_00,final_01,final_02);
	    MY_LOG("2tuningrCCM_final_10 final_11 final_12 = %d %d %d", final_10,final_11,final_12);
	    MY_LOG("2tuningrCCM_final_20 final_21 final_22 = %d %d %d", final_20,final_21,final_22);*/
	}
}
...
refine_NR3D(RAWIspCamInfo const& rCamInfo, IspNvramRegMgr & rIspRegMgr, ISP_NVRAM_NR3D_T& rNR3D)
{
    ...
    property_get("vendor.camera.3dnr.fixlimit", InputValue, "0");
    if (InputValue[0] == '1')   // Use fixed limit
    {
        ...
        property_get("vendor.camera.3dnr.lowiso", InputValue, "0");
        i4TempInputValue = atoi(InputValue);
        if (i4TempInputValue != 0)
        {
            u4RealIsoforThrLow = i4TempInputValue;
        }
        property_get("vendor.camera.3dnr.highiso", InputValue, "0");
        i4TempInputValue = atoi(InputValue);
        if (i4TempInputValue != 0)
        {
    ...
    property_get("vendor.camera.3dnr.tuning", InputValue, "0");
	if (InputValue[0] == '1')
	{
	...
	{
	    property_get("vendor.camera.3dnr.nr3dgain", InputValue, "0");
	...
	{
	    property_get("vendor.camera.3dnr.lmtyratio", InputValue, "0");
		MUINT32 u4LmtYRatio = atoi(InputValue);
	    property_get("vendor.camera.3dnr.lmtcratio", InputValue, "0");
		MUINT32 u4LmtCRatio = atoi(InputValue);
	    property_get("vendor.camera.3dnr.nr3dgain", InputValue, "256");
		MUINT32 u4Nr3dGain = atoi(InputValue);
    ...
    // Print log for debug.
    property_get("vendor.camera.3dnr.tuning.debuglog", InputValue, "0");
	if (InputValue[0] == '1')
	{
...
IspTuningCustom::
evaluate_ANR_TBL_ISO_index(RAWIspCamInfo const& rCamInfo, MBOOL &inter, MUINT32 &UpperIso, MUINT32 &LowerIso)
{
    ...
    MUINT32 const THA = 100;  //Low
    MUINT32 const THB = 400;  //Mid
    MUINT32 const THC = 1600;  //High


    EIndex_ANR_TBL_ISO_T eIdx_ANR_TBL_ISO;

    if (rCamInfo.u4ISOValue < THA){
        inter = MFALSE;
        UpperIso = THA;
        LowerIso = THA;
        eIdx_ANR_TBL_ISO = eIDX_ANR_TBL_ISO_0;
    }
    else if (rCamInfo.u4ISOValue < THB){
        inter = MTRUE;
        UpperIso = THB;
        LowerIso = THA;
        eIdx_ANR_TBL_ISO = eIDX_ANR_TBL_ISO_0;
    }
    else if (rCamInfo.u4ISOValue < THC){
        inter = MTRUE;
        UpperIso = THC;
        LowerIso = THB;
        eIdx_ANR_TBL_ISO = eIDX_ANR_TBL_ISO_1;
    }
    else{
        inter = MFALSE;
        UpperIso = THC;
        LowerIso = THC;
        eIdx_ANR_TBL_ISO = eIDX_ANR_TBL_ISO_2;
    }
...
//                    THA     THB            THC     THD
    ...
    MINT32 const THA = 3000;
    MINT32 const THB = 4500;
    MINT32 const THC = 6000;
    MINT32 const THD = 7500;

    switch  (rCamInfo.eIdx_ANR_TBL_CT)
    {
    case eIDX_ANR_TBL_CCT_3:
        if  ( rCamInfo.rAWBInfo.i4CCT < THA )
        {
            eIdx_ANR_TBL_new = eIDX_ANR_TBL_CCT_0;
        }
        else if ( rCamInfo.rAWBInfo.i4CCT < THB )
        {
            eIdx_ANR_TBL_new = eIDX_ANR_TBL_CCT_1;
        }
        else if ( rCamInfo.rAWBInfo.i4CCT < THC )
        {
            eIdx_ANR_TBL_new = eIDX_ANR_TBL_CCT_2;
        }
        else
        {
            eIdx_ANR_TBL_new = eIDX_ANR_TBL_CCT_3;
        }
        break;

    case eIDX_ANR_TBL_CCT_2:
        if  ( rCamInfo.rAWBInfo.i4CCT > THD )
        {
            eIdx_ANR_TBL_new = eIDX_ANR_TBL_CCT_3;
        }
    ...
    {
        MY_LOG(
            "[-evaluate_ANR_CT_index] ANR_CT_index(old,new)=(%d,%d)"
...
evaluate_ANR_TBL_CT_index(RAWIspCamInfo const& rCamInfo)
{
    ...
    MINT32 const THA = 6000;
    MINT32 const THB = 4317;
    MINT32 const THC = 2000;
...
//                    THA     THB            THC
    ...
    MINT32 const THB = 3200;  //Mid
    MINT32 const THC = 5500;  //High
    MBOOL fgHDR = MFALSE;
...
MVOID*
IspTuningCustom::
get_SmoothPCA_para (ESensorDev_T eSensorDev)
{
    switch (eSensorDev)
    {
    case ESensorDev_Main: //main
        return &gsSmoothPCA_para_main;
        break;
    case ESensorDev_Sub: //sub
        return &gsSmoothPCA_para_sub;
        break;
    case ESensorDev_MainSecond: //main2
        return &gsSmoothPCA_para_main2;
        break;
	case ESensorDev_SubSecond: //sub2
        return &gsSmoothPCA_para_sub2;
        break;
    default:
        return &gsSmoothPCA_para_main;
    }
...
static MINT32 ratioMapping(MINT32 i4Iso,MINT32 sensor_idx)
{
#define LERP(x, lo_x, lo_y, hi_x, hi_y)\
    (((hi_x) - (x))*(lo_y) + ((x) - (lo_x))*(hi_y)) / ((hi_x) - (lo_x))

    static const MINT32 iso[4][10] ={
    {100, 200, 400, 600, 800, 1200, 1600, 2000, 2400, 3200}, //0x30c6
	//{100, 200, 400, 600, 800, 1000, 1200, 1400, 1600, 1800}, //0x30c6
	{100, 200, 400, 600, 800, 1000, 1200, 1400, 1600, 1800}, //0x250a
	{100, 200, 400, 600, 800, 1000, 1200, 1500, 1800, 2000} //0x80c4 0x8044
	};

    static const MINT32 rto[4][10] =
    {
		{28, 26, 26, 26, 26, 26, 26, 20, 14, 10}, //0x30c6
		//{28, 26, 24, 20, 16, 12, 8, 4, 2, 0}, //0x30c6
		{28, 26, 24, 16, 8, 0, 0, 0, 0, 0}, //0x250a
		{24, 20, 16, 12, 8, 4, 0, 0, 0, 0} //0x80c4 0x8044
	}; //sub

    MINT32 i = 0;
    MINT32 i4Rto = 32;

    if (i4Iso < iso[sensor_idx][0])
    {
        i4Rto = rto[sensor_idx][0];
    }
    else if (i4Iso >= iso[sensor_idx][9])
    {
        i4Rto = rto[sensor_idx][9];
    }
    else
    {
        for (i = 1; i < 10; i++)
        {
            if (i4Iso < iso[sensor_idx][i])
                break;
        }
        i4Rto = LERP(i4Iso, iso[sensor_idx][i-1], rto[sensor_idx][i-1], iso[sensor_idx][i], rto[sensor_idx][i]);
    }
    return i4Rto;
}
...
)
{
    ...
    */
	...
	MINT32 sensor_idx = 0;
	//main:0x30c6 main2:0x250a  sub:0x80c4
	if((getSensorID() == 0x30c6) || (getSensorID() == 0xb0c6)){
		sensor_idx = 0;
	}else if(getSensorID() == 0x250a){
		sensor_idx = 1;
	}else if((getSensorID() == 0x80c4)||(getSensorID() == 0x8044)){
		sensor_idx = 2;
	}
    ...
    if (total_RA_num_frames_ < 0){
        avgISO = 8;
        MY_LOG_IF(customRatioLog,"[%s] first avgISO = %d\n", __FUNCTION__, avgISO);
    } else {
        // smooth
        ...
        avgISO /= n_frames;
        MY_LOG_IF(customRatioLog,"[%s] ISO_Buffer_[] = {%d, %d, %d, %d, %d, %d, %d, %d, %d, %d\n}", __FUNCTION__,
        ISO_Buffer_[0], ISO_Buffer_[1], ISO_Buffer_[2], ISO_Buffer_[3], ISO_Buffer_[4],
        ISO_Buffer_[5], ISO_Buffer_[6], ISO_Buffer_[7], ISO_Buffer_[8], ISO_Buffer_[9] );
        MY_LOG_IF(customRatioLog,"[%s] avgISO = %d", __FUNCTION__, avgISO);
        if (rCamInfo.rFlashInfo.isFlash == 2)
        {
            i4Rto = ratioMapping(i4Iso,sensor_idx);
            MY_LOG("[%s] Main flash iso(%d), ratio(%d)", __FUNCTION__, i4Iso, i4Rto);
        }
        else
        {
            i4Rto = ratioMapping(avgISO,sensor_idx);
			MY_LOG("[%s] tuning_rto = %d,avgISO = %d", __FUNCTION__, i4Rto,avgISO);
		}
    }
//modify XWWYSHSJW-80 by jialiang.xu 180306 start
MY_LOG("xujlrto: sensor_id=[0x%x],ISO,LV=[%d,%d];ratio=[%d]\
[eProfile,eSensor]=[%d ,%d],rCamInfo.eIdx_Scene=[%d]\n",getSensorID(),\
rCamInfo.u4ISOValue,rCamInfo.rAEInfo.i4LightValue_x10,i4Rto,\
rCamInfo.eIspProfile,rCamInfo.eSensorMode,rCamInfo.eIdx_Scene);
//modify XWWYSHSJW-80 by jialiang.xu 180306 end
    return i4Rto;
}
