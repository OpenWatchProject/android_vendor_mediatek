...
template <>
MBOOL
_getAFParam<ESensorDev_SubSecond>( AF_PARAM_T const **outAFParam)
{
    ...
    AAA_LOGD( "%s : dev( ESensorDev_SubSecond ) get specialization setting, 0x%x, tbl_size(%d)",
              __FUNCTION__,
              (*outAFParam),
              (*outAFParam)->i4TBLL);
...
template <>
MBOOL
_getAFConfig<ESensorDev_SubSecond>( AF_CONFIG_T const **outAFCfg)
{
    ...
    AAA_LOGD( "%s : dev( ESensorDev_SubSecond ) get specialization setting, 0x%x, AF_EXT_ENABLE(%d)",
              __FUNCTION__,
              (*outAFCfg),
