...
MBOOL IndexMgr::setIdx_PDC(UINT16 const idx)   { return setIdx<NUM_PDC>(PDC, idx); }
...
dump() const
{
    MY_LOG(
        "[IndexMgr][dump]"
        " DBS:%d, OBC:%d, BPC:%d, NR1:%d, UDM:%d, PDC:%d, GGM:%d, ANR:%d, ANR2:%d, CCR:%d, BOK:%d, EE:%d, NR3D:%d, MFB:%d, LCE:%d"
        , DBS,    OBC,    BPC,    NR1,    UDM,    PDC,    GGM,    ANR,    ANR2,    CCR,    BOK,    EE,    NR3D,    MFB,    LCE
    );
}
