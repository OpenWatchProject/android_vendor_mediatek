...
enum
{
    ...
    /*LukeHu++151111=For Main1 of Double EEPRom Main1*/
    CALIBRATION_LAYOUT_STEREO_MAIN1,
    CALIBRATION_LAYOUT_STEREO_MAIN2,
    /*LukeHu++151124=This is a workaround type, Remove if possible.*/
...
const CALIBRATION_LAYOUT_STRUCT CalLayoutTbl[MAX_CALIBRATION_LAYOUT_NUM]=
{
    ...
    {//CALIBRATION_LAYOUT_STEREO_MAIN1_LEGACY: LukeHu++151124=This is a workaround type, Remove if possible.
    ...
    {//CALIBRATION_LAYOUT_STEREO_MAIN2_LEGACY:  LukeHu++151124=This is a workaround type, Remove if possible.
        0x00001001, 0x020b00ff, CAM_CAL_DOUBLE_EEPROM_DATA,
        {
            ...
            {0x00000001, 0x00001017, 0x00000001, DoCamCalSingleLsc}, //CAMERA_CAM_CAL_DATA_SHADING_TABLE
...
UINT32 DoCamCalPartNumber(INT32 CamcamFID, UINT32 start_addr, UINT32 BlockSize, UINT32* pGetSensorCalData)
{
    ...
    CAM_CAL_LOG("[Part Number] = %x %x %x %x\n", pCamCalData->PartNumber[1], pCamCalData->PartNumber[2], pCamCalData->PartNumber[3], pCamCalData->PartNumber[4]);
    CAM_CAL_LOG("[Part Number] = %x %x %x %x\n", pCamCalData->PartNumber[5], pCamCalData->PartNumber[6], pCamCalData->PartNumber[7], pCamCalData->PartNumber[8]);
    CAM_CAL_LOG("======================Part Number==================\n");
#endif
    return err;
...
UINT32 DoCamCal2AGain(INT32 CamcamFID, UINT32 start_addr, UINT32 BlockSize, UINT32* pGetSensorCalData)
{
    ...
    u16 AFInf, AFMacro;
    UINT8 tempMax = 0;
    /*LukeHu+-151121=Fix Code Defects( No Algo, Just help to avoid being divided by 0)*/
    UINT8 CalR, CalGr, CalGb, CalG, CalB, FacR=1, FacGr, FacGb, FacG=1, FacB=1;
...
UINT32 DoCamCalSingleLsc(INT32 CamcamFID, UINT32 start_addr, UINT32 BlockSize, UINT32* pGetSensorCalData)
{
    ...
    else
    {
        ...
        cam_calCfg.sensorID = pCamCalData->sensorID;
        cam_calCfg.deviceID = pCamCalData->deviceID;
        CAM_CAL_LOG_IF(dumpEnable, "u4Offset=%d u4Length=%d pu1Params= %p",
            cam_calCfg.u4Offset, cam_calCfg.u4Length, cam_calCfg.pu1Params);
        ioctlerr= ioctl(CamcamFID, CAM_CALIOC_G_READ, &cam_calCfg);
        if(!ioctlerr)
        {
        ...
        if(table_size==0 && ioctlerr)/*Hard code, Need to be fixed : vendor write wrong lsc size in eeprom.*/
        {
            if(pCamCalData->sensorID == 0x023850 || pCamCalData->sensorID == 0x30D2
            || pCamCalData->sensorID == 0x2188)
                table_size = 1868;
        }
...
UINT32 DoCamCalLayoutCheck(UINT32* pGetSensorCalData)
{
    ...
    PCAM_CAL_DATA_STRUCT pCamCalData = (PCAM_CAL_DATA_STRUCT)pGetSensorCalData;
    stCAM_CAL_INFO_STRUCT  cam_calCfg;
    /*LukeHu+-151121=Fix Coverity Defects(Unintialized value)*/
    UINT32 CheckID = 0, i;
    INT32 err;
    ...
    /*LukeHu+-151125=For different calibration ID*/
    if (/*(gIsInitedCamCal==CAM_CAL_LAYOUT_RTN_QUEUE)*/(g_lastSensorID!=pCamCalData->sensorID)&&
        (CAM_CALInit() != CAM_CAL_NONE) && (CAM_CALDeviceName(&cBuf[0], sizeof(cBuf), pCamCalData->sensorID) == CAM_CAL_SUCCESS))
    {
        ...
        for (i = 0; i< MAX_CALIBRATION_LAYOUT_NUM; i++)
        {
            if (cam_calCfg.u4Offset != CalLayoutTbl[i].HeaderAddr)
            {
                ...
                cam_calCfg.sensorID = pCamCalData->sensorID;
                cam_calCfg.deviceID = pCamCalData->deviceID;
                CAM_CAL_LOG_IF(dumpEnable, "u4Offset=%d u4Length=%d pu1Params= %p sensorID=%x",
                    cam_calCfg.u4Offset, cam_calCfg.u4Length, cam_calCfg.pu1Params, cam_calCfg.sensorID);
            ...
            if(CheckID == CalLayoutTbl[i].HeaderId)
            {
                ...
                if(CalLayoutTbl[i].HeaderId == 0x020b00ff || CalLayoutTbl[i].HeaderId == 0x030b00ff)
                {/*LukeHu++151120=DeviceID Must accordant with IHalSensor.h*/
                    if(pCamCalData->deviceID==0x01 && CalLayoutTbl[i].DataVer!=CAM_CAL_SINGLE_EEPROM_DATA)
                        continue;
                    else if(pCamCalData->deviceID==0x04 && CalLayoutTbl[i].DataVer!=CAM_CAL_DOUBLE_EEPROM_DATA)
                        continue;
                }
                LayoutType = i;
...
//    CAM_CAL_LOG_IF(dumpEnable,"CAM_CALGetCalData(0x%8x)----\n",(unsigned int)pGetSensorCalData);
    ...
    property_get("vendor.camcalcamcal.log", value, "0");
    ...
    if(DoCamCalLayoutCheck(pGetSensorCalData)==CAM_CAL_ERR_NO_ERR)
    {
        ...
        if ((CAM_CALInit() != CAM_CAL_NONE) && (CAM_CALDeviceName(&cBuf[0], sizeof(cBuf), pCamCalData->sensorID) == CAM_CAL_SUCCESS))
        {
    ...
    UINT32* pGetSensorCalData)
{
        ...
        if(BlockSize!=CAM_CAL_PDAF_SIZE)
        {
            CAM_CAL_LOG_IF(dumpEnable, "BlockSize(%d) is not CAM_CAL_PDAF_SIZE=%d\n", BlockSize, CAM_CAL_PDAF_SIZE);
        }

        cam_calCfg.u4Offset = (start_addr-2);
        cam_calCfg.u4Length = CAM_CAL_PDAF_SIZE;
        ...
        CAM_CAL_LOG_IF(dumpEnable,"First five %x, %x, %x, %x, %x \n", pCamCalData->PDAF.Data[0],
