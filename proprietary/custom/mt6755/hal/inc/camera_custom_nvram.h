...
typedef struct
{
    ...
    // [3] enable firstCaf do tracking (for HybridAF only)
    // [4] laser dist change valid range (for LaserAF only)
    // [5] laser inf target setup (for LaserAF only)
    // [6] fd seek retrigger ratio
    // [7] ot size diff trigger
    // [8~10] reserved
    // [11~20] for SDAF
    // [21~30] for PDAF
    // [31]~[40] for HybridAF tracking
    // [41]~[50] for PL detect
    // [51]~[60]
    // [61]~[70] for LaserAF
    // [71]~[85] for LaserAF GoldenTable 15 Level : Laser Dist & Lens DAC , Value = DAC * 10000 + Dist
    // [86]~[87] for LaserAF CalibData Offset , XTalk ; Value = Enable * 10000 + CalibData
} AF_NVRAM_T;
...
#define NVRAM_BPC_TBL_NUM               (350)
#define NVRAM_NR1_TBL_NUM               (330)
