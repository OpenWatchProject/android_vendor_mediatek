...
CAMSV_CUSTOM_BUFFER_INFO getAeBufSize_a5142mipiraw();
CAMSV_CUSTOM_BUFFER_INFO getFlkBufsize_a5142mipiraw();
UINT32 DecodeAe_a5142mipiraw(VOID* const pData);
UINT32 DecodeFlk_a5142mipiraw(VOID* const pData);
...
impCamsvBufSize(CAMSV_CUSTOM_DECODE_ENUM const CamsvCusDecodeFeature) const
{
    ...
    switch(CamsvCusDecodeFeature)
    {
        case CAMSV_CUSTOM_DECODE_AE:
            buf_size = getAeBufSize_a5142mipiraw();
            break;
        case CAMSV_CUSTOM_DECODE_FLK:
            buf_size = getFlkBufsize_a5142mipiraw();
            break;
        default:
...
impCamsvCustomDecode(CAMSV_CUSTOM_DECODE_ENUM const CamsvCusDecodeFeature, VOID* const pData) const
{
    ...
    {
        case CAMSV_CUSTOM_DECODE_AE:
            DecodeAe_a5142mipiraw(pData);
            break;
        case CAMSV_CUSTOM_DECODE_FLK:
            DecodeFlk_a5142mipiraw(pData);
            break;
        default:
