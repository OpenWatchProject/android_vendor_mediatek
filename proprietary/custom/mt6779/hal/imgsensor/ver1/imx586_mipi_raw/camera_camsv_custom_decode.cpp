...
CAMSV_CUSTOM_BUFFER_INFO getAeBufSize_imx586mipiraw();
CAMSV_CUSTOM_BUFFER_INFO getFlkBufsize_imx586mipiraw();
UINT32 DecodeAe_imx586mipiraw(VOID* const pData);
UINT32 DecodeFlk_imx586mipiraw(VOID* const pData);
...
impCamsvBufSize(CAMSV_CUSTOM_DECODE_ENUM const CamsvCusDecodeFeature) const
{
    ...
    switch(CamsvCusDecodeFeature)
    {
        case CAMSV_CUSTOM_DECODE_AE:
            buf_size = getAeBufSize_imx586mipiraw();
            break;
        case CAMSV_CUSTOM_DECODE_FLK:
            buf_size = getFlkBufsize_imx586mipiraw();
            break;
        default:
...
impCamsvCustomDecode(CAMSV_CUSTOM_DECODE_ENUM const CamsvCusDecodeFeature, VOID* const pData) const
{
    ...
    {
        case CAMSV_CUSTOM_DECODE_AE:
            DecodeAe_imx586mipiraw(pData);
            break;
        case CAMSV_CUSTOM_DECODE_FLK:
            DecodeFlk_imx586mipiraw(pData);
            break;
        default:
...
UINT32 DecodeAe_imx586mipiraw(VOID* const pData)
{
    ...
    CAMSV_CUSTOM_BUFFER_INFO buf = getAeBufSize_imx586mipiraw();
    memset(pOutput, 0, buf.size * sizeof(MUINT32));
    MUINT32 u4AEHistTotalSum = 6209280; // Default for IMX586
...
UINT32 DecodeFlk_imx586mipiraw(VOID* const pData)
{
    ...
    CAMSV_CUSTOM_BUFFER_INFO buf = getFlkBufsize_imx586mipiraw();
    memset(pOutput, 0, buf.size * sizeof(MUINT32));
