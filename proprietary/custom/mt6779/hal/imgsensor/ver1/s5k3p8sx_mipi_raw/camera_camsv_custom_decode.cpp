...
CAMSV_CUSTOM_BUFFER_INFO getAeBufSize_s5k3p8sxmipiraw();
CAMSV_CUSTOM_BUFFER_INFO getFlkBufsize_s5k3p8sxmipiraw();
UINT32 DecodeAe_s5k3p8sxmipiraw(VOID* const pData);
UINT32 DecodeFlk_s5k3p8sxmipiraw(VOID* const pData);
...
impCamsvBufSize(CAMSV_CUSTOM_DECODE_ENUM const CamsvCusDecodeFeature) const
{
    ...
    switch(CamsvCusDecodeFeature)
    {
        case CAMSV_CUSTOM_DECODE_AE:
            buf_size = getAeBufSize_s5k3p8sxmipiraw();
            break;
        case CAMSV_CUSTOM_DECODE_FLK:
            buf_size = getFlkBufsize_s5k3p8sxmipiraw();
            break;
        default:
...
impCamsvCustomDecode(CAMSV_CUSTOM_DECODE_ENUM const CamsvCusDecodeFeature, VOID* const pData) const
{
    ...
    {
        case CAMSV_CUSTOM_DECODE_AE:
            DecodeAe_s5k3p8sxmipiraw(pData);
            break;
        case CAMSV_CUSTOM_DECODE_FLK:
            DecodeFlk_s5k3p8sxmipiraw(pData);
            break;
        default:
