...
UINT32 transBinSum_ov8858mipiraw(VOID* const pData);
UINT32 transATR_ov8858mipiraw(VOID* const pData);
...
impAECustomTransform(AE_CUSTOM_TRANSFORM_ENUM const AECusFuncType, VOID* const pData) const
{
    switch(AECusFuncType)
    {
        case AE_CUSTOM_TRANSFORM_BINSUM:
            transBinSum_ov8858mipiraw(pData);
            break;
        case AE_CUSTOM_TRANSFORM_ATR:
            transATR_ov8858mipiraw(pData);
            break;
        default:
