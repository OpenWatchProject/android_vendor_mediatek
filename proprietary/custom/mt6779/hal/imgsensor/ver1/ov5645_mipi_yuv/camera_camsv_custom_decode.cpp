...
//camera_info_header
#include "camera_info_ov5645mipiyuv.h"
...
CAMSV_CUSTOM_BUFFER_INFO getAeBufSize_ov5645mipiyuv();
CAMSV_CUSTOM_BUFFER_INFO getFlkBufsize_ov5645mipiyuv();
UINT32 DecodeAe_ov5645mipiyuv(VOID* const pData);
UINT32 DecodeFlk_ov5645mipiyuv(VOID* const pData);
...
impCamsvBufSize(CAMSV_CUSTOM_DECODE_ENUM const CamsvCusDecodeFeature) const
{
    ...
    switch(CamsvCusDecodeFeature)
    {
        case CAMSV_CUSTOM_DECODE_AE:
            buf_size = getAeBufSize_ov5645mipiyuv();
            break;
        case CAMSV_CUSTOM_DECODE_FLK:
            buf_size = getFlkBufsize_ov5645mipiyuv();
            break;
        default:
...
impCamsvCustomDecode(CAMSV_CUSTOM_DECODE_ENUM const CamsvCusDecodeFeature, VOID* const pData) const
{
    ...
    {
        case CAMSV_CUSTOM_DECODE_AE:
            DecodeAe_ov5645mipiyuv(pData);
            break;
        case CAMSV_CUSTOM_DECODE_FLK:
            DecodeFlk_ov5645mipiyuv(pData);
            break;
        default:
