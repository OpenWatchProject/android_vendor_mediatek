...
UINT32 transBinSum_imx258mipimono(VOID* const pData);
UINT32 transATR_imx258mipimono(VOID* const pData);
...
impAECustomTransform(AE_CUSTOM_TRANSFORM_ENUM const AECusFuncType, VOID* const pData) const
{
    switch(AECusFuncType)
    {
        case AE_CUSTOM_TRANSFORM_BINSUM:
            transBinSum_imx258mipimono(pData);
            break;
        case AE_CUSTOM_TRANSFORM_ATR:
            transATR_imx258mipimono(pData);
            break;
        default:
