...
CAMSV_CUSTOM_BUFFER_INFO getAeBufSize_imx318mipiraw();
CAMSV_CUSTOM_BUFFER_INFO getFlkBufsize_imx318mipiraw();
UINT32 DecodeAe_imx318mipiraw(VOID* const pData);
UINT32 DecodeFlk_imx318mipiraw(VOID* const pData);
...
impCamsvBufSize(CAMSV_CUSTOM_DECODE_ENUM const CamsvCusDecodeFeature) const
{
    ...
    switch(CamsvCusDecodeFeature)
    {
        case CAMSV_CUSTOM_DECODE_AE:
            buf_size = getAeBufSize_imx318mipiraw();
            break;
        case CAMSV_CUSTOM_DECODE_FLK:
            buf_size = getFlkBufsize_imx318mipiraw();
            break;
        default:
...
impCamsvCustomDecode(CAMSV_CUSTOM_DECODE_ENUM const CamsvCusDecodeFeature, VOID* const pData) const
{
    ...
    {
        case CAMSV_CUSTOM_DECODE_AE:
            DecodeAe_imx318mipiraw(pData);
            break;
        case CAMSV_CUSTOM_DECODE_FLK:
            DecodeFlk_imx318mipiraw(pData);
            break;
        default:
