...
CAMSV_CUSTOM_BUFFER_INFO getAeBufSize_s5k3p8spmipiraw();
CAMSV_CUSTOM_BUFFER_INFO getFlkBufsize_s5k3p8spmipiraw();
UINT32 DecodeAe_s5k3p8spmipiraw(VOID* const pData);
UINT32 DecodeFlk_s5k3p8spmipiraw(VOID* const pData);
...
impCamsvBufSize(CAMSV_CUSTOM_DECODE_ENUM const CamsvCusDecodeFeature) const
{
    ...
    switch(CamsvCusDecodeFeature)
    {
        case CAMSV_CUSTOM_DECODE_AE:
            buf_size = getAeBufSize_s5k3p8spmipiraw();
            break;
        case CAMSV_CUSTOM_DECODE_FLK:
            buf_size = getFlkBufsize_s5k3p8spmipiraw();
            break;
        default:
...
impCamsvCustomDecode(CAMSV_CUSTOM_DECODE_ENUM const CamsvCusDecodeFeature, VOID* const pData) const
{
    ...
    {
        case CAMSV_CUSTOM_DECODE_AE:
            DecodeAe_s5k3p8spmipiraw(pData);
            break;
        case CAMSV_CUSTOM_DECODE_FLK:
            DecodeFlk_s5k3p8spmipiraw(pData);
            break;
        default:
