...
UINT32 transBinSum_s5k3p8spmipiraw(VOID* const pData);
UINT32 transATR_s5k3p8spmipiraw(VOID* const pData);
...
impAECustomTransform(AE_CUSTOM_TRANSFORM_ENUM const AECusFuncType, VOID* const pData) const
{
    switch(AECusFuncType)
    {
        case AE_CUSTOM_TRANSFORM_BINSUM:
            transBinSum_s5k3p8spmipiraw(pData);
            break;
        case AE_CUSTOM_TRANSFORM_ATR:
            transATR_s5k3p8spmipiraw(pData);
            break;
        default:
