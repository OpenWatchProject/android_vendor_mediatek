...
CAMSV_CUSTOM_BUFFER_INFO getAeBufSize_ov8856mipiraw();
CAMSV_CUSTOM_BUFFER_INFO getFlkBufsize_ov8856mipiraw();
UINT32 DecodeAe_ov8856mipiraw(VOID* const pData);
UINT32 DecodeFlk_ov8856mipiraw(VOID* const pData);
...
impCamsvBufSize(CAMSV_CUSTOM_DECODE_ENUM const CamsvCusDecodeFeature) const
{
    ...
    switch(CamsvCusDecodeFeature)
    {
        case CAMSV_CUSTOM_DECODE_AE:
            buf_size = getAeBufSize_ov8856mipiraw();
            break;
        case CAMSV_CUSTOM_DECODE_FLK:
            buf_size = getFlkBufsize_ov8856mipiraw();
            break;
        default:
...
impCamsvCustomDecode(CAMSV_CUSTOM_DECODE_ENUM const CamsvCusDecodeFeature, VOID* const pData) const
{
    ...
    {
        case CAMSV_CUSTOM_DECODE_AE:
            DecodeAe_ov8856mipiraw(pData);
            break;
        case CAMSV_CUSTOM_DECODE_FLK:
            DecodeFlk_ov8856mipiraw(pData);
            break;
        default:
