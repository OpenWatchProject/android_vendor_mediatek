...
//camera_info_header
#include "camera_info_ov2281mipiraw.h"
...
CAMSV_CUSTOM_BUFFER_INFO getAeBufSize_ov2281mipiraw();
CAMSV_CUSTOM_BUFFER_INFO getFlkBufsize_ov2281mipiraw();
UINT32 DecodeAe_ov2281mipiraw(VOID* const pData);
UINT32 DecodeFlk_ov2281mipiraw(VOID* const pData);
...
impCamsvBufSize(CAMSV_CUSTOM_DECODE_ENUM const CamsvCusDecodeFeature) const
{
    ...
    switch(CamsvCusDecodeFeature)
    {
        case CAMSV_CUSTOM_DECODE_AE:
            buf_size = getAeBufSize_ov2281mipiraw();
            break;
        case CAMSV_CUSTOM_DECODE_FLK:
            buf_size = getFlkBufsize_ov2281mipiraw();
            break;
        default:
...
impCamsvCustomDecode(CAMSV_CUSTOM_DECODE_ENUM const CamsvCusDecodeFeature, VOID* const pData) const
{
    ...
    {
        case CAMSV_CUSTOM_DECODE_AE:
            DecodeAe_ov2281mipiraw(pData);
            break;
        case CAMSV_CUSTOM_DECODE_FLK:
            DecodeFlk_ov2281mipiraw(pData);
            break;
        default:
