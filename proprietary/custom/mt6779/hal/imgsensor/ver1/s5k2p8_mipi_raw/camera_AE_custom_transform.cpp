...
UINT32 transBinSum_s5k2p8mipiraw(VOID* const pData);
UINT32 transATR_s5k2p8mipiraw(VOID* const pData);
...
impAECustomTransform(AE_CUSTOM_TRANSFORM_ENUM const AECusFuncType, VOID* const pData) const
{
    switch(AECusFuncType)
    {
        case AE_CUSTOM_TRANSFORM_BINSUM:
            transBinSum_s5k2p8mipiraw(pData);
            break;
        case AE_CUSTOM_TRANSFORM_ATR:
            transATR_s5k2p8mipiraw(pData);
            break;
        default:
