...
UINT32 transBinSum_ov2735mipiraw(VOID* const pData);
UINT32 transATR_ov735mipiraw(VOID* const pData);
...
impAECustomTransform(AE_CUSTOM_TRANSFORM_ENUM const AECusFuncType, VOID* const pData) const
{
    switch(AECusFuncType)
    {
        case AE_CUSTOM_TRANSFORM_BINSUM:
            transBinSum_ov2735mipiraw(pData);
            break;
        case AE_CUSTOM_TRANSFORM_ATR:
            transATR_ov2735mipiraw(pData);
            break;
        default:
...
UINT32 transATR_ov2735mipiraw(VOID* const pData)
{
