...
CAMSV_CUSTOM_BUFFER_INFO getAeBufSize_imx258mipiraw();
CAMSV_CUSTOM_BUFFER_INFO getFlkBufsize_imx258mipiraw();
UINT32 DecodeAe_imx258mipiraw(VOID* const pData);
UINT32 DecodeFlk_imx258mipiraw(VOID* const pData);
...
impCamsvBufSize(CAMSV_CUSTOM_DECODE_ENUM const CamsvCusDecodeFeature) const
{
    ...
    switch(CamsvCusDecodeFeature)
    {
        case CAMSV_CUSTOM_DECODE_AE:
            buf_size = getAeBufSize_imx258mipiraw();
            break;
        case CAMSV_CUSTOM_DECODE_FLK:
            buf_size = getFlkBufsize_imx258mipiraw();
            break;
        default:
...
impCamsvCustomDecode(CAMSV_CUSTOM_DECODE_ENUM const CamsvCusDecodeFeature, VOID* const pData) const
{
    ...
    {
        case CAMSV_CUSTOM_DECODE_AE:
            DecodeAe_imx258mipiraw(pData);
            break;
        case CAMSV_CUSTOM_DECODE_FLK:
            DecodeFlk_imx258mipiraw(pData);
            break;
        default:
