...
typedef struct {
    strGreenSuppression GreenSup; // Reduce Green scene brightness
} strColorSuppression;
...
} strFaceBalancedTarget;

typedef struct {
    MBOOL bEnableFBT;
    MBOOL bFloatingPcentDR;
    strFaceBalancedTarget Normal;
    strFaceBalancedTarget Night;
} strFaceBalancedTargetCFG;
...
typedef struct {
    MBOOL   bEnableContrastLink; //Enable contrast link
    strFaceLCELink FDLCELinkParam; //Face LCE link param
    ...
    MUINT32 u4ResContrastLink[46]; //Reserve for other contrast Link param
...
typedef struct {
    ...
    strHdrFDAvgRatioTbl HdrFDAvgRatioTbl;
} strHdrPre2CapComp;
...
typedef struct
{
    ...
    strFaceTarget FdTarget;
    strFaceBalancedTargetCFG FBTParam; // Face balanced target parameter
    strFaceSorting FDSortParam;
    strFaceMotion  FDMotionParam;
    strFaceSizeVar FDSizeVarParam;
    strContrastLink ContrastLinkParam; //For Contrast link control
    strFolivoraEffectPara FolivoraEffectPara ;

    //CCU_HDR_AE_SUPPORT +++
    strMvhdrTargetCFG HdrTargetInfo;
    strMvhdrFDTargetCFG HdrFDTargetInfo;
    strHdrSmoothCFG HdrSmoothCFG;
    str3ExpomVHdrCFG ThreeExpomVHdrCFG;
    strHdrRatioCFG HdrRatioControl;
    strMHDRHSMultiStep HdrHsMultiStep;
    strHdrPre2CapComp HdrPre2CapCompCFG;
    ...
    AE_SMOOTH_V6P0_CFG smooth_cfg_v6p0;

    // Light Sensor
    strLightSensor strLightSensor;

    // Motion aware shutter control
    strAisControl strAisControl;
...
typedef struct
{
    strMvhdrTargetCFG* pHdrTargetInfo;
    strMvhdrFDTargetCFG* pHdrFDTargetInfo;
...
typedef struct {
    MINT32 i4SensorDev;
    std::unordered_map<MUINT32, MVOID*> mapCamsvDataPointer;
    MVOID* pAeOutputDataPointer;
    MVOID* pFlkOutputDataPointer;
    MUINT32 u4MVHDRRatio_x100;
} CAMSV_STAT_T;
#endif
