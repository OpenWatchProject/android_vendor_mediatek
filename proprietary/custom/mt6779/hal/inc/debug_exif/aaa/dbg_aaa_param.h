...
typedef union
{
    struct  {
        ...
        MUINT32 ver;
        MUINT32 reserved[4];
        MUINT32 P1Offset; // to skip isp reg reserved data
        MUINT32 P2Offset; // to skip isp reg reserved data
    };
    MUINT8   Data[MAXIMUM_AAA_DEBUG_COMM_SIZE];
} AAA_DEBUG_ISP_T;
static_assert( sizeof(AAA_DEBUG_COMM_T) == MAXIMUM_AAA_DEBUG_COMM_SIZE, "AAA_DEBUG_COM_T size mismatch");
static_assert( sizeof(AAA_DEBUG_ISP_T)  == MAXIMUM_AAA_DEBUG_COMM_SIZE, "AAA_DEBUG_ISP_T size mismatch");
...
#define AAA_DEBUG_LTM_MODULE_ID         0x6010
#define AAA_DEBUG_HLR_MODULE_ID         0x6011
//
...
typedef struct
{
    ...
    AAA_DEBUG_ISP_T   rISP;
} COMMON_DEBUG_INFO2_T;

#define STRING_DEBUG_INFO_SIZE 0x800
typedef struct
{
    MUINT32 u4Size; // sizeof(STRING_DEBUG_INFO_T)
    MUINT32 u4StrCount;
    MUINT8  u1StrData[STRING_DEBUG_INFO_SIZE];
} STRING_DEBUG_INFO_T;
...
typedef struct
{
    struct Header
    {
        ...
        MUINT32  u4AEDebugDataOffset;
        MUINT32  u4StringDebugInfoOffset;
    ...
    STRING_DEBUG_INFO_T   rStringDebugInfo;
