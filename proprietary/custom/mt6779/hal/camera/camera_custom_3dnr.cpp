...
MBOOL NR3DCustom::isSupportRSC()
{
    ...
    if (::property_get_int32("vendor.debug.3dnr.rsc.enable", 1))
        ret = true;
...
MBOOL NR3DCustom::isEnabledRSC(MUINT32 mask)
{
    ...
    // don't enable RSC in dual-cam, PIP scenario, and high speed mode
    if (isSupportRSC() &&
       ! isNR3DUsageMaskEnable(mask, USAGE_MASK_DUAL_ZOOM) &&
       ! isNR3DUsageMaskEnable(mask, USAGE_MASK_MULTIUSER) &&
       ! isNR3DUsageMaskEnable(mask, USAGE_MASK_HIGHSPEED)
       )
    {
        ret = true;
    }
