...
MBOOL EISCustom::isSupportPreviewEIS()
{
    unsigned int previewOff = ::property_get_int32(EIS_FORCE_PREVIEW_OFF, 0);
    return previewOff ? false : SUPPORT_EIS_PREVIEW;
}
...
MFLOAT EISCustom::getHeightToWidthRatio()
{
    MFLOAT desireHtoWRatio = 0;

#if 0 //Enable it to request RRZO ratio
    MFLOAT tolerance = 1.05;
    desireHtoWRatio = ((MFLOAT)9/16) * ((MFLOAT)EIS_DESIRE_FACTOR/EIS_FSC_FHD_FACTOR) * tolerance;
#endif

    return desireHtoWRatio;
}
