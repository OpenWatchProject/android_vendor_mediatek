...
{
    CCU_AE_NVRAM_T* pAENVRAMData = (CCU_AE_NVRAM_T*)pNVRAMData;
    strAERealSetting* pRealSetting = (strAERealSetting*)pData;
    // to do for shutter priority customization
    ALOGD("[%s +] exposure setting: %d/%d/%d, iso: %d, uBlockNumX: %d", __FUNCTION__, pRealSetting->EvSetting.u4Eposuretime, pRealSetting->EvSetting.u4AfeGain, pRealSetting->EvSetting.u4IspGain, pRealSetting->u4ISO, pAENVRAMData->rAeParam.strAEParasetting.uBlockNumX);
...
VOID transSuperNightShot(CCU_AE_CUST_Super_Night_Param_T* a_AeCustParam, const VOID* const pNVRAMData, VOID* const pData)
{
    CCU_AE_NVRAM_T* pAENVRAMData = (CCU_AE_NVRAM_T*)pNVRAMData;
    strAERealSetting* pRealSetting = (strAERealSetting*)pData;
