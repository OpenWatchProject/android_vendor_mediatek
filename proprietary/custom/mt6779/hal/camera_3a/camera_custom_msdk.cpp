...
MINT64 compareSensorIdAndModuleId(MUINT32 r_u4SensorId, MUINT32 r_u8ModuleId)
{
    ...
    {
        if (pstSensorInitFunc[i].SensorId == 0){
            CAM_MSDK_LOG("[%s] last item in sensorList found.  i(%u)", __func__, i);
            break;
        }
...
CAMSV_CUSTOM_BUFFER_INFO GetCamsvBufSize(CAMSV_CUSTOM_DECODE_ENUM CamsvCusDecodeFeature, MUINT32 SensorId)
{
    CAMSV_CUSTOM_BUFFER_INFO ret = {0};
    if (NULL == pstSensorInitFunc[0].getCamsvBufSize)
    {
        CAM_MSDK_LOG("[%s]: uninit yet", __func__);
        return ret; //not initial pstSensorInitFunc yet
    }
...
//    CAM_MSDK_LOG("[%s] Find sensor id %x", __func__, SensorId);
    MINT64 i=compareSensorIdAndModuleId(SensorId, 0);
    if (i==-1)
    {
        CAM_MSDK_LOG("[%s] No sensor %x in this list", __func__, SensorId);
        return ret; //no match sensorId
    }

    ret = pstSensorInitFunc[i].getCamsvBufSize(CamsvCusDecodeFeature);

    return ret;
}

MINT32 GetCamsvCustomDecode(CAMSV_CUSTOM_DECODE_ENUM CamsvCusDecodeFeature, MUINT32 SensorId, MVOID* pData)
{
    MINT32 ret = -1;
    if (NULL == pstSensorInitFunc[0].getCamsvCustomDecode)
    {
        CAM_MSDK_LOG("[%s]: uninit yet", __func__);
        return ret; //not initial pstSensorInitFunc yet
    }
...
//    CAM_MSDK_LOG("[%s] Find sensor id %x", __func__, SensorId);
    ...
    if(pData)
        ret = pstSensorInitFunc[i].getCamsvCustomDecode(CamsvCusDecodeFeature, pData);
    else
        CAM_MSDK_LOG("[%s] pData is NULL", __func__);

    return ret;
}

MINT32 GetAECustomTransform(AE_CUSTOM_TRANSFORM_ENUM AECusFuncType, MUINT32 SensorId, MVOID* pData)
{
	MINT32 ret = -1;
	if (NULL == pstSensorInitFunc[0].getAECustomTransform)
    {
        CAM_MSDK_LOG("[%s]: uninit yet", __func__);
        return ret; //not initial pstSensorInitFunc yet
    }
...
//    CAM_MSDK_LOG("[%s] Find sensor id %x", __func__, SensorId);
    ...
    if(pData)
        ret = pstSensorInitFunc[i].getAECustomTransform(AECusFuncType, pData);
    else
        CAM_MSDK_LOG("[%s] pData is NULL", __func__);
