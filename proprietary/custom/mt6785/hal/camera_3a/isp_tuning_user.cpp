...
//  EE
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MVOID IspTuningCustom::userSetting_EE(
	...
	MINT32	LOCK_EE_Y1		= rEE.glut_ctrl_1.bits.SEEE_GLUT_Y1;		// 0~1023
	MINT32	LOCK_EE_Y2		= rEE.glut_ctrl_2.bits.SEEE_GLUT_Y2;		// 0~1023
	MINT32	LOCK_EE_Y3		= rEE.glut_ctrl_3.bits.SEEE_GLUT_Y3;		// 0~1023
	MINT32	LOCK_EE_Y4		= rEE.glut_ctrl_4.bits.SEEE_GLUT_Y4;		// 0~1023
	MINT32	LOCK_EE_S5		= rEE.glut_ctrl_5.bits.SEEE_GLUT_S5;		// 0~255
	MINT32	LOCK_EE_CLIP	= rEE.clip_ctrl.bits.SEEE_RESP_CLIP;		// 0~255
	MINT32	LOCK_EE_TH_OV	= rEE.glut_ctrl_6.bits.SEEE_GLUT_TH_OVR;	// 0~255
	MINT32	LOCK_EE_TH_UN	= rEE.glut_ctrl_6.bits.SEEE_GLUT_TH_UND;	// 0~255
	MINT32	LOCK_EE_TH_MN	= rEE.glut_ctrl_6.bits.SEEE_GLUT_TH_MIN;	// 0~255
	MINT32	USM_ED_Y1, USM_ED_Y2, USM_ED_Y3, USM_ED_Y4, USM_CLIP, USM_ED_TH_OVER, USM_ED_TH_UNDER, USM_ED_TH_MIN;
	...
	USM_ED_TH_MIN	 = LIMIT(static_cast<MINT32>(static_cast<MDOUBLE>(LOCK_EE_TH_MN) * (1 + 0.2 * i4Slider) + 0.5), 0, 255);

	// USM_ED_S1
	MINT32	USM_ED_X1 = rEE.glut_ctrl_1.bits.SEEE_GLUT_X1;
	MINT32	USM_ED_S1;
	...
	if(USM_ED_X1 == 0){
		...
		USM_ED_S1 = static_cast<MINT32>(static_cast<MDOUBLE>(USM_ED_Y1) / USM_ED_X1 + 0.5);
	}
	...
	// USM_ED_S2
	MINT32	USM_ED_X2 = rEE.glut_ctrl_2.bits.SEEE_GLUT_X2;
	MINT32	USM_ED_S2;
	...
	if(dY > 0){
		if(dX == 0){
			...
			USM_ED_S2 = static_cast<MINT32>(static_cast<MDOUBLE>(dY) / dX + 0.5);
		}
	}
	else{
		if(dX == 0){
	...
	// USM_ED_S3
	MINT32	USM_ED_X3 = rEE.glut_ctrl_3.bits.SEEE_GLUT_X3;
	MINT32	USM_ED_S3;
	...
	if(dY > 0){
		if(dX == 0){
			...
			USM_ED_S3 = static_cast<MINT32>(static_cast<MDOUBLE>(dY) / dX + 0.5);
		}
	}
	else{
		if(dX == 0){
	...
	// USM_ED_S4
	MINT32	USM_ED_X4 = rEE.glut_ctrl_4.bits.SEEE_GLUT_X4;
	MINT32	USM_ED_S4;
	...
	if(dY > 0){
		if(dX == 0){
			...
			USM_ED_S4 = static_cast<MINT32>(static_cast<MDOUBLE>(dY) / dX + 0.5);
		}
	}
	else{
		if(dX == 0){
	...
	if(LOCK_EE_S5 < 128){
		if(dX == 0){
			...
			USM_ED_S5 = static_cast<MINT32>(static_cast<MDOUBLE>(LOCK_EE_S5) * (1 + 0.2 * i4Slider) + 0.5);
		}
	}
	else{
		if(dX == 0){
			...
			USM_ED_S5 = static_cast<MINT32>(static_cast<MDOUBLE>(LOCK_EE_S5 - 256) * (1 + 0.2 * i4Slider) - 0.5);
		}
	}
	...
	MY_LOG("[userSetting_EE] old\nX1, Y1, S1: %3d, %3d, %3d, 0x%08x\nX2, Y2, S2: %3d, %3d, %3d, 0x%08x\nX3, Y3, S3: %3d, %3d, %3d, 0x%08x\nX4, Y4, S4: %3d, %3d, %3d, 0x%08x\nS5: %3d, 0x%08x\nCLIP: %3d, 0x%08x\nTHO, THU, THM: %3d, %3d, %3d, 0x%08x\n",
		rEE.glut_ctrl_1.bits.SEEE_GLUT_X1,
		rEE.glut_ctrl_1.bits.SEEE_GLUT_Y1,
		rEE.glut_ctrl_1.bits.SEEE_GLUT_S1,
		rEE.glut_ctrl_1.val,
		rEE.glut_ctrl_2.bits.SEEE_GLUT_X2,
		rEE.glut_ctrl_2.bits.SEEE_GLUT_Y2,
		rEE.glut_ctrl_2.bits.SEEE_GLUT_S2,
		rEE.glut_ctrl_2.val,
		rEE.glut_ctrl_3.bits.SEEE_GLUT_X3,
		rEE.glut_ctrl_3.bits.SEEE_GLUT_Y3,
		rEE.glut_ctrl_3.bits.SEEE_GLUT_S3,
		rEE.glut_ctrl_3.val,
		rEE.glut_ctrl_4.bits.SEEE_GLUT_X4,
		rEE.glut_ctrl_4.bits.SEEE_GLUT_Y4,
		rEE.glut_ctrl_4.bits.SEEE_GLUT_S4,
		rEE.glut_ctrl_4.val,
		rEE.glut_ctrl_5.bits.SEEE_GLUT_S5,
		rEE.glut_ctrl_5.val,
		rEE.clip_ctrl.bits.SEEE_RESP_CLIP,
		rEE.clip_ctrl.val,
		rEE.glut_ctrl_6.bits.SEEE_GLUT_TH_OVR,
		rEE.glut_ctrl_6.bits.SEEE_GLUT_TH_UND,
		rEE.glut_ctrl_6.bits.SEEE_GLUT_TH_MIN,
		rEE.glut_ctrl_6.val);

#endif

	// Write back
	rEE.glut_ctrl_1.bits.SEEE_GLUT_Y1		= static_cast<MUINT32>(USM_ED_Y1);
	rEE.glut_ctrl_2.bits.SEEE_GLUT_Y2		= static_cast<MUINT32>(USM_ED_Y2);
	rEE.glut_ctrl_3.bits.SEEE_GLUT_Y3		= static_cast<MUINT32>(USM_ED_Y3);
	rEE.glut_ctrl_4.bits.SEEE_GLUT_Y4		= static_cast<MUINT32>(USM_ED_Y4);
	rEE.clip_ctrl.bits.SEEE_RESP_CLIP		= static_cast<MUINT32>(USM_CLIP);
	rEE.glut_ctrl_6.bits.SEEE_GLUT_TH_OVR	= static_cast<MUINT32>(USM_ED_TH_OVER);
	rEE.glut_ctrl_6.bits.SEEE_GLUT_TH_UND	= static_cast<MUINT32>(USM_ED_TH_UNDER);
	rEE.glut_ctrl_6.bits.SEEE_GLUT_TH_MIN	= static_cast<MUINT32>(USM_ED_TH_MIN);

	rEE.glut_ctrl_1.bits.SEEE_GLUT_S1	= (USM_ED_S1 >= 0) ? static_cast<MUINT32>(USM_ED_S1) : static_cast<MUINT32>(256 + USM_ED_S1);
	rEE.glut_ctrl_2.bits.SEEE_GLUT_S2	= (USM_ED_S2 >= 0) ? static_cast<MUINT32>(USM_ED_S2) : static_cast<MUINT32>(256 + USM_ED_S2);
	rEE.glut_ctrl_3.bits.SEEE_GLUT_S3	= (USM_ED_S3 >= 0) ? static_cast<MUINT32>(USM_ED_S3) : static_cast<MUINT32>(256 + USM_ED_S3);
	rEE.glut_ctrl_4.bits.SEEE_GLUT_S4	= (USM_ED_S4 >= 0) ? static_cast<MUINT32>(USM_ED_S4) : static_cast<MUINT32>(256 + USM_ED_S4);
	rEE.glut_ctrl_5.bits.SEEE_GLUT_S5	= (USM_ED_S5 >= 0) ? static_cast<MUINT32>(USM_ED_S5) : static_cast<MUINT32>(256 + USM_ED_S5);
	...
	MY_LOG("[userSetting_EE] new\nX1, Y1, S1: %3d, %3d, %3d, 0x%08x\nX2, Y2, S2: %3d, %3d, %3d, 0x%08x\nX3, Y3, S3: %3d, %3d, %3d, 0x%08x\nX4, Y4, S4: %3d, %3d, %3d, 0x%08x\nS5: %3d, 0x%08x\nCLIP: %3d, 0x%08x\nTHO, THU, THM: %3d, %3d, %3d, 0x%08x\n",
		...
		rEE.glut_ctrl_06.bits.SEEE_GLUT_TH_MIN,
...
)
{
    ...
    // Write back: SE
    rSE.top_ctrl.bits.SEEE_OUT_EDGE_SEL = SE_EDGE;
    rSE.se_y_specl_ctrl.bits.SEEE_SE_YOUT_QBIT = SE_YOUT_QBIT;
    rSE.se_y_specl_ctrl.bits.SEEE_SE_COUT_QBIT = SE_COUT_QBIT;
    rSE.se_y_specl_ctrl.bits.SEEE_SE_CONST_Y_EN = SE_Y;
    rSE.se_y_specl_ctrl.bits.SEEE_SE_CONST_Y_VAL = SE_Y_CONST;
    rSE.se_edge_ctrl_2.bits.SEEE_SE_OIL_EN = SE_OILEN;
    rSE.se_y_specl_ctrl.bits.SEEE_SE_KNEE_SEL = SE_KNEESEL;
    rSE.se_edge_ctrl_1.bits.SEEE_SE_HORI_EDGE_GAIN_B = SE_EGAIN_HB;
    rSE.se_edge_ctrl_1.bits.SEEE_SE_HORI_EDGE_GAIN_A = SE_EGAIN_HA;
    rSE.se_y_specl_ctrl.bits.SEEE_SE_SPECL_HALF_MODE = SE_SPECIPONLY;
    rSE.se_y_specl_ctrl.bits.SEEE_SE_SPECL_GAIN = SE_SPECIGAIN;
    rSE.se_edge_ctrl_1.bits.SEEE_SE_VERT_EDGE_GAIN_B = SE_EGAIN_VB;
    rSE.se_edge_ctrl_1.bits.SEEE_SE_VERT_EDGE_GAIN_A = SE_EGAIN_VA;
    rSE.se_y_specl_ctrl.bits.SEEE_SE_SPECL_ABS = SE_SPECIABS;
    rSE.se_y_specl_ctrl.bits.SEEE_SE_SPECL_INV = SE_SPECIINV;
    rSE.se_core_ctrl_1.bits.SEEE_SE_CORE_HORI_X0 = SE_COREH;
    rSE.se_core_ctrl_2.bits.SEEE_SE_CORE_VERT_X0 = SE_E_TH1_V;
    rSE.se_edge_ctrl_2.bits.SEEE_SE_BOSS_IN_SEL = SE_EMBOSS1;
    rSE.se_edge_ctrl_2.bits.SEEE_SE_BOSS_GAIN_OFF = SE_EMBOSS2;
#if 0

    MY_LOG("rSE.out_edge_ctrl.bits.SEEE_OUT_EDGE_SEL=%d \n",rSE.edge_ctrl.bits.SEEE_OUT_EDGE_SEL);
    ...
    )
{
    ...
    switch (eIdx_Effect) {
        ...
        case MTK_CONTROL_EFFECT_MODE_F1977 :
            ...
            {
                //rGGM.lut.set[idx]=LomoFilterGGM[0][idx];
                rGGM.lut.set[idx]=LomoFilterGGM[(MUINT32)eIdx_Effect-(MUINT32)MTK_CONTROL_EFFECT_MODE_NASHVILLE+1][idx];
