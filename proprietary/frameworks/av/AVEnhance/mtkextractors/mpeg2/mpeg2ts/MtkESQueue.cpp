...
//#define LOG_NDEBUG 0
#define LOG_TAG "MtkESQueue"
#include <media/stagefright/foundation/ADebug.h>

#include "MtkESQueue.h"
...
sp<ABuffer> ElementaryStreamQueue::dequeueAccessUnitAAC() {
    ...
    syncOff = findAACSyncCode(mBuffer->data(), mBuffer->size());
    if (syncOff > 0) {
        memmove(mBuffer->data(), mBuffer->data() + syncOff,
                mBuffer->size() - syncOff);
        mBuffer->setRange(0, mBuffer->size() - syncOff);
        ALOGD("Resync AAC frame header postion syncoff %d,buffer size %zu", syncOff, mBuffer->size());
    }
    ...
    while (offset < info.mLength) {
        ...
        if (number_of_raw_data_blocks_in_frame != 0) {
            ALOGD("number_of_raw_data_blocks_in_frame %d", number_of_raw_data_blocks_in_frame);
            // To be implemented.
...
//            return NULL;
        ...
        if (mBuffer->size() - 2 > offset) {
            if (!((mBuffer->data()[offset] == 0xFFu) && ((mBuffer->data()[offset+1]& 0xF0u) == 0xF0u))) {
                ALOGD("need resync AAC header");
                break;
            }
        }
    }
...
sp<ABuffer> ElementaryStreamQueue::dequeueAccessUnitH264() {
    ...
    while ((err = getNextNALUnit(&data, &size, &nalStart, &nalSize)) == OK) {
        ...
        if (nalType == 1 || nalType == 5) {
            ...
            if (nalType == 1 && !foundIDR && nalSize > 3) {
                ABitReader br(nalStart + 1, nalSize);
                parseUE(&br);
                unsigned slicetype = parseUE(&br);
                if (slicetype == 2 || slicetype == 4 || slicetype == 7 || slicetype == 9) {
                    ALOGD("slicetype % d", slicetype);
                    foundISlice = true;
                }
            }
            foundSlice = true;
...
//  seek patch --
            ...
            if (foundIDR) {
                ...
                ALOGD("set isIslice");
                accessUnit->meta()->setInt32("isISlice", 1);
            }
...
sp<ABuffer> ElementaryStreamQueue::dequeueAccessUnitHEVC() {
    ...
    while ((err = getNextNALUnit(&data, &size, &nalStart, &nalSize)) == OK) {
        ...
        unsigned nalType = (nalStart[0] & 0x7E) >> 1;
        bool flush = false;
        ...
        if ((nalType >= 0 && nalType <= 9) || (nalType >= 16 && nalType <= 21)) {
            ...
            foundSlice = true;
        } else if ((nalType == 35 || (nalType >= 32 && nalType <= 34)) && foundSlice) {
            // Access unit delimiter and SPS will be associated with the
            // next frame.
        ...
        if (flush) {
            ...
            ALOGV("[%s]flush nalType = %u nals.size():%zu",
                 __FUNCTION__, nalType, nals.size());
            ...
            for (size_t i = 0; i < nals.size(); ++i) {
                const NALPosition &pos = nals.itemAt(i);
                ALOGV("[hevc]pos:%u size:%u", pos.nalOffset, pos.nalSize);

                unsigned nalType = (mBuffer->data()[pos.nalOffset] & 0x7E) >> 1;
            ...
            if (mFormat == NULL) {
                mFormat = new MetaData;
                if (!MakeHEVCCodecSpecificData(*mFormat,
                        accessUnit->data(),
                        accessUnit->size())) {
                    mFormat.clear();
                }
        ...
        totalSize += nalSize;
        ALOGV("nals add nalType:%u, nals.size:%zu, totalSize %zu",
            nalType, nals.size(), totalSize);
    }
    if (err != (status_t)-EAGAIN) {
        ALOGE("Unexpeted err");
    }
...
int32_t ElementaryStreamQueue::findAACSyncCode(const void *data, size_t size) {
    uint32_t i = 0;
    if (size > 2) {
        for (i = 0; i < size -2; i++) {
            if ((((uint8_t *) data)[i] == 0xFFu) && ((((uint8_t *) data)[i+1])& 0xF0u) == 0xF0u)
                return i;
        }
    }
    return -1;
