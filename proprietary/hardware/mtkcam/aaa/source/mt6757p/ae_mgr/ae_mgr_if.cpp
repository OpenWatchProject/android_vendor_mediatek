...
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MRESULT IAeMgr::updateAEInitIndex(MINT32 i4SensorDev,  MINT32 i4LV9Offset)
{
    MINT32 ret_main, ret_sub, ret_main2, ret_sub2;
    ret_main = ret_sub = ret_main2 = ret_sub2 = S_AE_OK;

    if (i4SensorDev & ESensorDev_Main)
        ret_main = AeMgr::getInstance(ESensorDev_Main).updateAEInitIndex(i4LV9Offset);
    if (i4SensorDev & ESensorDev_Sub)
        ret_sub = AeMgr::getInstance(ESensorDev_Sub).updateAEInitIndex(i4LV9Offset);
    if (i4SensorDev & ESensorDev_MainSecond)
        ret_main2 = AeMgr::getInstance(ESensorDev_MainSecond).updateAEInitIndex(i4LV9Offset);
    if (i4SensorDev & ESensorDev_SubSecond)
        ret_sub2 = AeMgr::getInstance(ESensorDev_SubSecond).updateAEInitIndex(i4LV9Offset);
...
MRESULT IAeMgr::updateCaptureParams(MINT32 i4SensorDev, AE_MODE_CFG_T &a_rCaptureInfo)
{
    ...
    if (i4SensorDev & ESensorDev_SubSecond)
        ret_sub2 = AeMgr::getInstance(ESensorDev_SubSecond).updateCaptureParams(a_rCaptureInfo);
...
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MRESULT IAeMgr::prepareCapParams(MINT32 i4SensorDev, MBOOL bRemosaicEn, MBOOL bIsEVChange)
{
    return AeMgr::getInstance(i4SensorDev).prepareCapParams(bRemosaicEn, bIsEVChange);
    /*
    //MINT32 ret_main, ret_sub, ret_main2, ret_sub2, ret_main3;
    //ret_main = ret_sub = ret_main2 = ret_sub2 = ret_main3 =S_AE_OK;
    MINT32 ret_main, ret_sub, ret_main2, ret_sub2;
...
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MRESULT IAeMgr::IsEISRecording(MINT32 i4SensorDev, MBOOL bEISRecording)
{
    if (i4SensorDev & ESensorDev_Main)
        return AeMgr::getInstance(ESensorDev_Main).IsEISRecording(bEISRecording);
    if (i4SensorDev & ESensorDev_Sub)
        return AeMgr::getInstance(ESensorDev_Sub).IsEISRecording(bEISRecording);
    if (i4SensorDev & ESensorDev_MainSecond)
        return AeMgr::getInstance(ESensorDev_MainSecond).IsEISRecording(bEISRecording);
    if (i4SensorDev & ESensorDev_SubSecond)
        return AeMgr::getInstance(ESensorDev_SubSecond).IsEISRecording(bEISRecording);
...
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MRESULT IAeMgr::getStereoDenoiseInfo(MINT32 i4SensorDev, AE_MODE_CFG_T& strStereoDenoiseInfo)
{
    if (i4SensorDev & ESensorDev_Main)
        return AeMgr::getInstance(ESensorDev_Main).getStereoDenoiseInfo(strStereoDenoiseInfo);
    if (i4SensorDev & ESensorDev_Sub)
        return AeMgr::getInstance(ESensorDev_Sub).getStereoDenoiseInfo(strStereoDenoiseInfo);
    if (i4SensorDev & ESensorDev_MainSecond)
        return AeMgr::getInstance(ESensorDev_MainSecond).getStereoDenoiseInfo(strStereoDenoiseInfo);
    if (i4SensorDev & ESensorDev_SubSecond)
        return AeMgr::getInstance(ESensorDev_SubSecond).getStereoDenoiseInfo(strStereoDenoiseInfo);
...
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MRESULT IAeMgr::IsHDRShot(MINT32 i4SensorDev, MBOOL bHDRShot)
{
    if (i4SensorDev & ESensorDev_Main)
        return AeMgr::getInstance(ESensorDev_Main).IsHDRShot(bHDRShot);
    if (i4SensorDev & ESensorDev_Sub)
        return AeMgr::getInstance(ESensorDev_Sub).IsHDRShot(bHDRShot);
    if (i4SensorDev & ESensorDev_MainSecond)
        return AeMgr::getInstance(ESensorDev_MainSecond).IsHDRShot(bHDRShot);
    if (i4SensorDev & ESensorDev_SubSecond)
        return AeMgr::getInstance(ESensorDev_SubSecond).IsHDRShot(bHDRShot);
