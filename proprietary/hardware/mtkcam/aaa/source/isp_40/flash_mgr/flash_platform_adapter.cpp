...
{
    ISP_MGR_AWB_STAT_CONFIG_T::getInstance((ESensorDev_T)mSensorDev).setIspAWBPreGain1(*awbPreGain1Prev, false);
    return 0;
}
...
{
    ISP_MGR_AWB_STAT_CONFIG_T::getInstance((ESensorDev_T)mSensorDev).setIspAWBPreGain1(*awbPreGain1Curr, true);
    return 0;
}
...
{
    AeMgr::getInstance(mSensorDev).updatePreviewParams(*aeParam, bestInd);
    return 0;
}
