...
{
    memset(&m_bFlickerState, 0, sizeof(m_bFlickerState));
    memset(&m_u4SensorFrameRate, 0, sizeof(m_u4SensorFrameRate));
    memset(&m_pSensorModeDelay, 0, 11*sizeof(MINT32));
    memset(&m_i4FrameSyncFps, 0, sizeof(m_i4FrameSyncFps));
    memset(&m_bEnableBuffMode, 0, sizeof(m_bEnableBuffMode));
}
...
setSensorExpTime(MINT32 i4SensorDev, MUINT32 a_u4ExpTime)
{
    ...
    if(i4SensorDev >0) {
        if (m_i4FrameSyncFps[NS3Av3::mapSensorDevToIdx(i4SensorDev)] == 0){
            ret = m_pHalSensorObj->sendCommand(i4SensorDev, SENSOR_CMD_SET_SENSOR_EXP_TIME, (MUINTPTR)&a_u4ExpTime, 0, 0);
        } else {
            // set exposure time and fps
            ret = setStereoFrameSyncExpAndFps(i4SensorDev, a_u4ExpTime ,m_i4FrameSyncFps[NS3Av3::mapSensorDevToIdx(i4SensorDev)]);
        }
    } else{
        CAM_LOGE("[%s()] Sensor type error:%d", __FUNCTION__, i4SensorDev);
        return E_AAA_SENSOR_NULL;
...
setSensorFrameRate(MINT32 i4SensorDev, MUINT32 a_u4SensorFrameRate)
{
    ...
    // Current Frame Rate
    if(i4SensorDev >=0) {
        u4SensorFrameRateCur = m_u4SensorFrameRate[NS3Av3::mapSensorDevToIdx(i4SensorDev)];
    } else{
        CAM_LOGE("[%s()] Sensor type error:%d", __FUNCTION__, i4SensorDev);
        return E_AAA_SENSOR_NULL;
    }
    ...
    if(u4SensorFrameRate == u4SensorFrameRateCur){
        ...
        m_u4SensorFrameRate[NS3Av3::mapSensorDevToIdx(i4SensorDev)] = u4SensorFrameRate;
    }
...
setFlickerFrameRateActive(MINT32 i4SensorDev, MBOOL a_bFlickerFPSAvtive)
{
    ...
    if(i4SensorDev > 0) {
        i4SensorIdx = NS3Av3::mapSensorDevToIdx(i4SensorDev);
        CAM_LOGD_IF(m_bDebugEnable,"sensorDev(%d) a_bFlickerFPSAvtive(%d) m_bFlickerState(%d)\n", i4SensorDev, a_bFlickerFPSAvtive, m_bFlickerState[i4SensorIdx]);
        ret = m_pHalSensorObj->sendCommand(i4SensorDev, SENSOR_CMD_SET_FLICKER_FRAME_RATE, (MUINTPTR)&u4FlickerInfo, 0, 0);
        if(a_bFlickerFPSAvtive != m_bFlickerState[i4SensorIdx])
        m_bFlickerState[i4SensorIdx] = a_bFlickerFPSAvtive;
    } else{
            CAM_LOGE("[%s()] Sensor type error:%d", __FUNCTION__, i4SensorDev);
            return E_AAA_SENSOR_NULL;
        }
...
setPreviewParams(MINT32 i4SensorDev, MUINT32 a_u4ExpTime, MUINT32 a_u4SensorGain)
{
    ...
    if(i4SensorDev > 0) {
        m_u4SensorFrameRate[NS3Av3::mapSensorDevToIdx(i4SensorDev)] = 0xFFFF;
    } else {
        CAM_LOGE("[%s()] Sensor type error:%d", __FUNCTION__, i4SensorDev);
        return E_AAA_SENSOR_NULL;
    }
...
MRESULT AAASensorMgr::setPreviewMaxFrameRate(MINT32 i4SensorDev, MUINT32 frameRate, MUINT32 u4SensorMode)
{
    ...
    // Set max frame rate
        ...
        m_u4SensorFrameRate[NS3Av3::mapSensorDevToIdx(i4SensorDev)] = frameRate;
    }
...
updateStereoFrameSyncFps(MINT32 i4SensorDev, MINT32 i4SensorDev2, MUINT32 i4ExpTimeMain,MUINT32 i4ExpTimeMain2)
{
    ...
    if(ret < 0){
        ...
        if(i4SensorDev >0) {
            m_i4FrameSyncFps[NS3Av3::mapSensorDevToIdx(i4SensorDev)] = frmTime[0];
            if(i4SensorDev2 >0)
                m_i4FrameSyncFps[NS3Av3::mapSensorDevToIdx(i4SensorDev2)] = frmTime[1];
            else{
    ...
    CAM_LOGD("[updateStereoFrameSyncFps] i4SensorDev/Dev2:%d/%d exp:%d/%d (master/slave) , Fps:%d/%d\n",
        i4SensorDev,i4SensorDev2, i4ExpTimeMain,i4ExpTimeMain2,m_i4FrameSyncFps[NS3Av3::mapSensorDevToIdx(i4SensorDev)],m_i4FrameSyncFps[NS3Av3::mapSensorDevToIdx(i4SensorDev2)] );
...
enableSensorI2CBuffMode(MINT32 i4SensorDev, MBOOL bEnableBuffMode)
{
    ...
    if(i4SensorDev>0) {
        ...
        CAM_LOGD("[enableSensorI2CBuffMode] SensorDev:%d EnableBuffMode:%d -> %d\n", i4SensorDev, m_bEnableBuffMode[NS3Av3::mapSensorDevToIdx(i4SensorDev)], bEnableBuffMode);
        m_bEnableBuffMode[NS3Av3::mapSensorDevToIdx(i4SensorDev)] = bEnableBuffMode;
    } else{
        CAM_LOGE("[%s()] Sensor type error:%d", __FUNCTION__, i4SensorDev);
        return E_AAA_SENSOR_NULL;
    }
