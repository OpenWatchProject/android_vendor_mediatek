...
MRESULT AeMgr::sendAECtrl(EAECtrl_T eAECtrl, MINTPTR iArg1, MINTPTR iArg2, MINTPTR iArg3, MINTPTR iArg4)
{
    ...
    switch (eAECtrl)
    {
        ...
        case EAECtrl_GetAELock:
            *(reinterpret_cast<MBOOL*>(iArg1)) = m_bAPAELock;
            break;
      ...
      case EAECtrl_EnableShutterISOPriority: // iArg1: enable; iArg2: specified shutter; iArg3: specified ISO
            ...
            break;
      case EAECtrl_EnableSuperNightShot:
            {
                // iArg1: enable; iArg2: AeCustParam
                ...
                MBOOL bModeChange = (m_bSuperNightShotMode == (MBOOL)iArg1) ? MFALSE : MTRUE;
                m_bSuperNightShotMode = (MBOOL)iArg1;
                if (m_bSuperNightShotMode && bModeChange) // 0->1
                {
                    m_u4SuperNightShotFrameIndex = 1;
                }
                if (m_bSuperNightShotMode || bModeChange) // 1->1 and 0->1 and 1->0
                {
                    if (reinterpret_cast<CCU_AE_CUST_Super_Night_Param_T*>(iArg2) != NULL)
                    {
                    ...
                    if (m_bIsCCUStart)
                    {
                        ...
                        if (m_u1IsDummyFrameSet)
                        {
                            m_u4SuperNightShotFrameIndex--;
                        }
                        super_night_mode_info.ae_cust_param.u4FrameIndex = m_u4SuperNightShotFrameIndex;
                        m_pICcuAe->ccuControl(MSG_TO_CCU_SET_SUPER_NIGHT_MODE, &super_night_mode_info, NULL);
                        CAM_LOGD("[%s():EAECtrl_EnableSuperNightShot] MSG_TO_CCU_SET_SUPER_NIGHT_MODE, enable:%d FrameIndex:%d IsDummyFrameSet:%d\n", __FUNCTION__,
                            super_night_mode_info.enable, super_night_mode_info.ae_cust_param.u4FrameIndex, m_u1IsDummyFrameSet);
                        m_u4SuperNightShotFrameIndex++;
                    }
                }
                break;
            }
        case EAECtrl_SetReconfig:
            AE_LOG("[%s:EAECtrl_SetReconfig] i4SensorDev: %d, width/height: %d/%d\n", __FUNCTION__, m_eSensorDev, (MUINT32)iArg1, (MUINT32)iArg2);
            setSensorSize((MUINT32)iArg1, (MUINT32)iArg2);
            reconfig();
            setSkipCalCnt(2); // to skip 2-frame AE calculation
            break;
    }
    return i4Ret;
}
...
MRESULT AeMgr::setAEParams(AE_PARAM_SET_INFO const &rNewParam)
{
    ...
    AE_LOG( "[%s()] SensorDev:%d (Min/Max)Fps:%d/%d AeMeterMode:%d RotateDegree:%d IsoSpeedMode:%d rMeteringAreas Cnt:%d Left:%d Top:%d Right:%d Bottom:%d Weight:%d bIsAELock:%d i4ExpIndex:%d fExpCompStep:%f (Ae/Denoise/AntiBanding/Cam/Shot/Scene)Mode:%d/%d/%d/%d/%d/%d bBlackLvlLock:%d Zoom(X/Y)Offset:%d/%d Zoom(Width/Height/Ratio):%d/%d/%d u1HdrMode:%d IsDummyFrame:%d i4ManualPlineIdx:%d i4CustomPlineMode:%d\n",
              ...
              rNewParam.u4ZoomWidth, rNewParam.u4ZoomHeight, rNewParam.i4ZoomRatio, rNewParam.u1HdrMode, rNewParam.u1IsDummyFrame, rNewParam.i4ManualPlineIdx, rNewParam.i4CustomPlineMode);

    i4Ret = setDummyFrameInfo(rNewParam.u1IsDummyFrame)
          | setAEMinMaxFrameRate(rNewParam.i4MinFps, rNewParam.i4MaxFps)
          ...
          | setAEPlineIdx(rNewParam.i4ManualPlineIdx, rNewParam.i4CustomPlineMode)
