...
#define _NORMALPIPE_MAP_QRY_PXLMODE(Sen_Pixmode) ({\
    Normalpipe_PIXMODE _qPxlMode = _UNKNOWN_PIX_MODE;\
    switch (Sen_Pixmode) {\
    case ONE_PIXEL_MODE: _qPxlMode = _1_PIX_MODE; break;\
    case TWO_PIXEL_MODE: _qPxlMode = _2_PIX_MODE; break;\
    case FOUR_PIXEL_MODE: _qPxlMode = _4_PIX_MODE; break;\
    default: PIPE_ERR("Unknown tg pixel mode mapping"); break;\
    }\
    _qPxlMode;\
})

inline void _SET_SOF_IRQ(Irq_t& irq, MINT32 irqUserKey, MUINT32 timeout)
{
    irq.Type = Irq_t::_CLEAR_WAIT;
    irq.Status = Irq_t::_SOF_;
    irq.StatusType = Irq_t::_SIGNAL_INT_;
    irq.UserKey = irqUserKey;
    irq.Timeout = timeout;
}
    ...
    if (mTotalUserCnt == 0)  {
        ...
        if (mpIMem) {
            if (mImgHeaderPool.virtAddr) {
                ...
                if (mpIMem->freeVirtBuf(&mImgHeaderPool, MFALSE)) {
            ...
            if (mvSttMem.size()) {
                ...
                for (i = 0; i < mvSttMem.size(); i++) {
                    ...
                    if (mpIMem->freeVirtBuf(&(mvSttMem.at(i)), MFALSE)) {
        ...
        PIPE_INF("Skip destory NPipe[%d] - users(%d)", idx, mTotalUserCnt);
    }
    ...
    , mpIMem(NULL)
{
    ...
    memset((void*)mImgHeaderSize, 0, sizeof(mImgHeaderSize));
...
MBOOL NormalPipe::FSM_CHECK(E_FSM op, const char *callee)
{
    ...
    if (ret == MFALSE) {
        ...
        PIPE_INF("--mTotalUserCnt=%d--", mTotalUserCnt);
        for (MUINT32 i = 0; i < IOPIPE_MAX_NUM_USERS; i++) {
            PIPE_INF("idx=%d mUserName[%s] mUserCnt[%d]",
                i, mUserName[i], mUserCnt[i]);
        }
    }
...
MBOOL NormalPipe::init(MBOOL EnableSec)
{
    ...
    PIPE_INF("+ SenIdx=%d, EnableSec = %d", mpSensorIdx, EnableSec);
...
MBOOL NormalPipe::enque(QBufInfo const& rQBuf)
{
    ...
    if (this->m_FSM == op_cfg) {
        MUINT32 minDepth = 0xFFFFFFFF, _size = 0;

        for (i = 0; i < mOpenedPortNum; i++) {
            _port = _PortMap(PortIndex[i]);
    ...
    for (_port = 0; _port < MaxPortIdx; _port++) {
        ...
        for (i = 0; i < vPortQBufSlot[_port].mvOut.size(); i++) {
            ...
            if (mpSecMgr && (vPortQBufSlot[_port].mvOut.at(i).mSecon)) {
                MUINT32 curLeng = sprintf(_tmp,"dma:0x%x sec pa(0x%" PRIXPTR ")io(%d_%d_%d_%d_%d_%d)M:x%x,isUF(%d),RawType(%d)",
                ...
                MUINT32 curLeng = sprintf(_tmp,"dma:0x%x pa(0x%" PRIXPTR ")io(%d_%d_%d_%d_%d_%d) mag(0x%x)|",
    ...
    for (i = 0; i < _size; i++) {
        ...
        case EPortIndex_CAMSV2_IMGO: //TBD
            ...
            _PixMode = (NSCam::SENSOR_TYPE_YUV == _NORMALPIPE_GET_SENSORTYPE(mpSensorIdx))? _2_PIX_MODE:
                        _1_PIX_MODE;
            _ImgFmt = (EImageFormat)PortImgFormat[_PortMap(_portIdx)];
...
{
    MUINT32     _port = 0, i = 0, j = 0, _sof_idx = 0, _size = 0, PLANE_ID = 0;
    QBufInfo    _popQBuf;
...
MBOOL NormalPipe::deque(QPortID& rQPort, QBufInfo& rQBuf, MUINT32 u4TimeoutMs)
{
    ...
    //
    {
        if (u4TimeoutMs == 0xFFFFFFFF) {
            do {
                waitStat = ::sem_wait(&mpNPipeThread->m_semDeqDone);
            ...
            if (clock_gettime(CLOCK_REALTIME, &ts) == -1) {
                ...
                do {
                    waitStat = sem_timedwait(&mpNPipeThread->m_semDeqDone, &ts);
                    if ((waitStat == -1) && (errno == EINTR)) {
    ...
    for (ii = 0 ; ii < rQPort.mvPortId.size() ; ii++) {
        ...
        if (_qBuf.mvOut.size()) {
            sprintf(_tmp, "dma:0x%x pa(0x%" PRIXPTR ") io(%d_%d_%d_%d_%d_%d_%d_%d_%d_%d) mag(0x%x)|",
                portID.index,
                _qBuf.mvOut.at(0).mPa,
                _qBuf.mvOut.at(0).mMetaData.mCrop_s.p.x,
                _qBuf.mvOut.at(0).mMetaData.mCrop_s.p.y,
                _qBuf.mvOut.at(0).mMetaData.mCrop_s.s.w,
                _qBuf.mvOut.at(0).mMetaData.mCrop_s.s.h,
                _qBuf.mvOut.at(0).mMetaData.mCrop_d.p.x,
                _qBuf.mvOut.at(0).mMetaData.mCrop_d.p.y,
                _qBuf.mvOut.at(0).mMetaData.mCrop_d.s.w,
                _qBuf.mvOut.at(0).mMetaData.mCrop_d.s.h,
                _qBuf.mvOut.at(0).mMetaData.mDstSize.w,
                _qBuf.mvOut.at(0).mMetaData.mDstSize.h,
                _qBuf.mvOut.at(0).mMetaData.mMagicNum_tuning);
            strncat(deqInfo, _tmp, (sizeof(deqInfo) - strlen(deqInfo)) - 1);
        }
    }
    PIPE_INF("MyDeq:%d: %s", mpSensorIdx, deqInfo);
...
MBOOL NormalPipe::configPipe(QInitParam const& vInPorts, MINT32 burstQnum)
{
    ...
    EImageFormat    tgi_eImgFmt[IOPIPE_MAX_SENSOR_CNT]; //Temp Used
    CAMIO_Func      _func;
    _func.Raw = 0;
...
//        return MFALSE;
    ...
    if (!mpCamIOPipe) {
        ...
        MUINT32     tg = 0, _path = 0, imgHdrBufNum = 0, Sen_PixMode = 0;
        MUINTPTR    imgHdrAllocOfst = 0;
        NormalPipe_InputInfo    qryInput;
        NormalPipe_QueryInfo    queryRst;
        CAM_CAPIBILITY*         pinfo = NULL;
        E_INPUT                 TargetTG = (_NORMALPIPE_GET_TG_IDX(mpSensorIdx) == CAM_TG_1) ? TG_A : TG_B;

        Mutex::Autolock lock(NPipeCfgGLock); // for protect twin mode check

        //Secure cam support
        if (vInPorts.m_SecureCam.buf_type == E_SEC_LEVEL_SECURE ||
            vInPorts.m_SecureCam.buf_type == E_SEC_LEVEL_PROTECTED) {
            ...
            if (MFALSE == this->mpSecMgr->SecMgr_attach(
                                            (MUINT32)TargetTG,
                                            pAllSensorInfo->mSenInfo[mpSensorIdx].mDevId,
                                            sec_port,
                                            burstQnum,
                                            (MUINT32)vInPorts.m_SecureCam.buf_type,
                                            (MUINT64)vInPorts.m_SecureCam.chk_handle)) {
                PIPE_ERR("Secure CAM attach failure");
                return MFALSE;
            }

            MUINT32 _shuttertime_us = 0;
            mpHalSensor->sendCommand(pAllSensorInfo->mSenInfo[SIdx].mDevId, SENSOR_CMD_SET_STREAMING_RESUME, (MUINTPTR)&_shuttertime_us, 0, 0);
        }

        mpCamIOPipe = ICamIOPipe::createInstance((MINT8*)mpName,
                (_NORMALPIPE_GET_TG_IDX(mpSensorIdx) == CAM_TG_1)? TG_A : TG_B, NSImageio::NSIspio::ICamIOPipe::CAMIO);
        if (!mpCamIOPipe) {
        ...
        mpHalSensor->sendCommand(pAllSensorInfo->mSenInfo[mpSensorIdx].mDevId, SENSOR_CMD_GET_MIPI_PIXEL_RATE,
                                    (MUINTPTR)&m_sensorConfigParam.scenarioId, (MUINTPTR)&tgi.MIPI_PixRate, (MUINTPTR)&tgi.tgFps);
        mpHalSensor->sendCommand(pAllSensorInfo->mSenInfo[mpSensorIdx].mDevId, SENSOR_CMD_GET_SENSOR_PIXELMODE,
                                    (MUINTPTR)&m_sensorConfigParam.scenarioId, (MUINTPTR)&tgi.MIPI_PixRate, (MUINTPTR)&Sen_PixMode);
        ...
        PIPE_INF("N:%d TG=%d Scen=%d tgFmt=x%x devID=%d eRawPxlID=%d PixelMode=%d W/H=[%d,%d] FPSx10=%d Clk=%dkhz mipiPixRate=%d burst=%d",
        ...
        for (i = 0; i < vInPorts.mPortInfo.size(); i++) {
            ...
            if (pOut->index == EPortIndex_RRZO) {
                ...
                qryInput.pixelMode  = _NORMALPIPE_MAP_QRY_PXLMODE(Sen_PixMode);
            ...
            if (pOut->index == EPortIndex_IMGO) {
                ...
                qryInput.pixelMode  = (NSCam::SENSOR_TYPE_YUV == _NORMALPIPE_GET_SENSORTYPE(mpSensorIdx))
                                        ? _2_PIX_MODE : _NORMALPIPE_MAP_QRY_PXLMODE(Sen_PixMode);
            ...
            if ((pOut->index == EPortIndex_RRZO) || (pOut->index == EPortIndex_IMGO)) {
                ...
                #if 0  //For legacy PMQoS BW
                dmao_bw += pOut->u4Stride[ESTRIDE_1ST_PLANE] * pOut ->u4ImgHeight;
                #endif
            }
            ...
            PortImgFormat[_PortMap(pOut->index)] = pOut->eImgFmt;
            ...
            if((pOut->eImgFmt == eImgFmt_UFEO_BAYER10)||(pOut->eImgFmt == eImgFmt_UFEO_BAYER12)){
                ...
                PortImgFormat[_PortMap(ufeo.index)] = ufeo.eImgFmt;
        ...
        // check port info
        mOpenedPortNum = GetOpenedPortInfo();
        if(mOpenedPortNum > MaxPortIdx || mOpenedPortNum <= 0){
            PIPE_ERR("dmao config error, opened port num(%d)", mOpenedPortNum);
        }

        if (pinfo->GetCapibility(pOut->index, ENPipeQueryCmd_UNI_NUM, qryInput, queryRst) == MFALSE) {
            PIPE_ERR(" fmt(0x%x) | dma(0x%x) err", pOut->eImgFmt, pOut->index);
            ret = MFALSE;
        }
        ...
        case CAM_TG_2:
            if (vInPorts.m_UniLinkSel == E_UNI_LINK_AUTO) {
                if (queryRst.uni_num == 1) {
                    if (!(mEnablePath & (NPIPE_CAM_A|NPIPE_CAM_B))) {
                        _LinkToUni = MTRUE;
                    }
                    else if (_NORMALPIPE_GET_TG_IDX(mpSensorIdx) == mUniLinkTG) {
                        _LinkToUni = MTRUE;
                    }
                }
                else {
        ...
        //cfg Dynamic twin mode (only mt6763/mt6758 support this Scenario-based ON/OFF)
        {
        ...
        //config for dual cam
        _func.Bits.force_clk_level = vInPorts.m_bN3D;
        ...
        for (MUINT32 headersize = 0,i = 0; i < vInPorts.mPortInfo.size(); i++) {
            ...
            mImgHeaderSize[_PortMap(vInPorts.mPortInfo.at(i).mPortID.index)] = headersize;
        }
        ...
        if (mpIMem->mapPhyAddr(&mImgHeaderPool, MFALSE) < 0) {
        ...
        for (MUINT32 headersize = 0,i = 0; i < vInPorts.mPortInfo.size(); i++) {
            ...
            headersize = mImgHeaderSize[_PortMap(vInPorts.mPortInfo.at(i).mPortID.index)];
            ...
            for (j = 0; j < imgHdrBufNum; j++) {
                ...
                if (vInPorts.mPortInfo.at(i).mSecon &&
                    (vInPorts.m_SecureCam.buf_type == E_SEC_LEVEL_SECURE ||
                     vInPorts.m_SecureCam.buf_type == E_SEC_LEVEL_PROTECTED)) {
                    SecMgr_SecInfo secinfo;
                    secinfo.type = SECMEM_FRAME_HEADER;
                    secinfo.module = (MUINT32)TargetTG;
                    secinfo.buff_size = iHeader.size;
                    secinfo.buff_va = (uint64_t)iHeader.virtAddr;
                    secinfo.port = vInPorts.mPortInfo.at(i).mPortID.index;
                    secinfo.memID = iHeader.memID;
                    PIPE_INF("Register FH - size:0x%x VA:0x%llx PA:0x%x memID:0x%x",
                    iHeader.size,iHeader.virtAddr,iHeader.phyAddr,iHeader.memID);
                    if (this->mpSecMgr->SecMgr_P1RegisterShareMem(&secinfo) == MFALSE) {
                        PIPE_ERR("Register FH ShareMem failed(0x%x_0x%llx)",
                                    vInPorts.mPortInfo.at(i).mPortID.index, iHeader.virtAddr);
                        return MFALSE;
                    }
                }
            }
...
MBOOL NormalPipe::suspend(void)
{
...
#if 0
    for (i = 0; i < mOpenedPortNum; i++) {
        ::sem_post(&this->mpNPipeThread->m_semDeqDonePort[_PortMap(PortIndex[i])]);
    }
#endif
    ::sem_post(&this->mpNPipeThread->m_semDeqDone);
    ...
    for (p = 0 ; p < mOpenedPortNum ; p++) {
        port = _PortMap(PortIndex[p]);
 ...
 *
 ******************************************************************************/
inline MVOID _getSensorSize (MUINT32 sensorIdx, MUINT32 SenScenario, MUINT32 &size_w, MUINT32 &size_h)
{
#define scenario_case(scenario, KEY, pSize)       \
        case scenario:                            \
            size_w = NormalPipe::pAllSensorInfo->mSenInfo[sensorIdx].mSInfo.KEY##Width;  \
            size_h = NormalPipe::pAllSensorInfo->mSenInfo[sensorIdx].mSInfo.KEY##Height; \
            break;
    ...
    switch(SenScenario)
    {
        ...
        default:
            PIPE_ERR("not support sensor scenario(0x%x)\n", SenScenario);
            break;
    }
#undef scenario_case
}

MBOOL NormalPipe::get_isExecutable(NormalPipe_InputInfo const ResDataIn, MBOOL &bIsExe)
{

    MBOOL ret = MTRUE;
    MUINT32 i = 0, EfuseTGNum = 0, NextTG = 0, MipiPixRate = 0;

    PIPE_INF("+ SenId(%d)", mpSensorIdx);

    for (i = 0; i < (MUINT32)EPIPE_Sensor_RSVD; i++) {
        if (i == mpSensorIdx) {
            continue;
        }
        if (NormalPipe::pNormalPipe[i]) {
            if (MFALSE == NormalPipe::pNormalPipe[i]->FSM_CHECK(op_qry, mpName)){
                PIPE_ERR("previous Normalpipe[%d] is not config yet", i);
                ret = MFALSE;
                goto EXIT;
            }
        }
    ...
    if (getEfuseInfo(EfuseTGNum) == MFALSE) {
        PIPE_ERR("getEfuseInfo fail");
        ret = MFALSE;
        goto EXIT;
    }

    if (!mpHalSensor) {
        mpHalSensor = pAllSensorInfo->mSList->createSensor(mpName, 1, &mpSensorIdx);
    ...
    for (i = 0; i < (MUINT32)IOPIPE_MAX_SENSOR_CNT; i++) {
        MUINT32 PowerOnStatus = 0;
        mpHalSensor->sendCommand(pAllSensorInfo->mSenInfo[i].mDevId,
            NSCam::SENSOR_CMD_GET_SENSOR_POWER_ON_STETE, (MUINTPTR)&PowerOnStatus, 0, 0);
        if (PowerOnStatus) {
            NextTG++;
        }
    }

    if (EfuseTGNum > (MUINT32)NextTG) {
        ICamIOPipe* pCamIOPipe = NULL;
        MUINT32 Sen_PixMode = 0;
        CamResIn ResIn;

        mpHalSensor->sendCommand(pAllSensorInfo->mSenInfo[mpSensorIdx].mDevId,
            SENSOR_CMD_GET_DEFAULT_FRAME_RATE_BY_SCENARIO,
            (MUINTPTR)&ResDataIn.scenarioId,
            (MUINTPTR)&ResIn.tg_fps, 0);
        mpHalSensor->sendCommand(pAllSensorInfo->mSenInfo[mpSensorIdx].mDevId,
            SENSOR_CMD_GET_MIPI_PIXEL_RATE,
            (MUINTPTR)&ResDataIn.scenarioId,
            (MUINTPTR)&MipiPixRate,
            (MUINTPTR)&ResIn.tg_fps);
        mpHalSensor->sendCommand(pAllSensorInfo->mSenInfo[mpSensorIdx].mDevId,
            SENSOR_CMD_GET_SENSOR_PIXELMODE,
            (MUINTPTR)&ResDataIn.scenarioId,
            (MUINTPTR)&ResIn.tg_fps,
            (MUINTPTR)&Sen_PixMode);

        _getSensorSize(mpSensorIdx, ResDataIn.scenarioId, ResIn.tg_crop_w, ResIn.tg_crop_h);

        if (MipiPixRate) {
            ResIn.MIPI_pixrate = MipiPixRate;
        }

        ResIn.SrcImgFmt = TG_FMT_RAW10;
        ResIn.bypass_tg = MFALSE;
        ResIn.pix_mode_tg = _NORMALPIPE_MAP_QRY_PXLMODE(Sen_PixMode);

        ResIn.bypass_rrzo = MFALSE;
        ResIn.rrz_out_w = ResDataIn.rrz_out_w;
        ResIn.bypass_imgo = MFALSE;
        ResIn.offBin = ResDataIn.bin_off;

        pCamIOPipe = ICamIOPipe::createInstance((MINT8*)mpName, (E_INPUT)NextTG, ICamIOPipe::CAMIO);

        if (!pCamIOPipe) {
            PIPE_ERR("Create CamIOPipe Fail might be due to resource not enough.");
            bIsExe = MFALSE;
        }
        else{
            bIsExe = pCamIOPipe->getCamRes(ResIn);
            pCamIOPipe->destroyInstance();
        }

        PIPE_INF("Senserid(%d) PixMode(%d) w/h(%d/%d) pixmode(%d) MIPI_PixRate(%d) offbin(%d) NextTG(%d)",
            mpSensorIdx, Sen_PixMode, ResIn.tg_crop_w, ResIn.tg_crop_h, Sen_PixMode, ResIn.MIPI_pixrate, ResIn.offBin, NextTG);
    }
    else {
        bIsExe = MFALSE;
    }
...
EXIT:
    if (!bIsExe) {
        PIPE_WRN("All TG are occupied. (%d/%d)", EfuseTGNum, NextTG);
    }
...
MBOOL NormalPipe::getEfuseInfo(MUINT32 &tg_sum)
{
    int fd = 0;
    int fd_ret = 0;
    MBOOL ret = MTRUE;
    MUINT32 EfuseValue = 0;
    DEVINFO_S DEVINFO;

    tg_sum = (MUINT32)TG_CAM_MAX;
    ...
    if (fd < 0) { /* Use v2 device node if v1 device node is removed */
        PIPE_ERR("/proc/device-tree/chosen/atag,devinfo kernel open fail, errno(%d):%s",errno,strerror(errno));
        ret = MFALSE;
    } else {
        fd_ret = read(fd, (void *)&DEVINFO, sizeof(DEVINFO_S));
        if (fd_ret < 0) {
            PIPE_ERR("Get Devinfo data fail, errno(%d):%s",errno,strerror(errno));
            ret = MFALSE;
        } else {
            EfuseValue = DEVINFO.data[EFUSE_INDEX];
        }
        close(fd);
    }

    switch (EFUSE_DUAL_CAM(EfuseValue))
    {
        case _DUALCAM_LIMIT_HW:
        case _DUALCAM_LIMIT_HW_AND_SW:
            if (tg_sum > 0){
                tg_sum--;
            }
            else{
                PIPE_ERR("logic error: TG sum is not enough to limit");
                ret = MFALSE;
            }
            break;
        case _DUALCAM_NO_LIMIT:
            break;
        default:
            PIPE_ERR("efuse: unknown DUAL_CAM(0x%x) limit", EFUSE_DUAL_CAM(EfuseValue));
            ret = MFALSE;
            break;
    }

    PIPE_INF("tg_sum(%d) efuse(0x%x)", tg_sum, EfuseValue);
...
MBOOL NormalPipe::sendCommand(MINT32 cmd, MINTPTR arg1, MINTPTR arg2, MINTPTR arg3)
{
    ...
    case ENPipeCmd_UNI_SWITCH:
        PIPE_INF("Unsupported CMD ENPipeCmd_UNI_SWITCH at 2-UNI");
        ret = MFALSE;
        break;
    case ENPipeCmd_GET_UNI_SWITCH_STATE:
#define UNI_SWITCH_NULL 0
        *(MUINT32*)arg1 = UNI_SWITCH_NULL;//Such state is used for UNI-Switch project
        PIPE_INF("Unsupported CMD ENPipeCmd_GET_UNI_SWITCH_STATE at 2-UNI");
        ret = MFALSE;
        break;
...
MUINT32 NormalPipe::GetOpenedPortInfo()
{
    MUINT32 _num = 0;
 ...
 *
 ******************************************************************************/
void NormalPipe::FrameMgr::enque(IImageBuffer *pframe, MUINTPTR BufPA)
{
...
NormalPipe_Thread::NormalPipe_Thread(NormalPipe* obj)
{
    ...
    memset(&m_semDeqDonePort, 0, sizeof(sem_t) * MaxPortIdx);
    memset(&m_semDeqDone, 0, sizeof(sem_t));
}
...
MBOOL NormalPipe_Thread::init(MBOOL EnableSec)
{
    ...
    PIPE_INF("mInitCount(%d), EnableSec = %d", this->mCnt, EnableSec);
...
MBOOL NormalPipe_Thread::stop(MBOOL bForce)
{
    ...
    if(bForce == MTRUE) {
        ...
        /*abort deqThr*/
        for (i = 0 ; i < m_pNormalpipe->mOpenedPortNum ; i++) {
            m_pNormalpipe->abortDma(m_pNormalpipe->PortIndex[i], THIS_NAME);
        }
    }
...
#if 0
    for (i = 0; i < m_pNormalpipe->mOpenedPortNum; i++) {
        ::sem_post(&m_semDeqDonePort[_PortMap(m_pNormalpipe->PortIndex[i])]);
    }
#endif
    ::sem_post(&m_semDeqDone);
    ...
    for (i = 0; i < _size; i++) {
        ...
        if(PORT_UFEO.index == portID.index){
            ...
            if (mpSecMgr && pQBuf->mvOut.at(i).mSecon) {
                ...
                /*FIXME Secure CAM*/
                rQBufInfo.vBufInfo.at(i).u4BufPA[0] = SecPA;
                rQBufInfo.vBufInfo.at(i).u4BufVA[0] = (MUINT32)pQBuf->mvOut.at(i).mVa;
                rQBufInfo.vBufInfo.at(i).u4BufSize[0] = (MUINT32)pQBuf->mvOut.at(i).mSize;
                rQBufInfo.vBufInfo.at(i).m_num = (MUINT32)pQBuf->mvOut.at(i).FrameBased.mMagicNum_tuning;
            } else {
                _NOMRALPIPE_PASS_BUFINFO(pQBuf->mvOut.at(i), rQBufInfo.vBufInfo.at(i),PLANE_ID);
            }
        }
    ...
    if (m_pNormalpipe->mpCamIOPipe)  {
        ...
        if (ret < 0) {
            ...
            for (i = 0; i < _size; i++) {
                ...
                #ifdef USE_IMAGEBUF_HEAP
                if(mpSecMgr && pQBuf->mvOut.at(i).mSecon)
                    m_pNormalpipe->mpFrameMgr->enque(pQBuf->mvOut.at(i).mBuffer,rQBufInfo.vBufInfo.at(i).u4BufPA[0]);
                else
...
MBOOL NormalPipe_Thread::dmaCfg(QBufInfo* pQBuf)
{
    ...
    vector<NSImageio::NSIspio::STImgCrop>     listImgCrop;
    vector<NSImageio::NSIspio::STImgResize>   listImgRsz;
    vector<MUINT32>                           listImgRawType;
    ...
    for (i = 0; i < pQBuf->mvOut.size(); i++) {
        ...
        PIPE_INF("N:%d dma:x%x cmd:0x%x, crop:(%d,%d,%d,%d), outsize:(%d,%d)",\
...
MBOOL NormalPipe_Thread::tuningUpdate(vector<MUINT32>* pLMagicNum, vector<MUINT32>* pLPortNum)
{
    ...
    return m_pNormalpipe->mpCamIOPipe->sendCommand(EPIPECmd_SET_P1_UPDATE, (MINTPTR)pLPortNum, (MINTPTR)pLMagicNum,
...
{
    QBufInfo        _qBuf;
    vector<MUINT32> listMagic;
    vector<MUINT32> listPort;
    MUINT32         i, _port, sofIdx = 0;
    MBOOL           ret = MTRUE;
    MINT32          EqRet;
    E_EnqueRst      retRst = eEnqueRst_OK;
    ...
    for(i = 0 ; i < m_pNormalpipe->mOpenedPortNum ; i++) {
        ...
        _port = _PortMap(m_pNormalpipe->PortIndex[i]);

        m_pNormalpipe->mpImgHeaderMgr[_port].sendCmd(QueueMgr<IMEM_BUF_INFO>::eCmd_getsize, (MUINTPTR)&imgHeaderDepth, 0);

        if (0 == imgHeaderDepth) {
            PIPE_ERR("N:%d dma:0x%x imgheader buf empty", m_pNormalpipe->mpSensorIdx, m_pNormalpipe->PortIndex[i]);
    ...
    for(i = 0; i < m_pNormalpipe->mOpenedPortNum; i++) {
        ...
        listPort.push_back(_qBuf.mvOut.at(0).mPortID.index);
    ...
    if (MFALSE == tuningUpdate(&listMagic, &listPort)) {
        PIPE_ERR("N:%d p1_update failed", m_pNormalpipe->mpSensorIdx);
        retRst = eEnqueRst_NG;
        if (m_pNormalpipe->m_DropCB) {
            m_pNormalpipe->m_DropCB(listMagic.front(), m_pNormalpipe->m_returnCookie);
        }
        PIPE_INF("N:%d drop request mag_num[0x%x]", m_pNormalpipe->mpSensorIdx,\
                        listMagic.front());
    ...
    m_pNormalpipe->mpCamIOPipe->sendCommand(EPIPECmd_GET_CUR_SOF_IDX, (MINTPTR)&sofIdx, 0, 0);

    m_LastEnqSof = sofIdx;
    m_ConsumedReqCnt++;

    PIPE_INF("- sof(%d)", m_LastEnqSof);
...
E_EnqueRst NormalPipe_Thread::configEnquePath(NSImageio::NSIspio::ICamIOPipe::E_FRM_EVENT frameEvent)
{
    ...
    PIPE_INF("N:%d configFrame event:%d multiSenNum:%d occupy:%x status:%d(%d) fsm:%d depth:%d",
...
MVOID NormalPipe_Thread::enqueRequest(MUINT32 sof_idx)
{
    ...
    MUINT32     _OpenedPort = m_pNormalpipe->mOpenedPortNum;
    MUINT32     _size = 0, reqQueNum[MaxPortIdx], frm_status = 0;
    MUINT32     cqCnt[2] = {0, 0}; // phyCqCnt:cqCnt[0] virCqCnt:cqCnt[1]
    ...
    m_pNormalpipe->mpCamIOPipe->sendCommand(EPIPECmd_GET_CQUPDATECNT, 0, (MINTPTR)cqCnt, 0);

    PIPE_INF("+N:%d CurSof:%d enqCnt:%d cqCnt(p/v):%d/%d FrameStatus(%d)",
        m_pNormalpipe->mpSensorIdx, sof_idx, m_ConsumedReqCnt, cqCnt[0], cqCnt[1], frm_status);

    if (cqCnt[0] != cqCnt[1]) {
        PIPE_WRN("N:%d cqCnt phy:%d vir:%d reserve request, previous not consumed.", m_pNormalpipe->mpSensorIdx, cqCnt[0], cqCnt[1]);

        MUINT32 _port = _PortMap(m_pNormalpipe->PortIndex[0]);
        MUINT32 reqQueNum = 0;
    ...
    if (m_ConsumedReqCnt < m_pNormalpipe->mReqDepthPreStart) {
        ...
        for (; _availReq > 1; _availReq--) {
            m_DropEnQ.resize(0);
            for (i = 0; i < _OpenedPort; i++) {
                MUINT32 _port = _PortMap(m_pNormalpipe->PortIndex[i]);
    ...
    for (i = 0; i < _OpenedPort; i++) {
        ...
        if (0 == reqQueNum[_port]) {
            PIPE_INF("N:%d dma:x%x no request sof_idx:%d, FrameStatus(%d)", m_pNormalpipe->mpSensorIdx, m_pNormalpipe->PortIndex[i], sof_idx, frm_status);
    ...
    m_DropEnQ.resize(0);
    for (i = 0; i < _OpenedPort; i++) {
        MUINT32 _dmaIdx = m_pNormalpipe->PortIndex[i];
    ...
    /* if (0 == m_DropEnQ.size()) */
    {
        ...
        m_pNormalpipe->mpEnqueRequest[_PortMap(m_pNormalpipe->PortIndex[0])]
...
#define NPIPE_MAX_DEQ_COUNT     (3)
    ...
    MUINT32 _size = 0, _OpenedPort = 0, idx = 0, i, _port = 0;
    ...
    vMnum.clear();
    _OpenedPort = m_pNormalpipe->mOpenedPortNum;

    for (i = 0; i < _OpenedPort; i++) {
        QBufInfo    deqQBuf;
        MUINT32     deqTryCount;

        portID.index = m_pNormalpipe->PortIndex[i];
        ...
        if(vMnum.size() == 0){//if size != 0 => suspending
            ...
            for (idx = 0; idx < rQTSBufInfo.vBufInfo.size(); idx++) {
                ...
                result.mTimeStamp_B = m_pNormalpipe->getTimeStamp_ns_B(rQTSBufInfo.vBufInfo.at(idx).i4TimeStamp_sec_B, rQTSBufInfo.vBufInfo.at(idx).i4TimeStamp_us_B);
                ...
                imBuf.size      = m_pNormalpipe->mImgHeaderSize[_port];
                m_pNormalpipe->mpImgHeaderMgr[_port].sendCmd(QueueMgr<IMEM_BUF_INFO>::eCmd_push, (MUINTPTR)&imBuf, 0);
            }
...
MVOID* NormalPipe_Thread::ispEnqueThread(void *arg)
{
    ...
    while (1) {
        ...
        if (!_this->m_pNormalpipe->mpCamIOPipe) {
            PIPE_ERR("CamIOPipe NULL");
            break;
        }

        _this->mThreadIdleLock_Enq.unlock();
        _SET_SOF_IRQ(irq, _this->m_irqUserKey, _this->estimateFrameTime());
        ret = _this->m_pNormalpipe->mpCamIOPipe->irq(&irq);
        _this->mThreadIdleLock_Enq.lock();
        if (MFALSE == ret) {
        ...
        _this->m_pNormalpipe->mpCamIOPipe->sendCommand(EPIPECmd_GET_CUR_SOF_IDX,(MINTPTR)&_this->m_CurEnqSof,0,0);
        PIPE_DBG("N:%d: cur SOF: %d", _this->m_pNormalpipe->mpSensorIdx, _this->m_CurEnqSof);

        _this->m_pNormalpipe->mEnqContainerLock.lock();
        _this->enqueRequest(_this->m_CurEnqSof);
...
MVOID* NormalPipe_Thread::ispDequeThread(void *arg)
{
    ...
    while (1) {
        ...
        PIPE_INF("N:%d_deq@SOF_%d", _this->m_pNormalpipe->mpSensorIdx,  _sof_idx);
#endif
...
_exit_deq:
    if (ret == 0) {
        ...
        MUINT32 i = 0;
        MUINT32 _OpenedPort = _this->m_pNormalpipe->mOpenedPortNum;
...
#if 0
        for (i = 0; i < _OpenedPort; i++) {
            ::sem_post(&_this->m_semDeqDonePort[_PortMap(_this->m_pNormalpipe->PortIndex[i])]);
        }
#endif
        ::sem_post(&_this->m_semDeqDone);
    }
