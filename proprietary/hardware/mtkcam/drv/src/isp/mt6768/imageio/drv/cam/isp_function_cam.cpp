...
MINT32 CAM_TOP_CTRL::_config( void )
{
    ...
    // DMA CQ-ECO option set to OFF
    CAM_WRITE_REG(this->m_pDrv->getPhyObj(),CAM_DMA_RSV6,0xdfffffff);
...

    {
        ...
        if (pDrvUni->init("UNI_A_DefConfig", (static_cast<IspDrvImp*>(this->m_pDrv->getPhyObj()))) == MFALSE) {
        ...
        pDrvUni->uninit("UNI_A_DefConfig");
        pDrvUni->destroyInstance();
    }
...
MINT32 CAM_TOP_CTRL::_enable( void* pParam  )
{
    ...
    CAM_WRITE_REG(this->m_pDrv->getPhyObj(),CAM_CTL_UNI_B_DONE_SEL,0);
#endif
...
MINT32 CAM_RRZ_CTRL::_config(void)
{
    ...
    MUINT32 rrz_in_roi_DBG_w = 0,rrz_in_roi_DBG_h = 0;
    MUINT32 rrz_vert_tbl_sel = 0, rrz_hori_tbl_sel = 0;
    ...
    rrz_in_roi_DBG_w = this->rrz_in_roi.w;
    rrz_in_roi_DBG_h = this->rrz_in_roi.h;
    ...
    rrz_vert_tbl_sel = this->getCoefTbl(rrz_in_roi_with_bin.h,this->rrz_out_size.h);
    rrz_hori_tbl_sel = this->getCoefTbl(rrz_in_roi_with_bin.w,this->rrz_out_size.w);
    rrz_ctl = (rrz_vert_tbl_sel << 24) | (rrz_hori_tbl_sel << 16) | 0x03;

    if(0xEE == rrz_vert_tbl_sel || 0xEE == rrz_hori_tbl_sel){
        CAM_FUNC_ERR("CAM_RRZ_CTRL::_config+ cq:0x%x,page:0x%x,rrz_in_w/h(%ld/%ld),rrz_crop_x/y/fx/fy/w/h(%d/%d/%d/%d/%ld/%ld),rrz_out_w/h(%ld/%ld),rlb_offset(%d),rrz_h/v_step(%d/%d),dbn/bin(%d/%d),dbg(%d/%d/%d/%d)\n",cq,page,\
                rrz_in_size.w, \
                rrz_in_size.h, \
                this->rrz_in_roi.x, \
                this->rrz_in_roi.y, \
                ...
                dbn_en, bin_en,rrz_in_roi_DBG_w,rrz_in_roi_DBG_h, \
                rrz_in_roi_with_bin.w,rrz_in_roi_with_bin.h);
        return 1;
    }
...
MINT32 CAM_RCP3_CTRL::_config()
{
    ...
    switch(CAM_READ_BITS(this->m_pRaw_p->m_pDrv,CAM_CTL_SEL,RCP3_SEL))
    {
        ...
        case E_rcp3_3:
            ...
            v_bin_status = CAM_READ_BITS(this->m_pRaw_p->m_pDrv,CAM_CTL_EN,BIN_EN);
            pix_mode = _1_pix_;
            break;
        case E_rcp3_2:
...
MINT32 CAM_PMX_CTRL::_config(void)
{
    ...
    switch(CAM_READ_BITS(this->m_pRaw_p->m_pDrv,CAM_CTL_SEL,PMX_SEL)){
    case PMX_SEL_0:
                ...
                case E_FromTG:
                    {
                        ...
                        MUINT32 vbn_en = CAM_READ_BITS(this->m_pDrv,CAM_CTL_DMA_EN,VBN_EN);

                        /* Due to RCP3_sel is set to 2 in E_FromTG case, therefore pix mode should ref to DMXO not TG */
                        if((dmxo - (dbn_en+vbn_en)) == _4_pix_)
                            return _4_pix_;
                        else if((dmxo - (dbn_en+vbn_en)) == _2_pix_)
                            return _2_pix_;
                        else
                            return _1_pix_;
                    }
...
MINT32 CAM_DPD_CTRL::_config( void )
{
    ...
    else if(CAM_READ_REG(this->m_pDrv,CAM_CTL_SEL) & PMX_SEL_2){
