...
public:
    ...
    public:
        MdpRotDMACfg():
            crop_en(0),pixel_byte(1),uv_plane_swap(0),Format(eImgFmt_IMPLEMENTATION_DEFINED),Plane(0),Rotation(0),withCropM(0),enSrcCrop(0),srcCropX(0),srcCropFloatX(0),srcCropY(0),srcCropFloatY(0),srcCropW(0),srcCropFloatW(0),srcCropH(0),srcCropFloatH(0),Flip(0),capbility(0x00),secureTag(0)
            {};
};
...
typedef struct _ST_CAM_TOP_CTRL {
    ...
    CAM_REG_CTL_RAW_INT_EN    INTE_EN;
    CAM_REG_CTL_RAW_INT2_EN     DMA_INTE_EN;
}ST_CAM_TOP_CTRL;

typedef struct _ST_UNI_TOP_CTRL{
    CAM_UNI_REG_TOP_MOD_EN      FUNC_EN;
    CAM_UNI_REG_TOP_FMT_SEL     FMT_SEL;
    CAM_UNI_REG_TOP_PATH_SEL    CTRL_SEL;
    CAM_UNI_REG_TOP_DMA_INT_EN  INTE_EN;
}ST_UNI_TOP_CTRL;
...
struct ST_CAMSV_TOP_CTRL {
    ...
    CAMSV_REG_INT_EN               INTE_EN;
};

/* [mt6797 isp_reg review] need DIP owner to review this code
