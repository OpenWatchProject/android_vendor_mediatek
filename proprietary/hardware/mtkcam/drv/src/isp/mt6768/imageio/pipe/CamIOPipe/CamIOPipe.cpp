    ...
    if(this->m_occupied == MFALSE){
        if(*Name == '\0'){
            ...
            if((strcmp((char*)this->m_User,(char*)TwinMgr::ModuleName()) != 0) &&
                (strcmp((char*)this->m_User,(char*)"NpipeIsExe") != 0)){
                this->m_available--;
            }
        }
    }
...
inline E_PIX_MODE _PM_BitOP(E_CamPixelMode pix_mode) {
    E_PIX_MODE bitOP = _1_pix_;
    switch(pix_mode){
    case ePixMode_1: bitOP = _1_pix_; break;
    case ePixMode_2: bitOP = _2_pix_; break;
    case ePixMode_4: bitOP = _4_pix_; break;
    default:BASE_LOG_ERR("unsupport pix mode(%d)",pix_mode);break;
    }
    return bitOP;
}

MBOOL CamIOPipe::getCamRes(const CamResIn ResIn)
{
    MBOOL ret = MTRUE;
    TWIN_MGR_PATH_CFG_IN_PARAM cfgInParam;
    TWIN_MGR_PATH_CFG_OUT_PARAM cfgOutParam;

    cfgInParam.pix_mode_tg = _PM_BitOP(ResIn.pix_mode_tg);
    cfgInParam.bDbn = ResIn.bDbn;
    cfgInParam.bypass_imgo = ResIn.bypass_imgo;
    cfgInParam.bypass_rrzo = ResIn.bypass_rrzo;
    cfgInParam.MIPI_pixrate = ResIn.MIPI_pixrate;
    cfgInParam.offBin = ResIn.offBin;
    cfgInParam.bypass_tg = ResIn.bypass_tg;
    cfgInParam.rrz_out_w = ResIn.rrz_out_w;
    cfgInParam.SrcImgFmt = ResIn.SrcImgFmt;
    cfgInParam.tg_crop_h = ResIn.tg_crop_h;
    cfgInParam.tg_crop_w = ResIn.tg_crop_w;
    cfgInParam.tg_fps = ResIn.tg_fps;

    if(this->m_cam_dfs){
        this->m_pCmdQMgr = DupCmdQMgr::CmdQMgr_attach(cfgInParam.tg_fps, MFALSE ,this->m_hwModule,ISP_DRV_CQ_THRE0);
        if(this->m_pCmdQMgr){
            this->m_cam_dfs->m_pIspDrv = (IspDrvVir*)this->m_pCmdQMgr->CmdQMgr_GetCurCycleObj(0)[0];
            if(this->m_cam_dfs->m_pIspDrv){
                this->m_cam_dfs->CAM_DFS_INIT(&cfgInParam.vClk);
                this->m_TwinMgr = TwinMgr::createInstance(this->m_hwModule);
                if(this->m_TwinMgr){
                   ret = this->m_TwinMgr->configPath(cfgInParam, cfgOutParam, this->m_ResMgr.Get_AvailNum());
                ...
                ((ISP_DRV_CAM*)this->m_cam_dfs->m_pIspDrv)->destroyInstance();
            }
            else{
                PIPE_ERR("CmdQMgr_GetCurCycleObj is NULL");
                return MFALSE;
            }
            if(this->m_pCmdQMgr->CmdQMgr_detach() == 1) {
                PIPE_ERR("CmdQMgr_detach fail");
                return MFALSE;
            }
        }
        else{
            PIPE_ERR("CmdQMgr_attach fail\n");
            return MFALSE;
        }
    }
    else{
        PIPE_ERR("m_cam_dfs is null!\n");
        return MFALSE;
    }
...
CamIOPipe* CamIOPipe::Create(MINT8 const szUsrName[32], E_INPUT InPut)
{
    ...
    switch(InPut){
        case TG_A:
            if(*gCamioPipe[CAM_A].m_ResMgr.Get_Res() != '\0'){
                ...
                if(gCamioPipe[CAM_A].m_ResMgr.Register_Res((char*)szUsrName) == MTRUE){
        ...
        case TG_B:
            if(*gCamioPipe[CAM_B].m_ResMgr.Get_Res() != '\0'){
                ...
                if(gCamioPipe[CAM_B].m_ResMgr.Register_Res((char*)szUsrName) == MTRUE){
    ...
    if(ptr == NULL)
        return NULL;
    //
    if((ptr->m_cam_dfs = CAM_DFS_Mgr::createInstance(LOG_TAG, ptr->m_hwModule)) == 0){
       BASE_LOG_ERR("cam_dfs createInstance fail");
    }
...
void CamIOPipe::Destroy(void)
{
    ...
    if(*(this->m_ResMgr.Get_Res()) != '\0')
        ...
        PIPE_WRN("have suspend without resume ??\n");
...
MBOOL CamIOPipe::resume(E_SUSPEND_MODE suspendMode)
{
    ...
    if(gCamioPipe[this->m_hwModule].m_ResMgr.Register_Res((char*)"resume") == MFALSE){
        ret = MFALSE;
...
MBOOL CamIOPipe::configPipe(vector<PortInfo const*>const& vInPorts, vector<PortInfo const*>const& vOutPorts,CAMIO_Func func)
{
    ...
    for (MUINT32 i = 0 ; i < vInPorts.size() ; i++ ) {
        ...
        PIPE_INF("P1 vInPorts_%d:fmt(0x%x),PM(%d),w/h(%d_%d),crop(%d_%d_%d_%d),tg_idx(%d),dir(%d),fps(%d),timeclk(0x%x),MIPI_PixRate(0x%x),subsample(%d),bin_off(%d),dat_pat(%d),sen_num(%d),force_clk_level(%d)\n",\
                i, \
                vInPorts[i]->eImgFmt, \
                vInPorts[i]->ePxlMode,  \
    ...
    if(this->m_camPass1Param.bypass_tg == MFALSE)
    {
        ...
        //
        memcpy((void*)&this->m_PathCtrl.m_Src,(void*)vInPorts[idx_in_src],sizeof(NSImageio::NSIspio::PortID));
        ...
        if (func.Bits.force_clk_level) {
            this->m_PathCtrl.m_cfgInParam.force_clk_level = MTRUE;
        } else {
            this->m_PathCtrl.m_cfgInParam.force_clk_level = MFALSE;
        }
        ...
        if(cfgOutParam.bin_en== MTRUE) {
            ...
            PIPE_INF("Frontal binning is enable!! bin(%d), Set Raw type to PureRaw!!!\n",  cfgOutParam.bin_en);
        }
    ...
    if(this->m_TwinMgr->getIsTwin() == MTRUE)
    {
        ...
        for(it = l_module.begin();it!=l_module.end();it++){
            if(*gCamioPipe[*it].m_ResMgr.Get_Res() == '\0')
            {
                ...
                memcpy((void*)&gCamioPipe[*it].m_PathCtrl.m_Src,(void*)vInPorts[idx_in_src],sizeof(NSImageio::NSIspio::PortID));
...
MBOOL CamIOPipe::sendCommand(MINT32 cmd, MINTPTR arg1, MINTPTR arg2, MINTPTR arg3)
{
    ...
    switch ( cmd ) {
        ...
        case EPIPECmd_ALLOC_UNI:
            ...
            if(this->m_camPass1Param.bypass_tg == MTRUE){
                ...
                if(this->m_FSM == op_cfg){
                    ...
                    if((this->m_hwModule == CAM_A) && (this->m_UniMgr.UniMgr_attach(UNI_A,this->m_pCmdQMgr,NULL) != 0)){
                        ret = 1;
                    }
                    if((this->m_hwModule == CAM_B) && (this->m_UniMgr.UniMgr_attach(UNI_B,this->m_pCmdQMgr,NULL) != 0)){
                        ret = 1;
                    }
                }
                else{
        ...
        case EPIPECmd_SET_RRZ:
            ...
            {
                vector<STImgCrop>* pInput_L;
                vector<STImgResize>* pInput2_L;
                vector<STImgCrop>::iterator it;
                vector<STImgResize>::iterator it2;

                vector<IspRrzCfg> rrz_L;
                IspRrzCfg rrz;
                ...
                pInput_L = (vector<STImgCrop>*)arg1;
                pInput2_L = (vector<STImgResize>*)arg2;
                ...
                for(it=pInput_L->begin(),it2=pInput2_L->begin();it!=pInput_L->end();it++,it2++){
                    ...
                    if(this->m_FSM == op_start) {
                        for (MUINT32 dupCQ = 0;  dupCQ < this->m_pCmdQMgr->CmdQMgr_GetDuqQ(); dupCQ++){
                            for (MUINT32 Burst = 0;  Burst < this->m_pCmdQMgr->CmdQMgr_GetBurstQ(); Burst++){
                                if(bin_en != CAM_READ_BITS(this->m_pCmdQMgr->CmdQMgr_GetCurCycleObj(dupCQ)[Burst],CAM_CTL_EN,BIN_EN))
                                    PIPE_WRN("Bin might wrong\n");
                            }
                        }
                    }
        ...
        case EPIPECmd_SET_P1_UPDATE:
            ...
            {
                vector<MUINT32>* pInput_LPort = (vector<MUINT32>*)arg1;
                vector<MUINT32>* pInput_L = (vector<MUINT32>*)arg2;
                vector<MUINT32>::iterator it;
                IspP1TuningCfg cfg;
                vector<IspP1TuningCfg> cfg_L;
                vector<MUINT32> dmaChannel_L;
                vector<MUINT32> dmaChannel_Twin_L;
                MUINT32 dmao_bw = 0;
                MUINT32 dupq = 0;
                string _str;
                ...
                {//temp dbg log
                    _str.append("+");
                    for(MUINT32 i = 0;i<this->m_pCmdQMgr->CmdQMgr_GetDuqQ();i++){
                        for(MUINT32 j=0;j<this->m_pCmdQMgr->CmdQMgr_GetBurstQ();j++){
                            char _temp[64] = {'\0'};
                            snprintf(_temp, sizeof(_temp), "[%d_%d]:0x%x ", i, j,
                                CAM_READ_REG(this->m_pCmdQMgr->CmdQMgr_GetCurCycleObj(i)[j],CAM_CTL_EN));
                            _str.append(_temp);
                        }
                    }
                }
                ...
                */
                ...
                if (0 != this->m_CamPathPass1.setP1TuneCfg(&cfg_L)) {
                    ...
                    PIPE_INF("start drop enque buf Twin:%d M:0x%x",
                            this->m_TwinMgr->getIsTwin(), pInput_L->front());

                    dmaChannel_L.clear();
                    dmaChannel_Twin_L.clear();
                    for(it = pInput_LPort->begin(); it!= pInput_LPort->end(); it++){
                        MUINT32 dmaChannel;

                        PIPE_DBG("port ID:%d\n", *it);
                        if ((dmaChannel = (MUINT32)this->PortID_MAP(*it)) == _cam_max_) {
                            if ((dmaChannel = this->m_CamIO_stt.QueryPort(*it)) != _cam_max_) {
                                dmaChannel_L.push_back(dmaChannel);
                                PIPE_DBG("EPIPECmd_SET_P1_UPDATE[2]:dmaChannel:%d\n", dmaChannel);
                            }
                        } else {
                            dmaChannel_L.push_back(dmaChannel);
                            PIPE_DBG("EPIPECmd_SET_P1_UPDATE[1]:dmaChannel:%d\n", dmaChannel);
                        }
                    }
                    for (it = dmaChannel_L.begin(); it!= dmaChannel_L.end(); it++) {
                        switch (*it) {
                            case _imgo_:
                            case _rrzo_:
                                PIPE_INF("remove multiChannel port:%d\n", *it);
                                break;
                            default:
                                dmaChannel_Twin_L.push_back(*it);
                                break;
                        }
                    }
                    ...
                    if (MTRUE == this->m_TwinMgr->getIsTwin()) {
                        //drop imgo, rrzo
                        if (MFALSE == this->m_TwinMgr->dropEnqueTwin(this->m_PathCtrl.m_cfgInParam, pInput_L)) {
                            PIPE_ERR("dropEnqueTwin fail\n");
                        }
                        //drop other ports if opened
                        if (MFALSE == this->m_CamPathPass1.dropEnqueueBuf(&dmaChannel_Twin_L, pInput_L)) {
                            PIPE_ERR("dropEnqueueBuf fail\n");
                        }
                    } else {
                        if (MFALSE == this->m_CamPathPass1.dropEnqueueBuf(&dmaChannel_L, pInput_L)) {
                            PIPE_ERR("dropEnqueueBuf fail\n");
                        }
                    }
                    return MFALSE;
                ...
                for (MUINT32 burstq = 0; burstq < this->m_pCmdQMgr->CmdQMgr_GetBurstQ(); burstq++) {
                    ...
                    if(this->m_UniMgr.UniMgr_GetCurLink() == this->m_hwModule){
                        //FLKO
                        if(UNI_READ_BITS(this->m_pCmdQMgr->CmdQMgr_GetCurCycleObj(dupq)[burstq],CAM_UNI_TOP_DMA_EN,FLKO_A_EN) == 1){
                            dmao_bw += (UNI_READ_REG(this->m_pCmdQMgr->CmdQMgr_GetCurCycleObj(dupq)[burstq],CAM_UNI_FLKO_YSIZE)+1) * \
                                       UNI_READ_REG(this->m_pCmdQMgr->CmdQMgr_GetCurCycleObj(dupq)[burstq],CAM_UNI_FLKO_STRIDE);
                            PIPE_DBG("Total BW:%d FLKO(%d*%d)",dmao_bw,UNI_READ_REG(this->m_pCmdQMgr->CmdQMgr_GetCurCycleObj(dupq)[burstq],CAM_UNI_FLKO_STRIDE),\
                                                     UNI_READ_REG(this->m_pCmdQMgr->CmdQMgr_GetCurCycleObj(dupq)[burstq],CAM_UNI_FLKO_YSIZE)+1);
                        }
                        //LMVO
                        if(UNI_READ_BITS(this->m_pCmdQMgr->CmdQMgr_GetCurCycleObj(dupq)[burstq],CAM_UNI_TOP_DMA_EN,EISO_A_EN) == 1){
                            dmao_bw += (UNI_READ_REG(this->m_pCmdQMgr->CmdQMgr_GetCurCycleObj(dupq)[burstq],CAM_UNI_EISO_YSIZE)+1) * \
                                       UNI_READ_REG(this->m_pCmdQMgr->CmdQMgr_GetCurCycleObj(dupq)[burstq],CAM_UNI_EISO_STRIDE);
                            PIPE_DBG("Total BW:%d LMVO(%d*%d)",dmao_bw,UNI_READ_REG(this->m_pCmdQMgr->CmdQMgr_GetCurCycleObj(dupq)[burstq],CAM_UNI_EISO_STRIDE),\
                                       UNI_READ_REG(this->m_pCmdQMgr->CmdQMgr_GetCurCycleObj(dupq)[burstq],CAM_UNI_EISO_YSIZE)+1);
                        }
                        //RSSO
                        if(UNI_READ_BITS(this->m_pCmdQMgr->CmdQMgr_GetCurCycleObj(dupq)[burstq],CAM_UNI_TOP_DMA_EN,RSSO_A_EN) == 1){
                            dmao_bw += (UNI_READ_REG(this->m_pCmdQMgr->CmdQMgr_GetCurCycleObj(dupq)[burstq],CAM_UNI_RSSO_A_YSIZE)+1) * \
                                       UNI_READ_REG(this->m_pCmdQMgr->CmdQMgr_GetCurCycleObj(dupq)[burstq],CAM_UNI_RSSO_A_STRIDE);
                            PIPE_DBG("Total BW:%d RSSO(%d*%d)",dmao_bw,UNI_READ_REG(this->m_pCmdQMgr->CmdQMgr_GetCurCycleObj(dupq)[burstq],CAM_UNI_RSSO_A_STRIDE),\
                                       UNI_READ_REG(this->m_pCmdQMgr->CmdQMgr_GetCurCycleObj(dupq)[burstq],CAM_UNI_RSSO_A_YSIZE)+1);
                        }
                    }
                }

                dmao_bw = dmao_bw * OVERHEAD_RATIO;
...

                {
                    ...
                    PIPE_INF("[%d]P1_UPDATE_CHECK CTL_EN(0x%x) %s\n",this->m_pCmdQMgr->CmdQMgr_GetDuqQIdx(),
                        CAM_READ_REG((this->m_pCmdQMgr->CmdQMgr_GetCurCycleObj(0)[0])->getPhyObj(),CAM_CTL_EN),
                        _str.c_str());
                }
        ...
        case EPIPECmd_SET_IMGO:
            ...
            {
                vector<STImgCrop>* pInput_L;
                vector<MUINT32>* pInput2_L;
                vector<STImgCrop>::iterator it;
                vector<MUINT32>::iterator it2;
                DMACfg imgo;
                vector<DMACfg> input_L;
        ...
        case EPIPECmd_SET_UFEO:
            {
                vector<STImgResize>* pInput_L;
                vector<STImgResize>::iterator it;
                DMACfg ufeo;
                vector<DMACfg> ufeo_L;
                ...
                pInput_L = (vector<STImgResize>*)arg1;
        ...
        case EPIPECmd_GET_CQUPDATECNT:
            ...
            *(MUINT32*)arg2 = this->m_pCmdQMgr->CmdQMgr_GetCounter(MTRUE);
            *((MUINT32*)arg2 + 1) = this->m_pCmdQMgr->CmdQMgr_GetCounter(MFALSE);
        ...
        case EPIPECmd_GET_RCP_INFO:
            {
                ...
                if(pipe_DbgLogEnable_INFO){
                    ...
                    for(MUINT32 i=0;i<pVec->size();i++){
                        PIPE_INF("vec_%d: path:0x%x,size:%d_%d_%d_%d\n",i,pVec->at(i).Pipe_path,\
