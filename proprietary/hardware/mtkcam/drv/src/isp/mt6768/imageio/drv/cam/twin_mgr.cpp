    ...
    switch(op){
        case op_unknown:
            switch(this->m_FSM){
                ...
                case op_cfg:    //if twin is not enabled, status will be kept at cfg stage
...
MBOOL TwinMgr_IMP::configPath(const TWIN_MGR_PATH_CFG_IN_PARAM& cfgInParam, TWIN_MGR_PATH_CFG_OUT_PARAM &cfgOutParam, MUINT32 navailCam)
{
    ...
    //
    LOG_DBG("cfgIn:tg_en(%d), tg_pix(%d), tg_crop(%dx%d),fps(%d), rrzo_en(%d), rrz_out(%d),cur clk:%d,off_bin:%d,avail_cam:%d,force_clk_level:%d\n", \
            cfgInParam.bypass_tg, cfgInParam.pix_mode_tg, cfgInParam.tg_crop_w, cfgInParam.tg_crop_h,cfgInParam.tg_fps,\
            cfgInParam.bypass_rrzo, cfgInParam.rrz_out_w,cfgInParam.vClk.at(cfgOutParam.clk_level),cfgInParam.offBin,navailCam,cfgInParam.force_clk_level);

    //cfgOutParam.clk_level is started from 0. always use lowest clk, and notify clk request of isp to MMDVFS each frame.
    ...
    case TG_FMT_RAW14:
        if(cfgInParam.tg_crop_w > rst.pipeSize){
            ...
            if(bBin_useful){
                ...
                LOG_DBG("twin is enabled because of over max size:%d_%d\n",cfgInParam.tg_crop_w,rst.pipeSize);
            }
        }
        ...
        //4. Force raise clock for dual cam
        if (cfgInParam.force_clk_level) {
            cfgOutParam.clk_level = cfgInParam.vClk.size()-1;
            LOG_DBG("Clk level is forced to %d\n", cfgOutParam.clk_level);
        }

        //5. Force raise clock for burst mode.
        if (this->m_TwinCtrl.m_subsample >= 4) {
            cfgOutParam.clk_level = (cfgOutParam.clk_level + 1 >= cfgInParam.vClk.size()-1) ?
                cfgInParam.vClk.size()-1 : cfgOutParam.clk_level + 1;
            LOG_INF("Clk level is forced to %d due to subsample(%d)\n", cfgOutParam.clk_level, this->m_TwinCtrl.m_subsample);
        }
        break;
    default:
    ...
    LOG_DBG("cfgOutParam: pix_mode_dmxi(%d), pix_mode_dmxo(%d), pix_mode_bmxo(%d), pix_mode_rmxo(%d), bin_en(%d), isTwin(%d),clk rate(%d)", \
...
MBOOL TwinMgr_IMP::TwinInfo_update(const TWIN_MGR_PATH_CFG_IN_PARAM& cfgInParam,MBOOL isTwin)
{
    ...
    if(_size > MAX_QUE_SIZE){
        ...
        LOG_INF("no deque on AFO over %d frames\n",MAX_QUE_SIZE);
    }
...
MBOOL TwinMgr_IMP::initTwin(DupCmdQMgr* pMainCmdQMgr,const NSImageio::NSIspio::PortID* pInput,MUINT32 subSample,vector<MUINT32>* pChannle)
{
    MBOOL ret = MTRUE;
    ...
    //update state
    {
        for(vector<ISP_HW_MODULE>::iterator it = this->m_occupiedModule.begin();it != this->m_occupiedModule.end();it++){
...
MBOOL TwinMgr_IMP::suspendTwin(E_SUSPEND_OP op)
{
    ...
    switch(op){
        ...
        case E_SUS_READY:
            ...
            //update state
            i=0;
            for(vector<ISP_HW_MODULE>::iterator it = this->m_occupiedModule.begin(); \
...
vector<ISP_HW_MODULE> TwinMgr_IMP::getCurModule(void)
{
    Mutex::Autolock lock(this->mTwinLock);
    vector<ISP_HW_MODULE>     occupied;//double buffer for occupiedmodule,for multitreahd
    occupied.clear();
...
MBOOL TwinMgr_IMP::dropEnqueTwin(TWIN_MGR_PATH_CFG_IN_PARAM m_cfgInParam, vector<MUINT32>* pLMagicNum) {

    MUINT32 burstQ = 0;
    MUINT32 _size;

    burstQ = this->m_pMainCmdQMgr->CmdQMgr_GetBurstQ();
    if (pLMagicNum->size() != burstQ) {
        LOG_ERR("BurtQ number doesn't match, pLMagicNum size:%d burstQ:%d\n", pLMagicNum->size(), burstQ);
        return MFALSE;
    }

    if (this->m_TBC.enque_drop() == MFALSE) {
        // Revert TwinInfo_update()
        this->m_isTwin_AFO.sendCmd(QueueMgr<MBOOL>::eCmd_getsize,(MUINTPTR)&_size,0);
        if (_size > 1)
            this->m_isTwin_AFO.sendCmd(QueueMgr<MBOOL>::eCmd_pop_back, 0, 0);

        if (m_cfgInParam.bypass_imgo == MFALSE) {
            this->m_isTwin_IMGO.sendCmd(QueueMgr<MBOOL>::eCmd_getsize, (MUINTPTR)&_size, 0);
            if (_size > 1)
                this->m_isTwin_IMGO.sendCmd(QueueMgr<MBOOL>::eCmd_pop_back, 0, 0);
        }

        if (m_cfgInParam.bypass_rrzo == MFALSE) {
            this->m_isTwin_RRZO.sendCmd(QueueMgr<MBOOL>::eCmd_getsize, (MUINTPTR)&_size, 0);
            if (_size > 1)
                this->m_isTwin_RRZO.sendCmd(QueueMgr<MBOOL>::eCmd_pop_back, 0, 0);
        }
        return MTRUE;
    } else {
        return MFALSE;
...
MBOOL TwinMgr_IMP::getTwinALLCropinfo(vector<vector<ISP_HW_MODULE>>* pPath,vector<vector<STImgCrop>>* pcrop)
{
    ...
    if(this->m_hwModule == CAM_A){
        ...
        LOG_WRN("twin's master cam can only be CAM_A\n");
    }
    ...
    switch(cmd){
        ...
        case TWIN_CMD_GET_CUR_SLAVE_CQMGR:
            ...
            if((ret = this->sendCommand(TWIN_CMD_GET_CUR_TWINMODULE,arg2,0,0)) == MFALSE){
                ...
                vector<ISP_HW_MODULE>* l_module = (vector<ISP_HW_MODULE>*)arg2;
        ...
        case TWIN_CMD_GET_CUR_TWINMODULE:
            ...
            *(vector<ISP_HW_MODULE>*)arg1 = this->getCurModule();
            break;
        default:
            ret = MFALSE;
            break;
