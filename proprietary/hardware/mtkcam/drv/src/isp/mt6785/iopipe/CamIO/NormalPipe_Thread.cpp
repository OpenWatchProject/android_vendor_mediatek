...
MBOOL NormalPipe_Thread::tuningUpdate(list<MUINT32>* pLPortNum, list<MUINT32>* pLMagicNum)
{
...
E_EnqueRst NormalPipe_Thread::frameToEnque(void)
{
    ...
    if (MFALSE == tuningUpdate(&listPort, &listMagic)) {
...
MVOID* NormalPipe_Thread::ispEnqueThread(void *arg)
{
    ...
    int const priority  = NICE_CAMERA_P1_ENQUE; //Need to adjust priority to -13;
    ...
    PIPE_INF("NICE_CAMERA_P1_ENQUE priority :%d", NICE_CAMERA_P1_ENQUE);
...
MVOID* NormalPipe_Thread::ispDequeThread(void *arg)
{
    ...
    PIPE_INF("NICE_CAMERA_ISP_DEQUE priority :%d", NICE_CAMERA_P1_DEQUE);
    switch (_NORMALPIPE_GET_TG_IDX(_this->m_pNormalpipe->mpSensorIdx)) {
