...
NormalPipe*               NormalPipe::pNormalPipe[EPIPE_Sensor_RSVD] = { NULL };
...
MBOOL NormalPipe::configPipe(QInitParam const& vInPorts, MINT32 burstQnum)
{
    ...
    if (!mpCamIOPipe) {
        ...
        switch (vInPorts.m_Func.Bits.SensorNum) {
            case E_1_SEN:
                 SensorNum = ECamio_1_SEN;
                break;
            case E_2_SEN:
                 SensorNum = ECamio_2_SEN;
                break;
            default:
                PIPE_ERR("Unsupported sensorNum:%d", vInPorts.m_Func.Bits.SensorNum);
                return MFALSE;
        }
        // Use User specified TG to create CamIO
        PIPE_DBG("TG:%d from MW", camResConfig.Bits.targetTG);
        if (TG_CAM_MAX != camResConfig.Bits.targetTG) {
            TargetTG = (E_INPUT)camResConfig.Bits.targetTG;
        }

        mpCamIOPipe = ICamIOPipe::createInstance((MINT8*)mpName, TargetTG, NSImageio::NSIspio::ICamIOPipe::CAMIO, &tgi, SensorNum);
                        ...
                        MUINT32 TimeoutMs, SignalTimeInfo *pTime)
{
    ...
    if (this->m_FSM == op_stop ||
        ...
        this->m_FSM == op_uninit) {
        PIPE_WRN("m_FSM:%d, interrupt is stopped", this->m_FSM);
    }
...
MBOOL NormalPipe::resume(QBufInfo const * pQBuf, E_SUSPEND_TPYE etype)
{
    ...
    E_INPUT     TargetTG = (E_INPUT)mCamResConfig.Bits.targetTG;
    ...
    case ENPipeCmd_SET_QUALITY:
        {
            ...
            if(updateIQFSM(IQ_npipe_set, NPIPE_CAM_A) && updateIQFSM(IQ_npipe_set, NPIPE_CAM_B)){
                ...
                sIQData.IQInfo.lv[NPIPE_CAM_A] = (E_CamIQLevel)arg2;
                sIQData.IQInfo.lv[NPIPE_CAM_B] = (E_CamIQLevel)arg3;
            }
            else{
                ret = MFALSE;
                goto _EXIT_SEND_CMD;
