...
P1NodeImp()
    ...
    //
    , mQualitySwitchLock()
    , mQualitySwitchingState(QUALITY_SWITCH_PROGRESS_NOT_START)
    , mQualitySwitchingValue(0)
    //
    //, mImageStorage()
    //
    , m3AProcessedDepth(3)
...
check_config(ConfigParams const& rParams)
{
    ...
    android::String8 meta_str("");
    {
        ...
        if (rParams.pInImage_raw != NULL) {
            mvStreamImg[STREAM_IMG_IN_RAW] = rParams.pInImage_raw;
        }
        //
        ...
        if (rParams.pOutImage_full_HAL != NULL) {
            mvStreamImg[STREAM_IMG_OUT_FULL] = rParams.pOutImage_full_HAL;
        }
        if (rParams.pOutImage_full_APP != NULL) {
            mvStreamImg[STREAM_IMG_OUT_FULL_APP] = rParams.pOutImage_full_APP;
        }
        ...
        if (rParams.pOutImage_yuv_full != NULL) {
            mvStreamImg[STREAM_IMG_OUT_YUV_FULL] = rParams.pOutImage_yuv_full;
        };
        if (rParams.pOutImage_yuv_resizer1 != NULL) {
            mvStreamImg[STREAM_IMG_OUT_YUV_RESIZER1] = rParams.pOutImage_yuv_resizer1;
        };
        if (rParams.pOutImage_yuv_resizer2 != NULL) {
            mvStreamImg[STREAM_IMG_OUT_YUV_RESIZER2] = rParams.pOutImage_yuv_resizer2;
        };
        #if SUPPORT_LCS
        ...
        #if 1 // check conflict configuration
        {
            if (((EN_BURST_MODE) && (EN_BATCH_MODE)) ||
                ...
                MY_LOGE("[Check_Config_Conflict] P1Node::ConfigParams:: "
                    "burstNum(%d) batchNum(%d) laggingLaunch(%d) "
                    "enableCaptureFlow(%d) initRequest(%d) "
                    "pInImage_opaque[%#" PRIx64 "] "
                    "pInImage_yuv[%#" PRIx64 "] "
                    "pInImage_raw[%#" PRIx64 "] ",
                    rParams.burstNum, rParams.batchNum, rParams.laggingLaunch,
    ...
    if (mvStreamImg[STREAM_IMG_OUT_OPAQUE] != NULL) {
        if (mvStreamImg[STREAM_IMG_OUT_FULL] != NULL) {
            ...
            L_if (bufPlanes.count > 0) {
                mRawStride = bufPlanes.planes[0].rowStrideInBytes;
                mRawLength = bufPlanes.planes[0].sizeInBytes;
            } else {
                MY_LOGE("STREAM_IMG_OUT_FULL Planes(%zu)", bufPlanes.count);
                return BAD_VALUE;
            }
        } else if (mvStreamImg[STREAM_IMG_OUT_FULL_APP] != NULL) {
            mRawFormat = mvStreamImg[STREAM_IMG_OUT_FULL_APP]->getImgFormat();
            IImageStreamInfo::BufPlanes_t const & bufPlanes =
                mvStreamImg[STREAM_IMG_OUT_FULL_APP]->getBufPlanes();
            L_if (bufPlanes.count > 0) {
                mRawStride = bufPlanes.planes[0].rowStrideInBytes;
                mRawLength = bufPlanes.planes[0].sizeInBytes;
            } else {
                MY_LOGE("STREAM_IMG_OUT_FULL_APP Planes(%zu)", bufPlanes.count);
                return BAD_VALUE;
            }
        } else {
            mRawFormat = P1_IMGO_DEF_FMT;
    ...
    //
    if (mvStreamImg[STREAM_IMG_OUT_FULL_APP] != NULL) {
        if (mvStreamImg[STREAM_IMG_OUT_FULL_APP]->getImgSize() !=
            mSensorParams.size) {
            MY_LOGE("[Check_Config_Conflict] IMGO_Stream.ImgSize(%dx%d) != "
                "SensorParam.Size(%d,%d) - P1Node::ConfigParams:: "
                "IMGO_StreamID:%#" PRIx64 "_ImgFormat[0x%x]-ImgSize(%dx%d) "
                "SensorParam_mode(%d)_fps(%d)_pixelMode(%d)_vhdrMode(%d)_"
                "size(%dx%d)", mvStreamImg[STREAM_IMG_OUT_FULL_APP]->getImgSize().w,
                mvStreamImg[STREAM_IMG_OUT_FULL_APP]->getImgSize().h,
                mSensorParams.size.w, mSensorParams.size.h,
                mvStreamImg[STREAM_IMG_OUT_FULL_APP]->getStreamId(),
                mvStreamImg[STREAM_IMG_OUT_FULL_APP]->getImgFormat(),
                mvStreamImg[STREAM_IMG_OUT_FULL_APP]->getImgSize().w,
                mvStreamImg[STREAM_IMG_OUT_FULL_APP]->getImgSize().h,
                mSensorParams.mode, mSensorParams.fps, mSensorParams.pixelMode,
                mSensorParams.vhdrMode,
                mSensorParams.size.w, mSensorParams.size.h);
    ...
    #if (IS_P1_LOGI)
    {
        ...
        strInfo += String8::format("Param["
            "N:m%d,p%d,c%d,q%d,t%d,b%d,a%d,i%d,r%d,w%d,v%" PRId64"_"
            "B:p%d,b%d,t%d,h%d,u%d,e%d,l%d,r%d,f%d,d%d,q%d,c%d,f%d,v%d,s%d,LL%d,SSC%d,SE%d,ST%d,SB%p,O:%d] ",
            // Param-iNt/eNum
            rParams.pipeMode/*m*/, rParams.pipeBit/*p*/, rParams.camResConfig.Raw/*c*/,
        ...
        strInfo += String8::format("E(e%d,l%d,r%d,u%d,d%d,q%d,c%d,f%d,s%d) ",
            ...
            mStereoCamMode/*s*/);
        strInfo += String8::format("M(m0x%x,p0x%x,c0x%x,q%d,t%d,b%d,a%d,u%d,d%d,"
            "r%d,i%d,v%" PRId64 ") ",
            mPipeMode/*m*/, mPipeBit/*p*/, mCamResConfig.Raw/*c*/, mResizeQuality/*q*/,
            mTgNum/*t*/, mBurstNum/*b*/, mBatchNum/*a*/, mTuningGroup/*u*/,
        ...
        strInfo += String8::format("Pool(IMG%p,RRZ%p,LCS%p,RSS%p,YUV%p,CRZ1%p,CRZ2%p) ",
            (mpStreamPool_full != NULL) ?
        ...
        //
        if (!meta_str.isEmpty()) {
            MY_LOGI3("strInfo[%zu]+meta_str[%zu]=[%zu]", strInfo.length(),
                meta_str.length(), (strInfo.length() + meta_str.length()));
            if ((strInfo.length() + meta_str.length()) > P1_LOG_LINE_LEN) // use-one-line
...
)
{
    ...
    if (mp3A) {
        ...
        //
        checkQualitySwitch(&(pAct->qualitySwitchState));
    }
    if (LOGI_LV1) {
...
)
{
    ...
    //
    if (mIsSeparatedSensorCfg && (!mNeedConfigSensor) && (!isActive()) &&
        (pFrame != NULL)) {
        ...
        if (err == OK && pMetaStreamBuffer != NULL) {
            pMetadata = pMetaStreamBuffer->tryReadLock(getNodeName());
            if (pMetadata != NULL) {
                MBOOL bTagExist = MFALSE;
                MBOOL sensorCfg = MFALSE;
                if (tryGetMetadata<MBOOL>(pMetadata,
                    MTK_P1NODE_CTRL_RECONFIG_SENSOR_SETTING, sensorCfg)) {
                    MY_LOGI2("MTK_P1NODE_CTRL_RECONFIG_SENSOR_SETTING(%d)",
                        sensorCfg);
                    if (sensorCfg)
                        mNeedConfigSensor |= MTRUE;
                    bTagExist = MTRUE;
                ...
                if (P1GET_FRM_NUM(pFrame) > (mInitReqNum + mBurstNum)) { // re-start case
                    MY_LOGI0("MTK_P1NODE_CTRL_RECONFIG_SENSOR_SETTING "
                        "exist(%d) value(%d) [F:%d R:%d]", bTagExist, sensorCfg,
                        P1GET_FRM_NUM(pFrame), P1GET_REQ_NUM(pFrame));
                }
            }
        } else {
            MY_LOGW("CANNOT get HAL-META (%d) [F:%d R:%d]", err,
                P1GET_FRM_NUM(pFrame), P1GET_REQ_NUM(pFrame));
        }
    }
    //
    ...
    // else if (mInitReqCnt == mInitReqNum) go-on the following flow
        ...
        mpTaskCtrl->sessionUnLock();
        U_if (pAct->qualitySwitchState == QUALITY_SWITCH_STATE_RECEIVED) {
            MY_LOGI0("Call TG-Dynamic-Switch State(0x%x) Value(0x%x)",
                pAct->qualitySwitchState, pAct->qualitySwitchValue);
            if (mp3A) {
                #if SUPPORT_3A
                mp3A->send3ACtrl(E3ACtrl_SetCCUTGDynamicSwitch,
                    (MINTPTR)0, (MINTPTR)0);
                #endif
            }
            setQualitySwitching(QUALITY_SWITCH_PROGRESS_WAIT_TO_SENDCMD,
                &(pAct->qualitySwitchValue));
            pAct->qualitySwitchState = QUALITY_SWITCH_STATE_PROCESSING;
        }
...
)
{
    ...
    mQualitySwitchingState = uState;
    if (mQualitySwitchingState == QUALITY_SWITCH_PROGRESS_NOT_START) {
        CAM_QUALITY defaultCamQty;
        mQualitySwitchingValue = defaultCamQty.Raw; // reset by default value
    } else if (pValue != nullptr) {
        mQualitySwitchingValue = (*pValue);
    }
    P1_ATOMIC_RELEASE;
};
...
)
{
    Mutex::Autolock _l(mQualitySwitchLock);
    if (pState != nullptr) {
        *pState = mQualitySwitchingState;
    }
    if (pValue != nullptr) {
        *pValue = mQualitySwitchingValue;
    }
};
...
{
    Mutex::Autolock _l(mQualitySwitchLock);
    return (mQualitySwitchingState != QUALITY_SWITCH_PROGRESS_NOT_START);
};
    ...
    {   //mProcessingQueueLock.lock();
        ...
        if (it == mProcessingQueue.end()) {
            ...
            char const * str[STREAM_IMG_NUM] =
                {"Raw-in", "YUV-in", "OPQ-in", "OPQ", "IMG", "IMG_APP", "RRZ", "LCS", "RSS", "YUV", "CRZ1", "CRZ2"};
    ...
    for (MUINT8 i = 0; i < (MUINT8)job.size(); i++) {
        ...
        if (LOGI_LV1) {
            ...
            for (size_t n = index; n < deqBuf.mvOut.size(); n += subsample) {
                if (deqBuf.mvOut[n].mPortID.index == PORT_IMGO.index) {
                    ...
                    MSize size_d = deqBuf.mvOut[n].mMetaData.mDstSize;
                    strInfo += String8::format(
                        "YUV%d(%dx%d) ", mIsBinEn, size_d.w, size_d.h);
                } else if (deqBuf.mvOut[n].mPortID.index == PORT_CRZO_R1.index) {
                    MRect crop_s = deqBuf.mvOut[n].mMetaData.mCrop_s;
                    MRect crop_d = deqBuf.mvOut[n].mMetaData.mCrop_d;
                    MSize size_d = deqBuf.mvOut[n].mMetaData.mDstSize;
                    strInfo += String8::format(
                        "CRZ1%d(%d-%d-%dx%d)(%d-%d-%dx%d)(%dx%d) ", mIsBinEn,
                        crop_s.p.x, crop_s.p.y, crop_s.s.w, crop_s.s.h,
                        crop_d.p.x, crop_d.p.y, crop_d.s.w, crop_d.s.h,
                        size_d.w, size_d.h);
                    ...
                    MSize size_d = deqBuf.mvOut[n].mMetaData.mDstSize;
                    strInfo += String8::format(
                        "CRZ2%d(%d-%d-%dx%d)(%d-%d-%dx%d)(%dx%d) ", mIsBinEn,
                        crop_s.p.x, crop_s.p.y, crop_s.s.w, crop_s.s.h,
                        crop_d.p.x, crop_d.p.y, crop_d.s.w, crop_d.s.h,
                        size_d.w, size_d.h);
                }
        ...
        if (!match || act->getType() == ACT_TYPE_INTERNAL || !isActive()) {
            ...
            if (IS_LMV(mpConnectLMV) && mp3A != NULL) {
                ...
                MUINT32 idxRRZ = act->vSubSetting[defIdx].portBufIdx[P1_OUTPUT_PORT_RRZO];
                if (
                    #if 0
...
MERROR
P1NodeImp::
setAaaInitSensorCfg(IHalSensor::ConfigParam & rCfg,
    MBOOL const bTargetModeReady, MUINT32 & rTargetMode,
    NS3Av3::ConfigInfo_T & r3aCfg, IHal3A_T * p3a)
{
    IHal3A_T * p3aH = p3a;
    IHal3A_T * p3aT = NULL;
    if (p3aH == NULL) {
    ...
    //
    if (!bTargetModeReady) { // TargetMode is UNKNOWN
        MUINT32 AETargetMode = MFALSE;  // vHDR OFF mode
        { // decide AETargetMode
        #if SUPPORT_VHDR
            if (mpVhdr != NULL) {
                AETargetMode = mpVhdr->getCurrentAEMode();
            } else {
                VHdrHal* pVhdr = NULL;
                pVhdr = VHdrHal::CreateInstance(LOG_TAG, getOpenId());
                if(pVhdr == NULL)
                {
                    MY_LOGE("pVhdr is NULL");
                    return DEAD_OBJECT;
                }
                MINT32 hdrHalMode = MTK_HDR_FEATURE_HDR_HAL_MODE_OFF;
                if (!tryGetMetadata<MINT32>(&mCfgHalMeta, MTK_HDR_FEATURE_HDR_HAL_MODE, hdrHalMode)) {

                   MY_LOGD2("Cannot get MTK_HDR_FEATURE_HDR_HAL_MODE in Configuration HAL metadata");

                }
                if( pVhdr->Init((MUINT32)hdrHalMode) != VHDR_RETURN_NO_ERROR)
                {
                    pVhdr = NULL;
                }
                if(pVhdr != NULL) {
                    AETargetMode = pVhdr->getCurrentAEMode();
                    pVhdr->Uninit();
                    pVhdr = NULL;
                } else {
    ...
    //
    {
        ...
        initExpoSetting.u4AETargetMode = rTargetMode;
        ...
        #else
        initExpoSetting.sensorCfg.HDRMode = MFALSE;
        #endif
        #if (P1NODE_USING_MTK_LDVT > 0)
        ...
        //
        MY_LOGI1(
            "SensorCfg(T:%d i:%d %dx%d s:%d b:%d c:%d h:%d f:%d t:%d d:%d) "
...
MERROR
P1NodeImp::
sensorConfig(IHalSensor::ConfigParam * pCfg)
{
    ...
    if (pCfg != NULL) {
        ...
        MUINT32 AETargetMode = 0;
        prepareAaaIspCfgInfo(config);
        MERROR res = setAaaInitSensorCfg(sensorCfg, MFALSE, AETargetMode,
...
#define P1_START_INFO_STR "Cam::%d "\
        ...
        "Ratio(%d) SensorCfg(T:%d i:%d %dx%d s:%d b:%d c:%d, h:%d f:%d t:%d d:%d) "\
        "ConfigPort[%d]:(0x%x) InitParam[R:%d B:%d D:%d Nd:%d Ul:%d Pb:%d "\
        "Rc:x%x Dt:%d Iq:%d F(DataPattern:x%x OffBin:x%x SensorNum:x%x RAW:x%x)]"\
...
#define P1_START_INFO_VAR getOpenId(),\
...
#if SUPPORT_VHDR
    {
        ...
        if (mpVhdr->Init((MUINT32)hdrHalMode) != VHDR_RETURN_NO_ERROR)
        {
            //mpVhdr->DestroyInstance(LOG_TAG); // instance always exist until process kill
            mpVhdr = NULL;
        }
        P1_TRACE_S_END(); // "P1:VHDR-init"
    }
#endif
    //
...
#if SUPPORT_3A
    {
        ...
        setAaaInitSensorCfg(sensorCfg, MTRUE, AETargetMode, config, mp3A); // execute E3ACtrl_GetAEInitExpoSetting
        P1_TRACE_S_END(); // "P1:3A-create-GetAEInitExpoSetting"
    }
#else
    setAaaInitSensorCfg(sensorCfg, MTRUE, AETargetMode, config); // setup sensor config param without 3A
#endif
    ...
    //
    if (mNeedConfigSensor) {
        U_if (OK != sensorConfig(&sensorCfg)) {
    ...
    if (mvStreamImg[STREAM_IMG_OUT_FULL] != NULL)
    {
        ...
        MBOOL secureOn = mvStreamImg[STREAM_IMG_OUT_FULL]->isSecure();
        IImageStreamInfo::BufPlanes_t planes =
            mvStreamImg[STREAM_IMG_OUT_FULL]->getBufPlanes();
        if (mvStreamImg[STREAM_IMG_OUT_FULL_APP] != NULL) {
            IImageStreamInfo::BufPlanes_t const & planes_ap =
                mvStreamImg[STREAM_IMG_OUT_FULL_APP]->getBufPlanes();
            if (P1_STRIDE(planes_ap, 0) > P1_STRIDE(planes, 0)) {
                planes = planes_ap;
            }
        }
        portInfo OutPort(
        ...
        // STREAM_IMG_OUT_FULL_APP Might not be configed
        MINT fmt = mvStreamImg[STREAM_IMG_OUT_FULL_APP]->getImgFormat();
        MBOOL secureOn = mvStreamImg[STREAM_IMG_OUT_FULL_APP]->isSecure();
        IImageStreamInfo::BufPlanes_t const & planes =
            mvStreamImg[STREAM_IMG_OUT_FULL_APP]->getBufPlanes();
        portInfo OutPort(
            PORT_IMGO,
            (EImageFormat)fmt,
            mvStreamImg[STREAM_IMG_OUT_FULL_APP]->getImgSize(),
            MRect(MPoint(0,0), mSensorParams.size),
            P1_STRIDE(planes, 0), P1_STRIDE(planes, 1), P1_STRIDE(planes, 2),
            0, // pureraw
            MTRUE/*IS_RAW_FMT_PACK_FULL(fmt)*/,  //packed
        ...
        MBOOL secureOn = mvStreamImg[STREAM_IMG_OUT_OPAQUE]->isSecure();
        portInfo OutPort(
            PORT_IMGO,
            (EImageFormat)mRawFormat,
            mSensorParams.size,
    ...
    if (mEnableSecure)
    {
        switch(mSecType)
        {
            case SecType::mem_protected:
                ...
                halCamIOinitParam.m_SecureCam.chk_handle = mSecStatusHeap.get() ? mSecStatusHeap->getBufVA(0) : 0;
                break;
    ...
    MY_LOGD1("ConfigPipe-Bit(%d)-ResCfg(0x%X)-N3d(%d)-Uni(%d)-Qlv(%d)-Func(0x%X)-sec(%d-%d-%#" PRIxPTR ")",
        halCamIOinitParam.m_pipelinebitdepth, halCamIOinitParam.m_CamResConfig.Raw,
        halCamIOinitParam.m_bN3D, halCamIOinitParam.m_UniLinkSel,
    ...
    *pSizePure = MSize(0, 0);
    ...
    if (mpISP) {
        ...
        MY_LOGI3("for ISP.Config (SubsampleCnt:%d) (RequestCnt:%d)",
            config.i4SubsampleCount, config.i4RequestCount);
    ...
    IMetadata const &inHalMetadata, IMetadata &halMetadata, MUINT32 const index)
{
    ...
    //rrzo
    MUINT32 port_index = act->vSubSetting[defIdx].portBufIdx[P1_OUTPUT_PORT_RRZO]; // use index[0] for general case
    if (port_index != P1_PORT_BUF_IDX_NONE) {
    ...
    if (act->qualitySwitchState != QUALITY_SWITCH_STATE_NONE) {
        switch (act->qualitySwitchState) {
            ...
            case QUALITY_SWITCH_STATE_DONE_ILLEGAL:
                ...
                break;
            case QUALITY_SWITCH_STATE_RECEIVED:
            case QUALITY_SWITCH_STATE_PROCESSING: // still in switching
            default:
                break;
        }
    ...
    //
    MBOOL qtySwitch = isQualitySwitching();
    {
    ...
    //
    MY_LOGI2("QUALITY_STATUS[%d](%d) - QUALITY_SWITCHING[%d] - " P1NUM_ACT_STR,
...
)
{
    ...
    for (out = 0; out < REQ_OUT_MAX; out++) {
        ...
        if (out == REQ_OUT_FULL_STUFF || out == REQ_OUT_RESIZER_STUFF ||
            ...
            out == REQ_OUT_YUV_RESIZER2_STUFF) {
        ...
        if (needStuff != IMG_BUF_SRC_NULL) {
            ...
            if (keepHeightInStuff) {
                ...
                dstSize.h = P1_STUFF_BUF_HEIGHT(
                    (((out == REQ_OUT_RESIZER) ||
                        (out == REQ_OUT_YUV_RESIZER1) ||
                        (out == REQ_OUT_YUV_RESIZER2)) ? MTRUE : MFALSE), mConfigPort);
...
)
{
    ...
    for (MUINT8 idx = 0; idx < act->mBatchNum; idx++) {
        if (OK != setupSubInfo(rAct, info, strInfo, idx)) {
...
)
{
    ...
    if ((mspSyncHelper != NULL) && (act->reqType == REQ_TYPE_NORMAL)) {
    ...
    if (mEnableDumpRaw && deqBuf.mvOut.size() > 0) {
        ...
        if ( (magicNum <= mIndexRawDump) || mContinueDumpCount > 0 )
        {
            ...
            for(size_t i = 0; i < deqBuf.mvOut.size(); i++) {
                ...
                sprintf(filename, "/sdcard/raw/p1_%u_%d_%d_%04dx%04d_%04d_%d.%s",
                    magicNum,
                    (int)mOpenId,
                    ...
                    (int)mSensorFormatOrder, extension.string());
...
hardwareOps_streaming()
{
    ...
    if (mpHwStateCtrl->isLegacyStandby()) {
        ...
        MERROR status = hardwareOps_enque(job, ENQ_TYPE_DIRECTLY, MFALSE,
            (MINT64)nShutterTimeUs, ENQ_ISP_TYPE_FALSE);
        if (OK != status) {
...
)
{
    ...
    if (mvStreamImg[STREAM_IMG_OUT_FULL] != NULL) {
        ...
        act->dstSize_full = mvStreamImg[STREAM_IMG_OUT_FULL_APP]->getImgSize();
        act->cropRect_full = MRect(MPoint(0, 0),
            (isFullBin) ? refSensorSize : mSensorParams.size);
    } else if (mvStreamImg[STREAM_IMG_OUT_OPAQUE] != NULL) {
    ...
    if ((pAppMetadata != NULL) && (pHalMetadata != NULL)) {
        ...
        if (needConvert) {
            if (useAppMeta) {
                ...
                if (mvStreamImg[STREAM_IMG_OUT_YUV_RESIZER1] != NULL) {
                    cropRect_yuv_resizer1_control.p = transform(tranActive2Sensor,
                            cropRect_metadata.leftTop());
                    cropRect_yuv_resizer1_control.s = transform(tranActive2Sensor,
                            cropRect_metadata.size());
                }
                if (mvStreamImg[STREAM_IMG_OUT_YUV_RESIZER2] != NULL) {
                    cropRect_yuv_resizer2_control.p = transform(tranActive2Sensor,
                            cropRect_metadata.leftTop());
                    cropRect_yuv_resizer2_control.s = transform(tranActive2Sensor,
                            cropRect_metadata.size());
                }
                MY_LOGI3("bIsBinEn(%d) " "meta" P1_RECT_STR "ctrl" P1_RECT_STR P1_RECT_STR P1_RECT_STR,
                ...
                if (bIsBinEn) {
                    ...
                    // crz1
                    if (mvStreamImg[STREAM_IMG_OUT_YUV_RESIZER1] != NULL) {
                        BIN_RESIZE(cropRect_yuv_resizer1_control.p.x);
                        BIN_RESIZE(cropRect_yuv_resizer1_control.p.y);
                        BIN_RESIZE(cropRect_yuv_resizer1_control.s.w);
                        BIN_RESIZE(cropRect_yuv_resizer1_control.s.h);
                    }

                    // crz2
                    if (mvStreamImg[STREAM_IMG_OUT_YUV_RESIZER2] != NULL) {
                        BIN_RESIZE(cropRect_yuv_resizer2_control.p.x);
                        BIN_RESIZE(cropRect_yuv_resizer2_control.p.y);
                        BIN_RESIZE(cropRect_yuv_resizer2_control.s.w);
                        BIN_RESIZE(cropRect_yuv_resizer2_control.s.h);
                    }
                }
                MY_LOGI3("bIsBinEn(%d) " "ctrl" P1_RECT_STR P1_RECT_STR P1_RECT_STR,
    ...
    */
    ...
    if (appFrame != NULL) {
        ...
        if (pHalMeta != NULL) {
            ...
            if (mpRegisterNotify != NULL) {
                ...
                if (tryGetMetadata<MINT32>(pHalMeta,
                    ...
                    if (isQualitySwitching() || EN_BURST_BATCH_MODE ||
                        appFrame->IsReprocessFrame() || isActiveButNotReady()) { // must after the 3A start ready
                        act->qualitySwitchState =
                            QUALITY_SWITCH_STATE_DONE_ILLEGAL;
                    } else {
                        act->qualitySwitchState = QUALITY_SWITCH_STATE_RECEIVED;
                        act->qualitySwitchValue = ((MUINT32)quality_ctrl);
                    }
                }
                if (act->qualitySwitchState == QUALITY_SWITCH_STATE_RECEIVED) {
                    if (mpCamIO != NULL) {
                        CAM_QUALITY currentQty;
                        MBOOL ret = mpCamIO->sendCommand(ENPipeCmd_GET_QUALITY,
                            (MINTPTR)NULL, (MINTPTR)&currentQty,(MINTPTR)NULL);
                        if (ret && act->qualitySwitchValue == currentQty.Raw) {
                            act->qualitySwitchState =
        ...
        if (meta_zsl_req && !act->streamBufImg[STREAM_IMG_IN_RAW].bExist) {
            ...
            if (IS_PORT(CONFIG_PORT_YUVO, mConfigPort) &&
                act->streamBufImg[STREAM_IMG_OUT_YUV_FULL].bExist) {
    ...
    if (act->reqType == REQ_TYPE_NORMAL || act->reqType == REQ_TYPE_INITIAL ||
        ...
        if (IS_PORT(CONFIG_PORT_YUVO, mConfigPort)
            && (0 == IS_OUT(REQ_OUT_YUV_FULL, act->reqOutSet))) {
    ...
    if (LOGI_LV1) { // P1_LOGI(1)
        ...
        act->msg += String8::format("[P1::REQ]" P1INFO_ACT_STR
            ...
            (appFrame != NULL) ? "New-Request" : "New-Dummy", eType, meta_withoutWb);
        act->msg += info;
        ...
        if ((mvStreamMeta[STREAM_META_OUT_APP] != NULL) &&
            ...
            if (mspSyncHelper != NULL) {
                ...
                MY_LOGI2("SyncHelper.ResultCheck(Cam:%d F:%d R%d)",
                    s_in.CamId, s_in.frameNum, s_in.requestNum);
                MBOOL res = mspSyncHelper->syncResultCheck(s_in, s_out);
                act->needNotifySyncRes = MFALSE;
    ...
    for (size_t i = 0; i < (deqBuf.mvOut.size()); i += sub_size) {
        ...
        if (index == PORT_RRZO.index) {
            ...
            MY_LOGE("Output port is not match : [%zu/%zu] index(0x%x) : "
                ...
                "YUVO(0x%x) CRZO1(0x%x) CRZO2(0x%x)",
...
{
    MUINT32 switchState = QUALITY_SWITCH_PROGRESS_NOT_START;
    MUINT32 qualityValue = 0;
    getQualitySwitching(&switchState, &qualityValue);
    U_if (switchState != QUALITY_SWITCH_PROGRESS_NOT_START &&
        switchState != QUALITY_SWITCH_PROGRESS_WAIT_FOR_CALLBACK) {
        if (pActState != nullptr && (*pActState) == QUALITY_SWITCH_STATE_NONE) // update current act state as switching
            (*pActState) = QUALITY_SWITCH_STATE_PROCESSING;
    }
    L_if (switchState != QUALITY_SWITCH_PROGRESS_WAIT_TO_SENDCMD) {
        return MFALSE;
    }
    MBOOL toSendCmd = MFALSE;
    L_if (mp3A) {
        DualZoomInfo_T info;
        #if SUPPORT_3A
        mp3A->send3ACtrl(E3ACtrl_GetDualZoomInfo, (MINTPTR)(&info), (MINTPTR)0);
        #endif
        if (info.bIsFrameSyncDone) {
            toSendCmd = MTRUE;
        }
        MY_LOGI0("IsFrameSyncDone(%d)", info.bIsFrameSyncDone);
    }
    if (toSendCmd) {
        MUINT32 setSwitchState = QUALITY_SWITCH_STATE_NONE;
        MUINT32 setSwitchProgress = QUALITY_SWITCH_PROGRESS_NOT_START;
        MBOOL ret = MFALSE;
        L_if (mpCamIO != NULL && mpRegisterNotify != NULL) {
            CAM_QUALITY camQty;
            camQty.Raw = qualityValue;
            ret = mpCamIO->sendCommand(ENPipeCmd_SET_QUALITY,
                (MINTPTR)(mpRegisterNotify->getNotifyQuality()),
                (MINTPTR)(camQty.Raw), (MINTPTR)NULL);
            if (!ret) {
                setSwitchState = QUALITY_SWITCH_STATE_DONE_REJECT;
                setSwitchProgress = QUALITY_SWITCH_PROGRESS_NOT_START;
                MY_LOGI0("sendCommand ENPipeCmd_SET_QUALITY fail(%d)", ret);
            } else {
                setSwitchState = QUALITY_SWITCH_STATE_DONE_ACCEPT;
                setSwitchProgress = QUALITY_SWITCH_PROGRESS_WAIT_FOR_CALLBACK;
                MY_LOGI0("sendCommand ENPipeCmd_SET_QUALITY OK(%d)", ret);
            }
            MY_LOGI0("setSwitchProgress(%d) setSwitchState(%d) ActState(%d)",
                setSwitchProgress, setSwitchState,
                ((pActState != NULL) ? (*pActState) : (-1)));
            setQualitySwitching(setSwitchProgress);
            if (pActState != nullptr) // update current act state after send command
                (*pActState) = setSwitchState;
        }
    }
    return toSendCmd;
};
...
checkBufferDumping(P1QueAct & rAct)
{
    ...
    if (mCamDumpEn == 0) {
        ...
        #ifdef MTKCAM_CCT_TUNING_FOR_LOWDRAM
        MINT32 noskipframe = property_get_int32("vendor.debug.camera.noskip.frame", 0);
        if( (nPreDumpIMGO == 0 || nDumpIMGO != 0) && !noskipframe){
            nPreDumpIMGO = nDumpIMGO;
            return;
        }
        nPreDumpIMGO = nDumpIMGO;
        #else
        ...
        if (act->reqType == REQ_TYPE_NORMAL) {
            ...
            #if 1
                if (act->streamBufImg[STREAM_IMG_OUT_FULL].bExist) {
